        ...
        for pid in p2pDict.keys():
            ...
            ## Look for SQL Server instances using known process/service/software names
                ...
                if instanceNameRegexStr:
                    raw_instance_name=instanceNameRegexStr.group(1).strip().lower()
                    if raw_instance_name.upper() == DEFAULT_SQL_INSTRANCE:
                        logger.debug('Found default ms sql server instance, using host name')
                    else:
                        instanceNameFound = instanceNameFound + '\\' + raw_instance_name

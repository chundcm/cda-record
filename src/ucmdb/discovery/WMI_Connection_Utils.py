...
def doWMI(client, wmiOSH, ip_address, ip_domain, hostForLinkOSH, host_cmdbid=None, host_key=None, host_macs=None, ucmdb_version=None):
    ...
    if len(interfaceList) > 0:
        ...
        _wmiQuerybios = 'SELECT BIOSVersion,ReleaseDate FROM Win32_BIOS'
        biosSet = client.executeQuery(_wmiQuerybios)
        if biosSet.next():
            try:
                biosVersion = biosSet.getString(1)
                releaseDate = biosSet.getString(2)
                if biosVersion.count(',') == 0:
                    bios_version = biosVersion
                else:
                    bios_version = biosVersion.replace(',', ' ')
                hostOSH.setStringAttribute('bios_version', str(bios_version.strip()))
                sfDate = SimpleDateFormat('yyyyMMdd')
                bios_date = sfDate.parse(str(releaseDate[:8]), ParsePosition(0))
                hostOSH.setDateAttribute('bios_date', bios_date)
            except Exception, e:
                logger.warn('Cannot discover the BIOS information:', str(e))

        _wmiQueryOSInstalledDate = 'SELECT InstallDate FROM Win32_OperatingSystem'
        osInstalledDateResult = client.executeQuery(_wmiQueryOSInstalledDate)
        if osInstalledDateResult.next():
            try:
                osInstalledDateStr = osInstalledDateResult.getString(1)
                if osInstalledDateStr[0:14]:
                    sfDate = SimpleDateFormat('yyyyMMddHHmmss')
                    osInstalledDate = sfDate.parse(osInstalledDateStr[0:14], ParsePosition(0))
                    hostOSH.setDateAttribute('os_installed_date', osInstalledDate)
            except Exception, e:
                logger.warn('Cannot discover the OS Installed Date :', str(e))

        _wmiQueryService = 'SELECT DisplayName FROM Win32_Service'
        resultSet = client.executeQuery(_wmiQueryService)
        if not resultSet.next():
            logger.warn('Cannot discover Windows Services, check if have permission to get WinServices from Win32_Service')
...
def mainFunction(Framework):
    ...
    for credential in credentials:
        ...
        try:
            ...
            errormessages.resolveAndAddToObjectsCollections(strException, protocolName, warningsList, errorsList)
        except:

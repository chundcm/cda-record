...
DISCOVERY_STATUS_NOT_DISCOVERED = 0
DISCOVERY_STATUS_NEW = 1
DISCOVERY_STATUS_ACTIVE = 2
DISCOVERY_STATUS_DEACTIVE = 3
DISCOVERY_STATUS_MANUALLY_ADDED = 4
DISCOVERY_STATUS_NOT_CONFIRM = 5
...
def createScpOSHV(container, parent, type, host, port, context, shell=None, localIP=None, dnsServers=None):
    ...
    if netutils.isValidIp(host):
        ...
        if shell:
            try:
                ...
                logger.debug('Fail to resolve host name by nslookup:%s' % host)
    ...
    for ipAddress in ipAddresses:
        ...
        if parent:
            cplinkOsh = modeling.createLinkOSH('consumer_provider', parent, scpOsh)
            cplinkOsh.setEnumAttribute('discovery_status', DISCOVERY_STATUS_ACTIVE)
...
def createScpOsh(container, type, ip, port=None, context=None, hostname=None):
    ...
    scpOsh.setEnumAttribute('discovery_status', DISCOVERY_STATUS_NOT_DISCOVERED)
...
def createOshById(ci_class, ci_id):
    return ObjectStateHolder(ci_class, CmdbObjectID.Factory.restoreObjectID(ci_id))

...
_QRY_VERSION = "SELECT GETVARIABLE('SYSIBM.VERSION') as DB2VERSION FROM SYSIBM.SYSDUMMY1"
...
def get_db2_version(ls, db2SubsystemName):
    output = ls.evExecDb2Query(db2SubsystemName, _QRY_VERSION)
    if output.isSuccess() and len(output.cmdResponseList) > 0:
        results = output.cmdResponseList
    
    if results:
        for result in results:
            if isNotNull(result) and isNotNull(result[0]) and result[0] == 'DB2VERSION':
                continue
            if isNotNull(result) and isNotNull(result[0]):
                return result[0]
...
def osh_createDb2SubsystemOsh(lparOsh, db2Subsystem, db2_version = None):
    str_name = 'name'
    ...
    db2SubsystemOsh.setAttribute(str_discovered_product_name, 'DB2 for z/OS')
    ...
    db2SubsystemOsh.setAttribute('vendor', 'ibm_corp')
    if db2_version:
        db2SubsystemOsh.setAttribute('version', db2_version)
...
def osh_createDsgAndMemberOsh(db2SubsystemName, db2SubsystemOsh, dsg):
    ...
    return (_vector, dsgOsh)

def osh_createDb2Schema(db2SubsystemOsh, db, dsgOsh):
    str_name = 'name'
    LinkOsh = None
    if UCMDB_VERSION < 9:
        str_name = 'data_name'
    ...
    if isNotNull(db) and isNotNull(db[0]):
        ...
        if dsgOsh is not None:
           db2DbOsh.setContainer(dsgOsh)
           LinkOsh = modeling.createLinkOSH('containment', db2SubsystemOsh, db2DbOsh)
        else:
            db2DbOsh.setContainer(db2SubsystemOsh)
        return (db2DbOsh, LinkOsh)
...
def processDb2Subsystem(ls, Framework, lparOsh, lparname, useDsnRexx = 1):
    ...
    for db2 in db2Subsystems:
        ...
        if UCMDB_VERSION < 9:
            str_name = 'data_name'
            #str_discovered_product_name = 'data_name' # duplicated on purpose
        db2Version = get_db2_version(ls, db2[0])

        # create DB2 subsystem OSH ---------------------------------------------
        db2SubsystemOsh = osh_createDb2SubsystemOsh(lparOsh, db2, db2Version)
        ...
        if isNotNull(db2[0]) and isNotNull(db2[3]):
            ...
            if isNotNull(createDsgs) and string.lower(createDsgs) == 'true':
                ...
                for dsg in dsgs:
                    dsgdata_vector,dsgOsh = osh_createDsgAndMemberOsh(name, db2SubsystemOsh, dsg)
                    _vector.addAll(dsgdata_vector)
            ...
            if isNotNull(createDbs) and string.lower(createDbs) == 'true':
                ...
                if useDsnRexx:
                    ...
                    for db in dbs:
                        (db2Osh,LinkOsh) = osh_createDb2Schema(db2SubsystemOsh, db, dsgOsh)
                        if isNotNull(db2Osh):
                            _vector.add(db2Osh)
                        if isNotNull(LinkOsh):
                            _vector.add(LinkOsh)

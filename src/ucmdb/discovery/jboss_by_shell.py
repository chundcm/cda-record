...
def DiscoveryMain(Framework):
    ...
    try:
        ...
        discoverTnsName = Boolean.parseBoolean(Framework.getParameter('discoverTnsName'))
        if discoverTnsName:
        ...
        for process in jbossV3to6Processes:
            try:
                ...
                if not jbossHomePath:
                    raise Exception('Failed to get JBoss Home. Aborting')
...
#                return ObjectStateHolderVector()
        ...
        for process in jbossV7StandaloneProcesses:
            ...
            try:
                ...
                if not standaloneConfigFile.content:
                    logger.warn('Standalone config file was empty %s. Ignore.' % standaloneConfigPath)
                    continue
                ...
                serverName = serverName.split(".xml")[0]
                # If not found server name, parse it from jboss.server.home.dir
                if not serverName:
                    serverHomeDir = serverSystemProperties.get('jboss.server.home.dir')
                    if serverHomeDir:
                        serverName = path.baseName(serverHomeDir)
                if not serverName:
                ...
                connectorResources = profile.getConnectorResources()
                if connectorResources:
                    for connectorName, connectorSocketBing in connectorResources.items():
                        port = None
                        try:
                            port = int(connectorSocketBing)
                        except ValueError:
                            try:
                                port = socket.getservbyname(connectorSocketBing.lower())
                            except (Exception, JException): logger.debug('Failed to get port for ', connectorSocketBing)
                        if port:
                            for address in ipAddressList:
                                endpoint = netutils.createTcpEndpoint(address, port)
                                logger.debug('Found Connector Endpoint: %s' % endpoint)
                                serverEndpoints.append(endpoint)

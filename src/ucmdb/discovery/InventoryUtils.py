...
JOB_PARAMETER_UPLOAD_SCANNER_TO_AGENT_PATH = 'uploadScannerToAgentPath'
...
def generateScanFileName(Framework, extension=SCANFILE_EXTENTION):
    ...
    nodeName = Framework.getTriggerCIData('hostName')
    jobId = Framework.getDiscoveryJobId()
    scheduleName = None
    try:
        try:
            scheduleName = Framework.getParameter('scheduleName')
        except:
            logger.warn('No schedule name found. Will treat as generic flow.')
    except JException, e:
        logger.warn('No schedule name found. Will treat as generic flow.')
    if not scheduleName:
    ...
    return scheduleName + CollectorsConstants.XMLENRICHER_FILENAME_SEPARATOR + udUuid + CollectorsConstants.XMLENRICHER_FILENAME_SEPARATOR + nodeName + '.' + extension
...
#            props.setProperty('ssh-log-level', '7')
            ...
            resolveBaseDir(Framework, shell)
...
def copyLocalFileToRemote(Framework, localPath, remotePath, reportError=1):
    ...
    shell = shellutils.ShellUtils(client, skip_set_session_locale=True)
    if shell.checkSizeForCopy(localPath, remotePathTmp):
...
def resolveBaseDir(Framework, shell, isDDMiMigrate=None):
    ...
    if resolvedBaseDir is None:
        ...
        uploadScannerToAgentPath = Boolean.parseBoolean(Framework.getParameter(JOB_PARAMETER_UPLOAD_SCANNER_TO_AGENT_PATH))
        uploadScannerToAgentPath = uploadScannerToAgentPath and (Framework.getConnectedClient().getClientType() == ClientsConsts.DDM_AGENT_PROTOCOL_NAME) and shell.isWinOs()
        logger.debug('Resolving base dir, upload scanner to agent path: ', uploadScannerToAgentPath )

        if uploadScannerToAgentPath:
            clientOptions = Framework.getConnectedClient().getOptionsMap()
            resolvedBaseDir = clientOptions.get(AGENT_OPTION_BASEDIR)
            resolvedBaseDir = resolvedBaseDir.strip()
            logger.debug('Resolving base dir, agent base dir is: ', resolvedBaseDir)

        if not resolvedBaseDir:

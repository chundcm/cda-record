...
def getCommandOutput(command, client, timeout=0):
    ...
    if client.getLastCmdReturnCode() == 0 and result:
        ...
        if result and (re.search('not found', result, re.I) or re.search('no such file', result, re.I)) and re.search(
                'zoneadm\s', command):
...
def parseVirtualCpusOutput(cpuOutput):
    virtualCpusById = {}
    results = re.findall(
        r"Status of virtual processor (\d+) as of[\:\w\d\s\/.-]+The (.*?) processor operates at (\d+) MHz", cpuOutput)
    if results:
        for row in results:
            cpuId = int(row[0])
            cpuName = row[1]
...
def createZoneFileSystemObjects(zone, globalZoneOsh, resultsVector):
    for fileSystem in zone.fileSystems:
        ...
        localZoneDiskOsh.setAttribute('filesystem_type', fileSystem.type)
...
def _getIsZoneReportableFunction(networking):
    ...
    return lambda z: _zoneHasLowestMac(z) and (
                _zoneHasUuid(z) or _zoneHasNetworking(z, networking) or _zoneHasHostname(z))
...
def discoverZones(client, username, protocolName, framework, resultsVector, reportBasicTopology=0):
    ...
    if notRunningExclusiveZones:
        ...
        logger.warn(
            "The following exclusive zones are not in running state and won't be reported, since networking information is not available: %s" % notRunningExclusiveZoneNames)
    ...
    globalZoneOsh = modeling.createOshByCmdbIdString('unix', globalZoneCmdbId)
    globalZoneOsh.setStringAttribute('platform_vendor', PlatformVendors.Solaris)
    resultsVector.add(globalZoneOsh)

    hypervisorOsh = createHypervisorObject(globalZoneOsh, resultsVector)

    zoneOshByName = {}
    report_poweredOff_zones = framework.getParameter('reportPoweredOffZones')
    for zone in zones:
        if zone.status == Zone.STATUS_RUNNING or report_poweredOff_zones == 'true':
    ...
    if not reportBasicTopology:
        ...
        for zone in zones:
            if zone.fileSystems and (zone.status == Zone.STATUS_RUNNING or report_poweredOff_zones == 'true'):
...
def DiscoveryMain(Framework):
    ...
    reportNonGlobalZoneWarning = Boolean.parseBoolean(Framework.getParameter('reportNonGlobalZoneWarning'))

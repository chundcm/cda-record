...
class A10Discoverer:
    ...
    def discoverVirtualServers(self, a10_vthunder):
        ...
        for virtualServer in virtualServers:
            ...
            logger.debug(virtualServer.groupName)
    ...
    def discoverGroups(self, groupNameToVirtualServer, a10_vthunder):
        ...
        for group in groups:
            ...
            logger.debug(group.server_name)
            self.OSHVResult.add(groupOsh)

            txt = (group.name).split(":")
            if len(txt) >= 2:
                logger.debug('The group name is %s .' % txt[1])
                if groupNameToVirtualServer.has_key(txt[1]):
                    virtualServerHelper = groupNameToVirtualServer[txt[1]]
                    virtualServerHelper.linkToContainingCluster(groupOsh, txt[1])
                else:
                    logger.debug('Group %s is not related to any virtual server.' % group.name)
            else:
                logger.debug('The group name is %s .' % group.name)
    ...
    def discoverGroupMembers(self, serverNameToGroup):
        ...
        for groupMember in groupMembers:
            ...
            gmname = (groupMember.server_name).split(":")
            if len(gmname) >= 2:
                logger.debug("groupname%s" %gmname[1])
                tst = gmname[1].strip()
                if serverNameToGroup.has_key(tst):
                    groupOsh = serverNameToGroup[tst]
                    self.OSHVResult.add(modeling.createLinkOSH('member', groupOsh, serviceAddressOsh))
            else:
                logger.debug("server name is %s" %groupMember.server_name)
                if serverNameToGroup.has_key(groupMember.server_name):

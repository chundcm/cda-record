...
class Manager:
    ...
    def getProtocols(self, name, ip, domain):
        ...
        return self._getProtocolManager().getProtocolParameters(name,
                                                                ip,
                                                                domain)
    def getProtocolParameters(self, name, ip):
        return self._getProtocolManager().getProtocolParameters(name, ip)
...
def createDictionaryManager():
    from com.hp.ucmdb.discovery.library.credentials.dictionary \
        import ProtocolDictionaryManager
    return Manager(ProtocolDictionaryManager)

MANAGER_INSTANCE = createInfraManager()
DICTIONARY_MANAGER_INSTANCE = createDictionaryManager()

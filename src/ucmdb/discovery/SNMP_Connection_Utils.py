...
from  java.lang import Exception as JavaExcdeption
...
def isIPv6Overflow(probeRangesForIPv6):
    ...
    for probeRange in probeRangesForIPv6:
        totalIpCount += long(probeRange.getRangeSize())
...
def discoverClientInRange(Framework, client, probeRange, excludePatterns=None, reportFakeNode=None):
    ...
    while ip.compareTo(endIP) <= 0:
        ...
        if icmp_utils.shouldPingIp(ipStr, excludePatterns, None):
            ...
            if len(pingResult) > 0:
                ...
                clientDiscovery(Framework, pingResult[0], reportFakeNode)
            ipForICMPList = []
        else:
            filteredIpCount += 1
        ip = ip.nextIP()
...
def clientDiscovery(Framework, ip, reportFakeNode):
    (vec, errObj) = mainFunction(Framework, 1, ip, reportFakeNode)
...
def _transformOctetStringtoIPv6(rawValue):
    result = None
    if rawValue:
        try:
            b = [("%2x" % int(x)).replace(" ", "0") for x in rawValue.strip().split(".")]
            ip_str = ":".join([ b[x] + b[x+1] for x in range(0, len(b), 2)])
            result = ip_addr_util.IPAddress(ip_str)
        except:
            logger.debugException('String %s does not appear to be a valid IPv6' % rawValue)
    return result

def _parseIPv6FromIndex(rawIndexValue):
    rawIndexVal = rawIndexValue.split('.') #split it to array
    ifIndex = int(rawIndexVal[0])
    if len(rawIndexVal) > 18:
        rawIpValue = ".".join(rawIndexVal[1:])
    else:
        rawIpValue = ".".join(rawIndexVal[2:])
    ipv6Addr = _transformOctetStringtoIPv6(rawIpValue)
...
def getNetPrinterNameAndSN(client):
    '''
    if Net Printers supports net Printer-MIB then get  sysmane and serialnumber from  prtGeneralPrinterName
    '''
    snmpQuery = '1.3.6.1.2.1.43.5.1.1.16,1.3.6.1.2.1.43.5.1.1.17,string,1.3.6.1.2.1.43.5.1.1.16,string,1.3.6.1.2.1.43.5.1.1.17,string'
    table = client.executeQuery(snmpQuery)#@@CMD_PERMISION snmp protocol execution
    ...
    try:
        if table.next():
            ...
            SerialNumber = table.getString(4).strip()
            return sysname, SerialNumber
    except:
...
def doSnmp(client, isClient, snmpOSH, ip_address, ip_domain, Framework, host_cmdbid, host_key, host_macs):
    ...
    # system table
    snmpQueryBasic = '1.3.6.1.2.1.1.1,1.3.6.1.2.1.1.2,string,1.3.6.1.2.1.1.2,string,1.3.6.1.2.1.1.5,string'
    table = client.executeQuery(snmpQueryBasic)#@@CMD_PERMISION snmp protocol execution
    ...
    snmpQueryMore = '1.3.6.1.2.1.1.1,1.3.6.1.2.1.1.2,string,1.3.6.1.2.1.1.2,string,1.3.6.1.2.1.1.4,string,1.3.6.1.2.1.1.5,string,1.3.6.1.2.1.1.6,string'
    table = client.executeQuery(snmpQueryMore)
    try:
        if table.next():
            description = table.getString(2)
            oid = table.getString(3)
            sysContact = table.getString(4)
            sysname = table.getString(5)
            sysLocation = table.getString(6)
    except:
        logger.debugException('')
    ...
    if className == 'netprinter':
        sysname_temp, serialNumber = getNetPrinterNameAndSN(client)
        sysname = sysname or sysname_temp
...
#    upTime = client.executeQuery(snmpQuery)
    ...
    interfaceList = getInterfaceDict(client)
    ...
    if len(interfaceList) > 0:
        ...
        try:
            ...
            if sysContact:
                hostOSH.setAttribute('discovered_contact', sysContact)
            if sysLocation:
                hostOSH.setAttribute('discovered_location', sysLocation)
...
#            setLastBootDate(hostOSH, upTime, today, Framework)
    ...
    if not serialNumber and className == 'netprinter':
        sysname, serialNumber = getNetPrinterNameAndSN(client)
    if serialNumber:
...
def doSnmpFakeNode(client, isClient, snmpOSH, ip_address, ip_domain, Framework, host_cmdbid, host_key, host_macs):
    ...
    if len(interfaceList) > 0:
        ...
        try:
            hostOSH = modeling.createHostOSH(ip_address, hostClassName="node", osName=None, machineName=None, machineBootDate=None, filter_client_ip=None)
            if (sysname != None) and (sysname != ''):
                hostOSH.setAttribute('host_hostname', sysname)
            if sysContact:
                hostOSH.setAttribute('discovered_contact', sysContact)
            if sysLocation:
                hostOSH.setAttribute('discovered_location', sysLocation)
            ...
            logger.warn('Cannot create Host object. ')
            raise ex
        ...
        Arp_CacheOsh = ObjectStateHolder('arpcache')
        Arp_CacheOsh.setAttribute('name', 'ArpCache')
        Arp_CacheOsh.setContainer(hostOSH)
        vector.add(Arp_CacheOsh)
        snmpOSH.setContainer(hostOSH)
        if className == 'mainframe':
            modeling.setHostOsFamily(hostOSH, 'mainframe')
            ...
            if ipv6Map and interfaceOshToIndex:
                for ifIndex in ipv6Map.keys():
                    ...
                    if interfaceOsh:
                        ...
                        if (isClient == TRUE):
                            ipOSH.setAttribute('arp_mac', interfaceMacAddress)
                            if (ipv6 == ip_addr_util.IPAddress(ip_address)):#compare ipv6 by the formatting to avoid same IPv6 with different format
                                snmpOSH.setAttribute('arp_mac', interfaceMacAddress)
                    parent = modeling.createLinkOSH('containment', hostOSH, ipOSH)
                    vector.add(parent)
        except:
            logger.debug(str(sys.exc_info()))
        ...
        if isClient == TRUE:
            ...
            logger.reportWarningObject(errormessages.resolveError(msg, 'SNMP'))
            return vector
        hostOSH = modeling.createHostOSH(ip_address, hostClassName="node")
        ipOSH = modeling.createIpOSH(ip_address)
        link = modeling.createLinkOSH('contained', hostOSH, ipOSH)
        if((sysname != None) and (sysname != '') and (sysname != 'unknown')):
            hostOSH.setAttribute('snmp_sys_name', sysname)
...
def createClient(Framework, isClient, ip_address, ip_domain, protocol, priv_algorithm=None):
    if (isClient == TRUE):
        ...
        if priv_algorithm:
            properties.setProperty(Protocol.SNMP_PROTOCOL_ATTRIBUTE_PRIV_ALGORITHM, priv_algorithm)
...
def mainFunction(Framework, isClient, ip_address = None, reportFakeNode=None):
    ...
    for protocol in protocols:
        client = None
        priv_algorithm = None
        if Framework.getProtocolProperty(protocol, Protocol.SNMP_PROTOCOL_ATTRIBUTE_VERSION, "") == 'version 3':
            priv_algorithm = Framework.getProtocolProperty(protocol, Protocol.SNMP_PROTOCOL_ATTRIBUTE_PRIV_ALGORITHM, "")
        try:
            try:
                logger.debug('try to get snmp agent for: %s:%s' % (ip_address, ip_domain))
                client = createClient(Framework, isClient, ip_address, ip_domain, protocol, priv_algorithm)
                logger.debug('Running test connection queries')
                try:
                    testConnection(client)
                except JavaExcdeption, ex:
                    if priv_algorithm:
                        if priv_algorithm == 'AES192':
                            logger.debug('trying PrivAES192With3DESKeyExtension')
                            client = createClient(Framework, isClient, ip_address, ip_domain, protocol, 'PrivAES192With3DESKeyExtension')
                        elif priv_algorithm == 'AES256':
                            logger.debug('trying PrivAES256With3DESKeyExtension')
                            client = createClient(Framework, isClient, ip_address, ip_domain, protocol, 'PrivAES256With3DESKeyExtension')
                    else:
                        raise Exception(ex)
                    testConnection(client)
                ...
                if reportFakeNode:
                    _vector = doSnmpFakeNode(client, isClient, snmpOSH, ip_address, ip_domain, Framework, host_cmdbid, host_key, host_macs)
                else:

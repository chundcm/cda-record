...
def addInfoOnRegions(ls, cicsRegionOSH):
    ...
    if isNotNull(name):        
        ...
        if output.isSuccess() and len(output.cmdResponseList) > 0:
            ...
            #since the format of applid is completely different from the prior ones.
            m = re.search('APPLID\s*=\s*(.+)', ' '.join(output.cmdResponseList))
            if m:
                modeling.__setAttributeIfExists(cicsRegionOSH, 'cicsregion_applid', m.group(1).strip())
    return       
            ...
            for line in  output.cmdResponseList:
                ...
                if (m):
                    ...
                    if isNotNull(db2id):
                        ...
                        db2subsystemOSH.setAttribute(str_disc_name, 'DB2 for z/OS')

...
def getCommandOutput(command, shell, timeout=0, path = "/usr/sbin/"):
    ...
    if result:
        if shell.getLastCmdReturnCode() == 0:
            ...
            elif result and re.search("CIM_ERR_NOT_SUPPORTED|The platform is not supported", result, re.I):
                raise ValueError, "Destination is not a part of HP nPartition system"
...
def parseComplex(parstatusOutput):
    ...
    if match:
        ...
        logger.errorException('Discovered incomplete Complex: missing Serial Number')
        logger.reportError('Discovered incomplete Complex: missing Serial Number')
    ...
    if match:
        ...
        logger.errorException('Discovered incomplete Complex: missing Cabinet')
        logger.reportError('Discovered incomplete Complex: missing Cabinet')
...
# Sample:
# /opt/fcms/bin/fcmsutil /dev/fcd16 vpd
# Product Description    : HP PCI-X 2p 4Gb FC AND 2p 1000BT Adptr # HP is the vendor
# Part number            : AD194-60001
# Part Serial number     : JP49022315
# Part Serial number     : "JP49022315"
# Part Serial number     : 'JP49022315'
# Part Serial number     :'JP49022315'
# BIOS version           : 01.26
...
def parseFcmsutilOutput4DriverInfo(fcmsutilOutput):
    for line in split(fcmsutilOutput, mode = ONLY_LINES_WITH_CONTENT):
        ...
        if match:
            ...
            fchba_model = match.group(2)
            fchba_vendor = match.group(1)

            if driverVersion:
                driverVersion = driverVersion.strip()

            if fchba_model:
                fchba_model = fchba_model.strip()

            if fchba_vendor:
                fchba_vendor = fchba_vendor.strip()
            break
...
def fillFibreChannelWWN(fibreChannel, fcmsutilOutput):
    ...
    (driverVersion, fchba_model, fchba_vendor) = parseFcmsutilOutput4DriverInfo(fcmsutilOutput)
    fibreChannel.fchba_driverversion = driverVersion
...
def fillFibreChannelVPD(fibreChannel, fcmsutilVPDOutput):
    try:
        ...
        if not fibreChannel.fchba_model:
            fibreChannel.fchba_model = parseFcmsutilVpdOutput("Part number", fcmsutilVPDOutput)

        if not fibreChannel.fchba_vendor:
            desc = parseFcmsutilVpdOutput("Product Description", fcmsutilVPDOutput)
            match = re.match("\s*[\'\"]*(\S*)\s+(.*)", desc)
            if match:
                fibreChannel.fchba_vendor = match.group(1)
    except Exception, ex:
...
def createFibreChannelOsh(fibreChannel):
    ...
    fibreChannelOsh.setStringAttribute("fchba_driverversion", fibreChannel.fchba_driverversion)
    fibreChannelOsh.setStringAttribute("fchba_model", fibreChannel.fchba_model)
...
def discoverNparTopology(shell, vector, Framework):
    ...
    nparOsh = nparIdToNpar.get(getCurrentNparNumber(shell))
    if not nparOsh:
        return

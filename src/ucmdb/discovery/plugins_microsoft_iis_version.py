...
def modelConnectedPortByAppcmd(context, ispowershell = False):
    ...
    if context.client.getLastCmdReturnCode() == 0 and result:
        ...
        try:
            ...
            logger.debugException('Failed to parse site IIS configuration')
            # we don't want to raise and exception since the rest of info, like version, etc. is still there
            logger.reportWarning(
                'Exception during processing of plug-in: Failed to parse site IIS configuration. Failed to get binding info.')
            return

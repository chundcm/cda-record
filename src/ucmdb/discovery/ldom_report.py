...
class DomainHostBuilder:
    ...
    def build(self, domain):
        ...
        hostOsh = modeling.createCompleteHostOSH('host_node', domain._hostKey)
        if domain._hostname:
...
class LdomTopologyReporter:
    ...
    def _createHostByCmdbIdString(self, hostId):
        return modeling.createOshByCmdbIdString('host_node', hostId)
    ...
    def report(self, topology):
        ...
        ldomServerOsh = None
        if topology.report_poweredOff_ldoms == 'true' or controlDomain.state.value() == ldom.Domain.STATE_ACTIVE:
        ...
        for guestDomain in validGuestDomains:
            if topology.report_poweredOff_ldoms == 'true' or guestDomain.state.value() == ldom.Domain.STATE_ACTIVE:

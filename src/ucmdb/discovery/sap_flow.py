...
def connection_by_jco(main_fn):
    ...
    def decorator(framework):
        config = (flow.DiscoveryConfigBuilder(framework)
                ...
                .build())
        creds_id = config.credentialsId[0]
        inst_nr = config.sapas_instance[0]
        sapsystem = config.sapsystem[0]
        ...
        connection_client = cred_args[0][2]
        successful_cre = (sapsystem or creds_id, inst_nr, connection_client)
        if successful_cre:
            cred_args.insert(0 , successful_cre)
...
def iterate_over_args(main_fn, framework, cred_args, proto_name, stop_on_first):
    ...
    if first_ is None:
        ...
        # restore cred_args
        if not isinstance(cred_args, tuple):
        ...
        for args in cred_args:
            if (args and args[0] and (args[1] not in successful_ins_n)) or not args:
                try:
                    oshs, warnings_ = main_fn(framework, creds_manager, *args)
                    warnings.extend(warnings_ or ())
                    vector.addAll(oshs)
                    args and args[1] and successful_ins_n.append(args[1])

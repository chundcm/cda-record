...
from file_info_discoverers import WindowsFileInfoDiscovererByBatch,\
    ...
    UnixFileInfoDiscovererByLsAwk, SunOSFileInfoDiscovererByLsAwk, LinuxFileInfoDiscovererByLsAwk
...
PERL_DISCOVERER_BY_OS_TYPE = {
    'linux': LinuxFileInfoDiscovererByPerl,
    'sunos': SunOSFileInfoDiscovererByPerl
}

LSAWK_DISCOVERER_BY_OS_TYPE = {
    'linux': LinuxFileInfoDiscovererByLsAwk,
    'sunos': SunOSFileInfoDiscovererByLsAwk
}
...
class UnixFileSystem(FileSystem):
    ...
    def __init__(self, shell):
        ...
        self.__discovererType, self.__fallbackDiscovererType =  self.get_fs_discover(shell.getOsType().lower())
        self.FileSeparator = '/'
        logger.debug('UnixFileSystem created successfully, backend: %s' % self.__discovererType)

    def perl_exists(self):
        self._shell.execCmd('which perl')
        return not self._shell.getLastCmdReturnCode()

    def get_fs_discover(self, os_type):
        fs_discovery_method = getGlobalSetting().getPropertyStringValue('fs_discovery_method', "perl")
        if fs_discovery_method == 'perl' and self.perl_exists():
            return PERL_DISCOVERER_BY_OS_TYPE.get(os_type, UnixFileInfoDiscovererByPerl), LSAWK_DISCOVERER_BY_OS_TYPE.get(os_type, UnixFileInfoDiscovererByLsAwk)
        else:
            return LSAWK_DISCOVERER_BY_OS_TYPE.get(os_type, UnixFileInfoDiscovererByLsAwk), PERL_DISCOVERER_BY_OS_TYPE.get(os_type, UnixFileInfoDiscovererByPerl)
    ...
    def exists(self, path):
        ...
        result = self.__discovererType(self._shell).exists(path)
        if self._shell.getLastCmdReturnCode() == 0 or self._shell.getLastCmdReturnCode() == 1:
            return result
        else:
            return self.__fallbackDiscovererType(self._shell).exists(path)
    ...
    def getFiles(self, path, recursive=0, filters=[], fileAttrs=[]):
        ...
        if self._shell.getLastCmdReturnCode() != 0 and self._shell.getLastCmdReturnCode() != 1:
            files = self.__fallbackDiscovererType(self._shell, fileAttrs).getFiles(path)
    ...
    def getFile(self, path, fileAttrs=[], includeAll=0):
        ...
        perlFileDiscoverer = self.__discovererType(self._shell, fileAttrs)
        path = self.__normalizePath(path)
        result = perlFileDiscoverer.getFile(path)
        if self._shell.getLastCmdReturnCode() == 0 or self._shell.getLastCmdReturnCode() == 1:
            return result
        else:
            return self.__fallbackDiscovererType(self._shell, fileAttrs).getFile(path)
...
class WindowsFileSystem(FileSystem):
    ...
    def __normalizePath(self, path):
        r''' @types: str -> str
        skip normalization for network path
        @raise ValueError: Path to wrap with quotes is empty
        '''
        if not path.startswith('\\\\'):
            path = _NT_PATH.normalizePath(_NT_PATH.wrapWithQuotes(path))
        return path

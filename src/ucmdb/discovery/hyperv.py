...
def getSwitchPorts(wmiProvider):
    ...
    queryBuilder.addWmiObjectProperties('ElementName', 'SystemName', 'Name', 'PortNumber')
    ...
    for row in results:
        ...
        switchPort.portNumber = row.PortNumber
...
def getSwitchPortsV2(wmiProvider):
    ...
    queryBuilder.addWmiObjectProperties('ElementName', 'Name', 'SystemCreationClassName', 'SystemName', 'PortNumber')
...
def getEthernetPortsByClassName(wmiProvider, className = None):
    ...
    for row in results:
        ...
        if deviceId in deviceIdList:
            portsByDeviceId[row.SystemName] = port
        else:
...
def getVLansDiscoverer(namespace):
    if namespace == WmiNamespaceLookUp.HYPERV_WMI_NAMESPACE_V2: return getVlansV2
    return getVlans


def getVlansV2(wmiProvider, vlansBySystem):
    queryBuilder = wmiProvider.getBuilder('Msvm_EthernetSwitchPortVlanSettingData')
    queryBuilder.addWmiObjectProperties('InstanceID', 'AccessVlanId')

    wmiAgent = wmiProvider.getAgent()
    results = wmiAgent.getWmiData(queryBuilder)
    for row in results:
        instanceID = row.InstanceID
        accessVlanId = row.AccessVlanId

        if not instanceID or re.match(r"Microsoft:Definition", instanceID, re.I):
            continue
        bulk = instanceID.split('\\')
        vlanEndpoint = VlanEndpoint(bulk[1])
        try:
            vlanEndpoint.system = bulk[0].split(':')[1]
        except:
            logger.warn("Failed to convert to get systemid, check the instanceID format")
        vlanEndpoint.name = bulk[1]
        try:
            vlanEndpoint.vlanId = int(accessVlanId)
        except:
            logger.warn("failed to convert vlan id value '%s'" % accessVlanId)

        vlansBySystem[vlanEndpoint.name] = vlanEndpoint


def getVlans(wmiProvider, vlansBySystem):
    ...
    for row in results:
        ...
        vlanEndpoint = VlanEndpoint(bulk[1])
        try:
            result = re.search(r'(\w+):(\w+):(\w+)', bulk[0])
            vlanEndpoint.system = result.group(3)
        except:
            logger.warn('Failed to convert to get systemid, check the instanceID format')

        vlanEndpoint.name = bulk[1]
        try:
            vlanEndpoint.vlanId = int(vlanNumber)
        except:
            logger.warn("failed to convert vlan id value '%s'" % vlanNumber)
...
def createManagementPartition(hostId, resultsVector):
    ...
    managementPartitionOsh.setStringAttribute('platform_vendor', PlatformVendors.Hyperv)
...
def createVirtualMachine(vm, hypervisorOsh, resultsVector, reportMsStandardUuids=0):
    ...
    vmOsh = modeling.createCompleteHostOSH('host_node', lowestMac)
...
def createPort(port, parentOsh, physical_port_dict, physical_port_link, physical_port_vlan):
    ...
    port.osh = portOsh
    if not physical_port_dict.get((port.switchName, port.portNumber), None):
        physical_portOsh = ObjectStateHolder('physical_port')
        if port.portNumber:
            physical_portOsh.setIntegerAttribute('port_index', int(port.portNumber))
        else:
            physical_portOsh.setIntegerAttribute('port_index', 0)
        physical_portOsh.setAttribute('name', port.name)
        physical_portOsh.setContainer(parentOsh)
        physical_port_dict[(port.switchName, port.portNumber)] = physical_portOsh
        physical_port_vlan[port.name] = physical_portOsh

    physicalport = physical_port_dict.get((port.switchName, port.portNumber))
    linkOsh = modeling.createLinkOSH('realization', physicalport, portOsh)
    physical_port_link[portOsh.getAttribute("interface_name")] = (physicalport, linkOsh)
    return portOsh


def createVlan(vlan, nodeOsh, physicalPortOsh, resultsVector):
    if vlan:
        ...
        vlanOsh.setContainer(nodeOsh)
        vlan.osh = vlanOsh

        member_link = modeling.createLinkOSH('membership', vlanOsh, physicalPortOsh)
        resultsVector.add(vlanOsh)
        resultsVector.add(member_link)
...
#    getVlans(wmiProvider, switchesByName)
    vlansBySystem = {}
    getvlan = getVLansDiscoverer(namespace)
#    associateVlansAndPorts(wmiProvider, switchesByName)
    getvlan(wmiProvider, vlansBySystem)
    ...
    vm_dict = {}
    for guid, vm in vmsByGuid.items():
        ...
        # guid is systemid , report VM ,vlan ,phycial_port,interface topology
        for name, endpoint in vlansBySystem.items():
            if guid == endpoint.system:
                physical_portOsh = ObjectStateHolder('physical_port')
                physical_portOsh.setIntegerAttribute('port_index', 0)
                physical_portOsh.setAttribute('name', str(name))
                physical_portOsh.setContainer(vmOsh)
                for insterfaceid, ethernetPort in vm.syntheticPortsByDeviceId.items():
                    if ':' in insterfaceid:
                        insterfaceid = insterfaceid.split(':')[1]
                    if name == insterfaceid:
                        linkOsh = modeling.createLinkOSH('realization', physical_portOsh, ethernetPort.osh)
                        resultsVector.add(linkOsh)
                resultsVector.add(physical_portOsh)

                createVlan(vlansBySystem.get(name, None), vmOsh, physical_portOsh, resultsVector)
    ...
    if not reportBasicTopology:
        ...
        for switch in switchesByName.values():
            ...
            switch_dict[switch.name] = switchOsh
    #        for vlan in switch.vlanByName.values():
    #            createVlan(vlan, switch.osh, resultsVector)

            for port in switch.portsByName.values():
                portOsh = createPort(port, switchOsh, physical_port_dict, physical_port_link, physical_port_vlan)
                ...
                if port.connectedEthernetPort is not None and port.connectedEthernetPort.osh is not None:
                    ...
                    if physical_port_link.get(portOsh.getAttribute("interface_name"), None):
                        resultsVector.add(physical_port_link.get(portOsh.getAttribute("interface_name"))[0])
                        resultsVector.add(physical_port_link.get(portOsh.getAttribute("interface_name"))[1])
                        if port.name in vlansBySystem.keys():
                            interfaceName = port.name
                            createVlan(vlansBySystem.get(interfaceName, None), switchOsh,
                                       physical_port_link.get(portOsh.getAttribute("interface_name"))[0], resultsVector)

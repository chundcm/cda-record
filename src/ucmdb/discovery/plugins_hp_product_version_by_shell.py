...
class UcmdbServer9And10VersionShellPlugin(UcmdbTopologyReporter):
    ...
    def process(self, context):
        ...
        hostName = self.getPropertyByRegexp(r'dal\.datamodel\.host\.name=(.+)', content).strip()
        dbType = self.getPropertyByRegexp(r'dal\.datamodel\.db\.type=(.+)', content).strip()
        port = self.getPropertyByRegexp(r'dal\.datamodel\.port=(\d+)', content).strip()
        ...
        # QCCR1H117671: plugins_hp_product_version_by_shell.py incorrectly requires sid (ignores MSSQL server)
        # sid is looking for 'dal.datamodel.sid in the cmdb.conf file.  If using MSSQL server, this won't be there.
        # use the short name of hostName as the sid
        if not sid:
            sname = re.search(r'(\w+)', hostName)
            if sname:
                sid = sname.group(0)
        if hostName and dbType and port and sid:
        ...
        # QCCR1H117698: plugins_hp_product_version_by_shell.py tries to parse 'history' information in cmdb.conf
        # put the 'if' statement before history to see if it is UCMDB 9.x or not
        if discoveredVersion.startswith('9'):
...
class OmwVersionShellPlugin(RegistryBasedPlugin):
    def __init__(self):
        ...
        self.registryKeyAndValue = {
            'HKEY_LOCAL_MACHINE\\SOFTWARE\\Hewlett-Packard\\Measureware Agent\\CurrentVersion': 'Version'}

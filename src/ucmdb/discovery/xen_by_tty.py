...
def getProfileInstance(output):
    ...
    elif re.search('<domain\s+type=\'xen\'', output):
...
def getIfconfig(shell):
    ifaceNameToIfaceMacMap = {}
    output = executeCommand(shell, 'ifconfig -s')
    if output:
        lines = output.splitlines()
        for index in range(1, len(lines)):
            interfaceName = lines[index].split()[0]
            try:
                out = executeCommand(shell, 'ifconfig %s | grep ether' % interfaceName)
                if out:
                    match = re.match('.*ether\s+(..:..:..:..:..:..)', out)
                    if match:
                        ifaceMac = match.group(1)
                        if ifaceMac and netutils.isValidMac(ifaceMac):
                            ifaceNameToIfaceMacMap[interfaceName] = netutils.parseMac(ifaceMac)
            except:
                logger.debug('MAC address not found for interface: ', interfaceName)
    return ifaceNameToIfaceMacMap
...
def discoverInterfacesByIfconfig(shell):
    try:
        ...
        logger.debug('Fail to get result from \'ifconfig -a | grep HWaddr\', try ifconfig by grep ether.')
        return getIfconfig(shell)
...
def createVirtHostOsh(config):
    vHostOsh = modeling.createCompleteHostOSHByInterfaceList('host_node', config.interfaceList)

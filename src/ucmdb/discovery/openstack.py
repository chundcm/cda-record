...
class Vm:
    ...
    def report(self, region_osh, event_osh=None):
        ...
        osh.setStringAttribute('platform_vendor', PlatformVendors.OpenStack)
        vector.add(osh)
        if self.ips:
            for ip in self.ips:
...
class Hypervisor:
    ...
    def report(self, regionOsh):
        ...
        hostOsh.setAttribute("name", netutils.getShortHostName(self.name))
...
class Flavor:
    ...
    def report(self, region_osh):
        ...
        if self.swap_disk:
            flavorOsh.setAttribute("swap_disk_capacity", int(self.swap_disk))
        else:
            flavorOsh.setAttribute("swap_disk_capacity", 0)
...
class ServiceCatalog:
    def __init__(self, name, type, endpoints=None):
        self.name = name
        self.type = type
        self.endpoints = endpoints

class Endpoint:
    def __init__(self, id, publicUrl, region, internalUrl=None, adminUrl=None):
        self.id = id

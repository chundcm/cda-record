...
def _getWorkingDir(shell, pid, commandTemplate):
    ...
    if not output:
        raise ValueError("Failed to get current working directory, no output")
    return output
...
def DiscoveryMain(Framework):
    ...
    try:
        ...
        # make filtering for Running Weblogic processes
        weblogicProcess = filter(isWeblogicProcess, runningProcesses)
        # obvious to terminate discovery job without weblogic processes running
        if not (serverProcesses or weblogicProcess):
            logger.reportWarning("No Weblogic processes currently running")
            return ObjectStateHolderVector()
        # create catalog of processes by PID
        processByPid = applyMapping(lambda r: r.getPid(), runningProcesses)
        ...
        for rootDirPath, processes in processesByRootDirPath.items():
            ...
            try:
                ...
                deploymentTargetByName = applyMapping(lambda c: c.getName(), clusters)
                ...
                for server in servers:
                    ...
                    if runtime:
                        ...
                        jvm = jvmDiscoverer.discoverJvmByServerRuntime(runtime)
                        server.jvm = jvm
                ...
                deploymentTargetByName.update(applyMapping(lambda r: r.getName(), resolvedServers))
                # create reporters
                ...
                if platformTrait.majorVersion.value() < 9:
                    ...
                    for configuration in domainDescriptor.getJmsSystemResourceConfigurations():
                        ...
                        if descriptorFilePath is None:
                            logger.warn("JdbcSystemResourceConfiguration file path was None - ignoring (SUPPORT)")
                            continue
                        if not domainLayout.path().isAbsolute( descriptorFilePath ):
                            descriptorFilePath = domainLayout.path().join(domainLayout.getDomainConfigDirPath(), descriptorFilePath)
                        try:
...
def isWeblogicProcess(process):
    r''' Running Weblogic server command line contains propery 'startWebLogic'
    @types: process.Process -> bool
    '''
    return str(process.commandLine).find('startWebLogic') != -1

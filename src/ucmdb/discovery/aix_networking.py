...
class ShellDiscoverer:
    ...
    def discoverNetworking(self, aixNetworking = None):
        ...
        aixNetworking.FILTERED_IPS = self.discoverFilteredIPs()
    ...
    def discoverFilteredIPs(self):
        filterIPs=[]
        filterIPs.extend(self.discoverWParIPs())
        return filterIPs
    ...
    def discoverWParIPs(self):
        IPs = []
        DELIM = 'UCMDB'
        WPAR_IP_IDEX = 2
        LSWPAR_CMD = 'lswpar -N -c -q -d '   # output:ddmiwparUCMDBen0UCMDB16.155.192.233UCMDB255.255.248.0UCMDB16.155.199.255
        output = self._execCmd(''.join([LSWPAR_CMD,DELIM]), useCache = 1)
        if self.__shell.getLastCmdReturnCode() == 0:
            for line in output.split('\n'):
                tokens = line.split(DELIM)
                if len(tokens) == 5:
                    IPs.append(tokens[WPAR_IP_IDEX])
        else:
            logger.debug('Failed getting WPar IPs. %s' % output)
        return IPs

...
class Host(ManagedEntity):
    ...
    def __init__(self):
        ...
        self.powerState = None  # HostSystem.summary.powerState
        ...
        self.biosInfo = None  # HostSystem.hardware.biosInfo
        ...
        self.hyperThread = None  # HostSystem.config.hyperThread
        ...
        self._portGroupPorts_Mac_PG = {}
        self._portGroupPorts_Mac_Vswitch = {}
...
class VirtualMachine(ManagedEntity):
    ...
    def __init__(self):
        ...
        self.vmDiskAllocation = None  # VirtualMachine.storage.perDatastoreUsage.Committed:long
    ...
    def _findHostKeyFromVirtualNics(self):
        ...
        connectedNics = [vnic for vnic in vnics if
                         vnic and vnic.getConnectable() and vnic.getConnectable().isConnected()]
...
class ResourcePoolMapper(_vmware_vim_base.PropertiesMapper):
    ...
    def handleChildPools(self, childPoolReferences, resourcePoolObject):
        if childPoolReferences:
            resourcePoolObject.childPoolReferences = _vmware_vim_base.wrapMorefList(
                childPoolReferences.getManagedObjectReference())
...
class HostMapper(_vmware_vim_base.PropertiesMapper):
    ...
    def __init__(self, crossClientHelper):
        ...
        self._handlers['summary.runtime.powerState'] = self.handlePowerState
        ...
        self._handlers['config.hyperThread'] = self.handleHostHyperThread
        ...
        # BIOS
        self._handlers['hardware.biosInfo'] = self.handleBiosInfo
                ...
                if targets:
                    for target in targets:
                        ...
                        for lun in hostTarget.LUNs:
                            ...
                            if lunDisk:
                                hostObject.lunDiskToInterface[
                                    lunDisk.getCanonicalName() + scsiTopologyInterface.adapter] = scsiTopologyInterface
                                hostObject.lunDiskToLunId[
                                    lunDisk.getCanonicalName() + scsiTopologyInterface.adapter] = lun.getLun()
    ...
    def handleHostHyperThread(self, hyperThreadInfo, hostObject):
        if hyperThreadInfo is not None:
            hostObject.hyperThread = hyperThreadInfo.isActive()
        else:
            hostObject.hyperThread = False
    ...
    def handlePowerState(self, powerState, hostObject):
        if powerState is not None:
            hostObject.powerState = self.getCrossClientHelper().getEnumValue(powerState)
    ...
    def handleBiosInfo(self, biosInfo, hostObject):
        hostObject.biosInfo = biosInfo
...
class VirtualMachineMapper(_vmware_vim_base.PropertiesMapper):
    def __init__(self, crossClientHelper):
        ...
        self._handlers['storage.perDatastoreUsage'] = self.handleStorageAllocation
    ...
    def handleStorageAllocation(self, perDatastoreUsage, vmObject):
        virtualMachineUsageOnDatastore = perDatastoreUsage.getVirtualMachineUsageOnDatastore()
        if virtualMachineUsageOnDatastore:
            vmObject.vmDiskAllocation = long(
                virtualMachineUsageOnDatastore[0].getCommitted()) / 1024 / 1024  # covert to Mb
    ...
    def _handleVirtualDisk(selfself, vdisk, vmObject):
        ...
        virtualDisk.dsRef = backing and backing.getDatastore() and _vmware_vim_base.wrapMoref(
            backing.getDatastore()) or None
...
class TopologyDiscoverer(_vmware_vim_base.TopologyDiscoverer):
    ...
    def _getDatacenterMapper(self):
        ...
        return _vmware_vim_base.CompoundMapper(_ccHelper, ManagedEntityReferenceMapper(_ccHelper),
                                               ManagedEntityMapper(_ccHelper), DatacenterMapper(_ccHelper))
    ...
    def _getComputeResourceMapper(self):
        _ccHelper = self.getCrossClientHelper()
        return _vmware_vim_base.CompoundMapper(_ccHelper, ManagedEntityReferenceMapper(_ccHelper),
                                               ManagedEntityMapper(_ccHelper), ComputeResourceMapper(_ccHelper))
    ...
    def _getResourcePoolMapper(self):
        _ccHelper = self.getCrossClientHelper()
        return _vmware_vim_base.CompoundMapper(_ccHelper, ManagedEntityReferenceMapper(_ccHelper),
                                               ManagedEntityMapper(_ccHelper), ResourcePoolMapper(_ccHelper))
    ...
    def _getHostMapper(self):
        _ccHelper = self.getCrossClientHelper()
        return _vmware_vim_base.CompoundMapper(_ccHelper, ManagedEntityReferenceMapper(_ccHelper),
                                               ManagedEntityMapper(_ccHelper), HostMapper(_ccHelper))
    ...
    def _getVirtualMachineMapper(self):
        _ccHelper = self.getCrossClientHelper()
        return _vmware_vim_base.CompoundMapper(_ccHelper, ManagedEntityReferenceMapper(_ccHelper),
                                               ManagedEntityMapper(_ccHelper), VirtualMachineMapper(_ccHelper))
    ...
    def _getNetworkMapper(self):
        _ccHelper = self.getCrossClientHelper()
        return _vmware_vim_base.CompoundMapper(_ccHelper, ManagedEntityReferenceMapper(_ccHelper),
                                               NetworkManagedEntityMapper(_ccHelper), NetworkMapper(_ccHelper))
...
class ExtentBuilder(_vmware_vim_base._HasCrossClientHelper):
    ...
    def build(self, extent, parentOsh):
        ...
        if extent.blockCount and extent.blockSize:
            extentOsh.setDoubleAttribute('logicalvolume_size',
                                         (float(extent.blockCount) * float(extent.blockSize)) / (1024 * 1024))
...
class DiskBuilder(_vmware_vim_base._HasCrossClientHelper):
    ...
    def build(self, disk, parentOsh):
        ...
        diskOsh.setIntegerAttribute('disk_size',
                                    long((float(disk.capacity.block) * float(disk.capacity.blockSize)) / (1024 * 1024)))
...
class VirtualMachineBuilder(_vmware_vim_base._HasCrossClientHelper):
    ...
    def getVmHostClass(self, vm):
        ...
        return self._vmHostClassByGuestFamily.get(vmFamily) or 'host_node'
    ...
    def createHost(self, vm):
        ...
        hostOsh.setStringAttribute('platform_vendor', PlatformVendors.VMware)
        ...
        if hostName or vm.name:
            hostOsh.setStringAttribute('host_hostname', hostName or vm.name)
        if domainName:
    ...
    def createHostResource(self, vm):
        ...
        if cpuAllocation is not None:
            ...
            if cpuSharesInfo:
                ...
                if cpuSharesLevel:
                    hrOsh.setStringAttribute('vm_cpu_shares_level',
                                             self.getCrossClientHelper().getEnumValue(cpuSharesLevel))
        ...
        if memoryAllocation:
            ...
            if memorySharesInfo:
                ...
                if memorySharesLevel:
                    hrOsh.setStringAttribute('vm_memory_shares_level',
                                             self.getCrossClientHelper().getEnumValue(memorySharesLevel))
        ...
        toolsVersionStatus = vm.guest and vm.guest.getToolsVersionStatus()
        if toolsVersionStatus:
            hrOsh.setStringAttribute('vm_tools_version_status', toolsVersionStatus)
        toolsRunningStatus = vm.guest and vm.guest.getToolsRunningStatus()
        if toolsRunningStatus:
            hrOsh.setStringAttribute('vm_tools_running_status', toolsRunningStatus)
        ...
        if vm._primaryIpAddressString:
            hrOsh.setAttribute('management_ip', vm._primaryIpAddressString)

        if vm.vmDiskAllocation:
            hrOsh.setLongAttribute('vm_disk_allocation', vm.vmDiskAllocation)

        if vm.id:
            hrOsh.setStringAttribute('vm_id', vm.id)
...
class HostBuilder(_vmware_vim_base._HasCrossClientHelper):
    ...
    def createHost(self, host):
        ...
        if host.aboutInfo:
            ...
            hostOsh.setStringAttribute('discovered_os_vendor', host.aboutInfo.getVendor())

        if host.biosInfo:
            hostOsh.setStringAttribute('bios_version', host.biosInfo.getBiosVersion())
            biosReleaseDate = host.biosInfo.getReleaseDate()
            if biosReleaseDate:
                try:
                    biosReleaseDate = biosReleaseDate.getTime()
                except:
                    biosReleaseDate = biosReleaseDate.toGregorianCalendar().getTime()
                hostOsh.setDateAttribute('bios_date', biosReleaseDate)

        if host.serviceTag:
            hostOsh.setStringAttribute('serial_number', host.serviceTag)

        if host.connectionState:
            hostOsh.setStringAttribute('connection_state', host.connectionState)

        if host.powerState:
            hostOsh.setStringAttribute('power_state', host.powerState)
...
class EsxCpuBuilder(_vmware_vim_base._HasCrossClientHelper):
    ...
    def build(self, cpu, hostOsh):
        ...
        return modeling.createCpuOsh(cpuId, hostOsh, cpu.speed, cpu.coresCount, cpu.vendor, description, description,
                                     cpu.logicalProcessorCount, cpu.hyperThread)
...
class VmCreatedOrChangedEventListener(_vmware_vim_base.VmCreatedOrChangedEventListener):
    def __init__(self, client, crossClientHelper):
        _vmware_vim_base.VmCreatedOrChangedEventListener.__init__(self, client, crossClientHelper)
...
class VmCreatedOrChangedEventReporter(_vmware_vim_base.VmCreatedOrChangedEventReporter):
    def __init__(self, crossClientHelper, framework):
        _vmware_vim_base.VmCreatedOrChangedEventReporter.__init__(self, crossClientHelper, framework)
...
class LicensingDiscoverer(_vmware_vim_base._HasCrossClientHelper):
    ...
    def _getReservationFromReservationInfo(self, reservationInfo):
        ...
        reservation.state = reservationInfo.getState() and self.getCrossClientHelper().getEnumValue(
            reservationInfo.getState())
        return reservation
...
def getVmCreatedOrChangedEventListener(client, crossClientHelper):
    return VmCreatedOrChangedEventListener(client, crossClientHelper)


def getVmCreatedOrChangedEventReporter(crossClientHelper, framework):
    return VmCreatedOrChangedEventReporter(crossClientHelper, framework)

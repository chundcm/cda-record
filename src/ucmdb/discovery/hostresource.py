...
class InstalledSoftware():
    ...
    def __init__(self, softName, path=None,
               ...
               softwareType=None, description=None, comment=None, release=None):
        '''
...
class InstalledSoftwareBuilder(object):
    ...
    def build(self, installedSoftware):
        ...
        if installedSoftware.comment:
            softwareOSH.setAttribute('comments',
                                     installedSoftware.comment.strip())
        return softwareOSH
...
def _parseDateString(installDateString):
    ...
    if installDateString:
        for format in ['yyyyMMdd', 'yyyyMMddHHmmss.SSSSSS-000', 'EEE dd MMM yyyy HH:mm:ss aa zzz', 'MMM dd yyyy HH:mm','DD/MM/YYYY','D/MM/YYYY','D/M/YYYY','DD/M/YYYY','M/dd/yyyy']:
            if len(installDateString) == len(format):
                try:
                    ...
                    logger.debug("could not parse"+installDateString+ "as"+format)
                    pass
...
def createInstalledSoftwareOSH(hostOSH, installedSoftware):
    ...
    if installedSoftware.comment:
        softwareOSH.setAttribute('comments',
                                 installedSoftware.comment.strip())
    if installedSoftware.release:
        softwareOSH.setAttribute('release',
                                 installedSoftware.release.strip())
    softwareOSH.setContainer(hostOSH)
    return softwareOSH
...
def makeSoftwareOSH(softwareName, softwarePath, softwareVer, hostOSH,
                     ...
                     softwareProductCode=None, softwareVendor=None, softwareComment=None):
    '''
    ...
    return createSoftwareOSH(hostOSH, softwareName, path=softwarePath,
                     ...
                     installDate=softwareData, comment=softwareComment)


def makeSoftwareOSH2(hostOSH, sw_name, vendor='', version='', path='', swid='', swtype='', data='', description='', release=''):
    '''
    @deprecated: use createSoftwareOSH
    '''
    return createSoftwareOSH(hostOSH, sw_name.strip(),
                     ...
                     installDate=data.strip(), softwareType=swtype.strip(), release=release.strip())

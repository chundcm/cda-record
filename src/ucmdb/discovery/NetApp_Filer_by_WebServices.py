...
import netapp_topology
import netapp_cluster_discoverer
...
def buildvFilerLUNOsh(lun, hostOSH, volumeOshDict, lunOshDict):
    try:
        ...
        if netapp_webservice_utils.isValidString(lunID) and netapp_webservice_utils.isValidString(lunPath):
            ...
            lunOsh.setStringAttribute('name', lunPath)
            lunOsh.setStringAttribute('logicalvolume_fstype', "lun")
            ...
            if netapp_webservice_utils.isValidString(lunSerialNumber):
                lunOsh.setStringAttribute('serial_number', lunSerialNumber)
...
def getvFilerQtreeDetails(localFramework, wsConnection, hostOSH, physicalHostOSH):
    try:
        try:
            resultVector = ObjectStateHolderVector()
            qTreeRequest = NaElement('qtree-list')
            qTreeResponse = netapp_webservice_utils.wsInvoke(wsConnection, qTreeRequest)
            if not qTreeResponse:
                logger.warn('Failed getting QTREE information')
                return None
        except:
            logger.debug("Details Failed getting API results")
        try:
            logger.debug("Now Parsing results")
            qTreeList = qTreeResponse.getChildByName('qtrees') and qTreeResponse.getChildByName('qtrees').getChildren()
        except:
            logger.debug("Failed getting QTREE child items")
        try:
            for qTreeEntity in qTreeList:
                QTVolume = qTreeEntity.getChildContent('volume')
                QTName = qTreeEntity.getChildContent('qtree')
                QTreeOsh = ObjectStateHolder('file_system')
                volumeOsh = ObjectStateHolder('logical_volume')

                completeRelation = 1

                if not QTName:
                    logger.debug("No Name found for QTREE, Skipping")
                    completeRelation = 0
                else:
                    mountPoint = QTVolume + "/" + QTName
                    QTreeOsh.setStringAttribute('name', mountPoint)
                    QTreeOsh.setStringAttribute('mount_point', mountPoint)
                    QTreeOsh.setStringAttribute('filesystem_type', "qtree")
                    QTreeOsh.setContainer(hostOSH)
                    resultVector.add(QTreeOsh)

                if not QTVolume:
                    logger.debug("No Name found for Volume, Skipping")
                    completeRelation = 0
                else:
                    volumeOsh.setStringAttribute('name', QTVolume)
                    volumeOsh.setContainer(physicalHostOSH)
                    resultVector.add(volumeOsh)

                if completeRelation:
                    resultVector.add(modeling.createLinkOSH('dependency', QTreeOsh, volumeOsh))
                else:
                    logger.debug("Skipping creation of relation due to missing CI")
        except:
            logger.debug("Failed adding results to the result vector")
        return resultVector

    except:
        logger.debug("Failed creating QTREE")
        return None
...
def getvFilerLUNs(localFramework, wsConnection, hostOSH, volumeOshDict, lunOshDict):
    try:
        resultVector = ObjectStateHolderVector()
        ...
        if not lunResponseElement:
            logger.warn('lun-list-info information request failed')
            return None
        luns = lunResponseElement.getChildByName('luns').getChildren()
        for lun in luns:
            resultVector.addAll(buildvFilerLUNOsh(lun, hostOSH, volumeOshDict, lunOshDict))
        return resultVector
    except:
        excInfo = logger.prepareJythonStackTrace('')
        logger.warn('[' + SCRIPT_NAME + ':getvFilerLUNs] Exception: <%s>' % excInfo)
        pass
...
def DiscoveryMain(Framework):
    ...
    try:
        ...
        if wsConnection:
            ## Discover!
            netapp = NetAppFactory().createNetApp(Framework, wsConnection)
            if netapp:
                hostDetails = netapp.getHostDetails(nodeOshDict, vserverOshDict)
                OSHVResult.addAll(hostDetails)
                OSHVResult.addAll(netapp.getAggrs(nodeOshDict, aggrOshDict))
                OSHVResult.addAll(netapp.getVolumes(vserverOshDict, volumeOshDict, aggrOshDict))
                OSHVResult.addAll(netapp.getDisks(nodeOshDict, aggrOshDict))
                OSHVResult.addAll(netapp.getLUNs(vserverOshDict, volumeOshDict, lunOshDict))
                OSHVResult.addAll(netapp.getISCSIAdapter(vserverOshDict, scsiOshDict))
                OSHVResult.addAll(netapp.getISCSIInitiators(scsiOshDict, lunOshDict))
                OSHVResult.addAll(netapp.getFCAdapter(vserverOshDict, hbaOshDict))
                OSHVResult.addAll(netapp.getFCPInterface(hbaOshDict, fcpOshDict))
                OSHVResult.addAll(netapp.getFCPInitiators(fcpOshDict))
                if getSnapShotInfo.strip().lower() in ['yes', 'y', 'true', 1]:
                    OSHVResult.addAll(netapp.getVolumeSnapshots(volumeOshDict, snapshotOshDict))
                # if getSnapVaultInfo.strip().lower() in ['yes', 'y', 'true', 1]:
                # doesn't support snap vault info
                if getNetworkShareInfo.strip().lower() in ['yes', 'y', 'true', 1]:
                    OSHVResult.addAll(netapp.getNetworkShares(vserverOshDict, volumeOshDict))
                    OSHVResult.addAll(netapp.getCifsSessions(vserverOshDict))
            ...
            if hostDetails and len(hostDetails) == 3:
                ...
                for vfilerOSH in vfilerOSHList:
                    ...
                    try:
                        wsConnectionVfiler = netapp_webservice_utils.connectVfiler(wsConnection,vfilerOSH.getAttributeValue('name'))
                        if wsConnectionVfiler:
                            OSHVResult.addAll(getvFilerLUNs(Framework, wsConnectionVfiler, vfilerOSH, volumeOshDict, lunOshDict))
                            OSHVResult.addAll(getvFilerQtreeDetails(Framework, wsConnectionVfiler, vfilerOSH, hostOSH))
                    except:
                        logger.debug("Failed to connect to vFiler: " + vfilerOSH.getAttributeValue('name'))
            else:
...
class NetAppFactory:
    def createNetApp(self, framework, ws_connection):
        if framework.getDestinationAttribute("product_name") == 'NetApp Cluster':
            return NetAppCluster(framework, ws_connection)
        else:
            return None
...
class NetApp:
    def __init__(self, framework, ws_connection):
        ...
        self._module = netapp_cluster_discoverer

    def getHostDetails(self, nodeOshDict, vserverOshDict):
        raise NotImplementedError()


    def getAggrs(self, hostOshDict, aggrOshDict):
        try:
            vector = ObjectStateHolderVector()
            reporter = netapp_topology.VolumeReporter()
            aggr_discoverer = self._module.AggrDiscoverer(self._framework, self._ws_connection)
            aggrs = aggr_discoverer.discover()
            for aggr in aggrs:
                host_osh = hostOshDict.get(aggr.host_name, None)
                if host_osh:
                    aggr_osh = reporter.report(aggr, host_osh)
                    aggrOshDict[aggr.name] = aggr_osh
                    vector.add(aggr_osh)
            return vector
        except:
            excInfo = logger.prepareJythonStackTrace('')
            logger.warn('[' + SCRIPT_NAME + ':getAggrs] Exception: <%s>' % excInfo)
            pass
        return None


    def getVolumes(self, hostOshDict,  volumeOshDict, aggrOshDict=None):
        try:
            vector = ObjectStateHolderVector()
            reporter = netapp_topology.VolumeReporter()
            volume_discoverer = self._module.VolumeDiscoverer(self._framework, self._ws_connection)
            volumes = volume_discoverer.discover()
            for volume in volumes:
                if hostOshDict and hostOshDict.get(volume.host_name, None):
                    volume_osh = reporter.report(volume, hostOshDict.get(volume.host_name))
                    volumeOshDict[volume.name] = volume_osh
                    vector.add(volume_osh)
                    if aggrOshDict and aggrOshDict.get(volume.aggr_name, None):
                        vector.add(modeling.createLinkOSH('dependency', volume_osh, aggrOshDict.get(volume.aggr_name)))
            return vector
        except:
            excInfo = logger.prepareJythonStackTrace('')
            logger.warn('[' + SCRIPT_NAME + ':getVolumes] Exception: <%s>' % excInfo)
    ...
    def getDisks(self, hostOshDict, aggrOshDict=None):
        try:
            vector = ObjectStateHolderVector()
            reporter = netapp_topology.DiskReporter()
            disk_discoverer = self._module.DiskDiscoverer(self._framework, self._ws_connection)
            disks = disk_discoverer.discover()
            for disk in disks:
                host_osh = hostOshDict.get(disk.node_name, None)
                if host_osh:
                    disk_osh = reporter.report(disk, host_osh)
                    vector.add(disk_osh)
                    if disk.aggregate_name and aggrOshDict:
                        aggr_osh = aggrOshDict.get(disk.aggregate_name, None)
                        if aggr_osh:
                            vector.add(modeling.createLinkOSH('dependency', aggr_osh, disk_osh))
            return vector
        except:
            excInfo = logger.prepareJythonStackTrace('')
            logger.warn('[' + SCRIPT_NAME + ':getDisks] Exception: <%s>' % excInfo)
    ...
    def getLUNs(self, hostOshDict, volumeOshDict, lunOshDict):
        try:
            vector = ObjectStateHolderVector()
            reporter = netapp_topology.VolumeReporter()
            lun_discoverer = self._module.LUNDiscoverer(self._framework, self._ws_connection)
            luns = lun_discoverer.discover()
            for lun in luns:
                host_osh = hostOshDict.get(lun.host_name, None)
                if host_osh:
                    lun_osh = reporter.report(lun, host_osh)
                    lunOshDict[lun.path] = lun_osh
                    vector.add(lun_osh)
                    if lun.volume_name and volumeOshDict:
                        volume_osh = volumeOshDict.get(lun.volume_name, None)
                        if volume_osh and lun_osh:
                            vector.add(modeling.createLinkOSH('dependency', lun_osh, volume_osh))
            return vector
        except:
            excInfo = logger.prepareJythonStackTrace('')
            logger.warn('[' + SCRIPT_NAME + ':getLUNs] Exception: <%s>' % excInfo)
    ...
    def getISCSIAdapter(self, vserverOshDict, scsiOshDict):
        try:
            vector = ObjectStateHolderVector()
            reporter = netapp_topology.ISCSIAdapterReporter()
            iscsi_discoverer = self._module.ISCSIAdapterDiscoverer(self._framework, self._ws_connection)
            iscsi_list = iscsi_discoverer.discover()
            for iscsi in iscsi_list:
                if iscsi.host_name:
                    vserver_osh = vserverOshDict.get(iscsi.host_name, None)
                    if vserver_osh:
                        iscsi_osh = reporter.report(iscsi, vserver_osh)
                        scsiOshDict[iscsi.name] = iscsi_osh
                        vector.add(iscsi_osh)
            return vector
        except:
            excInfo = logger.prepareJythonStackTrace('')
            logger.warn('[' + SCRIPT_NAME + ':getISCSIAdapter] Exception: <%s>' % excInfo)
    ...
    def getISCSIInitiators(self, scsiOshDict, lunOshDict):
        try:
            vector = ObjectStateHolderVector()
            reporter = netapp_topology.ISCSIAdapterReporter()
            initiator_discoverer = self._module.ISCSIInitiatorDiscoverer(self._framework, self._ws_connection)
            initiators = initiator_discoverer.discover()
            for initiator in initiators:
                vector.addAll(reporter.reportRemoteISCSIAdapter(initiator, scsiOshDict, lunOshDict))
            return vector
        except:
            excInfo = logger.prepareJythonStackTrace('')
            logger.warn('[' + SCRIPT_NAME + ':getISCSIInitiators] Exception: <%s>' % excInfo)
    ...
    def getFCAdapter(self, hostOshDict, hbaOshDict):
        try:
            vector = ObjectStateHolderVector()
            hba_reporter = netapp_topology.FCAdapterReporter()
            hba_discoverer = self._module.FCAdapterDiscoverer(self._framework, self._ws_connection)
            hbas = hba_discoverer.discover()
            for wwn, host_name in hbas:
                host_osh = hostOshDict.get(host_name, None)
                if host_osh:
                    vector.add(hba_reporter.report(wwn, host_osh, hbaOshDict))
            return vector
        except:
            excInfo = logger.prepareJythonStackTrace('')
            logger.warn('[' + SCRIPT_NAME + ':getFCAdapter] Exception: <%s>' % excInfo)
            pass
        return None

    def getFCPInterface(self, hbaOshDict, fcpOshDict):
        try:
            vector = ObjectStateHolderVector()
            fcp_reporter = netapp_topology.FCPortReporter()
            fcp_discoverer = self._module.FCPortDiscoverer(self._framework, self._ws_connection)
            fcports = fcp_discoverer.discover()
            for fcport in fcports:
                fcport_osh, fcport_vector = fcp_reporter.report(fcport, hbaOshDict)
                fcpOshDict[fcport.vserver_name] = fcport_osh
                vector.addAll(fcport_vector)
            return vector
        except:
            excInfo = logger.prepareJythonStackTrace('')
            logger.warn('[' + SCRIPT_NAME + ':getFCPInterface] Exception: <%s>' % excInfo)
            pass
        return None

    def getFCPInitiators(self, fcpOshDict):
        try:
            vector = ObjectStateHolderVector()
            reporter = netapp_topology.FCPortReporter()
            initiator_discoverer = self._module.FCPInitiatorDiscoverer(self._framework, self._ws_connection)
            initiators = initiator_discoverer.discover()
            for fcport in initiators:
                vector.addAll(reporter.reportRemoteFCPort(fcport, fcpOshDict))
            return vector

        except:
            excInfo = logger.prepareJythonStackTrace('')
            logger.warn('[' + SCRIPT_NAME + ':getFCPInitiators] Exception: <%s>' % excInfo)
            pass
        return None



    def getVolumeSnapshots(self, volumeOshDict, snapshotOshDict):
        try:
            vector = ObjectStateHolderVector()
            reporter = netapp_topology.SnapshotReporter()
            for volume_osh in volumeOshDict.values():
                volume_name = volume_osh.getAttribute('name').getStringValue()
                volume_state = volume_osh.getAttribute('logicalvolume_status').getStringValue()
                if volume_state.strip().lower() != 'online':
                    netapp_webservice_utils.debugPrint(3, '[' + SCRIPT_NAME + ':getClusterVolumeSnapshots] Skipping volume <%s> with status <%s>...' % (volume_name, volume_state))
                    continue
                volume_snapshot_discoverer = self._module.SnapshotDiscoverer(self._framework, self._ws_connection)
                conditions = {}
                conditions['volume'] = volume_name
                snapshots = volume_snapshot_discoverer.discover(conditions)
                for snapshot in snapshots:
                    vector.add(reporter.report(snapshot, volume_osh, snapshotOshDict))
            return vector
        except:
            excInfo = logger.prepareJythonStackTrace('')
            logger.warn('[' + SCRIPT_NAME + ':getVolumeSnapshots] Exception: <%s>' % excInfo)
            pass
        return None


    def getNetworkShares(self, hostOshDict, volumeOshDict):
        try:
            vector = ObjectStateHolderVector()
            fsOshDict = {}
            file_system_reporter = netapp_topology.FileSystemReporter()
            cifs_server_discoverer = self._module.CifsServerDiscoverer(self._framework, self._ws_connection)
            cifs_servers = cifs_server_discoverer.discover()
            for name, host_name in cifs_servers:
                host_osh = hostOshDict.get(host_name, None)
                if host_osh:
                    fils_system_osh = file_system_reporter.report(name, host_osh)
                    fsOshDict[name] = fils_system_osh
                    vector.add(fils_system_osh)

            network_share_reporter = netapp_topology.NetworkShareReporter()
            # report CIFS
            cifs_share_discoverer = self._module.CifsShareDiscoverer(self._framework, self._ws_connection)
            cifs_shares = cifs_share_discoverer.discover()
            for network_share in cifs_shares:
                host_osh = hostOshDict.get(network_share.host_name, None)
                if host_osh:
                    vector.addAll(network_share_reporter.report(network_share, host_osh, fsOshDict))

            #report NFS from Vserver
            for vserverOsh in hostOshDict.values():
                vserver_name = vserverOsh.getAttributeValue('data_name')
                logger.debug('vserver name:%s'%vserver_name)
                nfs_shares = []
                if vserver_name:
                    try:
                        wsConnectionVserver = netapp_webservice_utils.connectVserver(self._ws_connection,
                                                                            vserver_name)

                        nfs_share_discoverer = self._module.NfsShareDiscoverer(self._framework, wsConnectionVserver, vserver_name)
                        nfs_shares.extend(nfs_share_discoverer.discover())
                    except:
                        logger.debug('Did not get NFS info from vServer: %s'%vserver_name)
                for network_share in nfs_shares:
                    host_osh = hostOshDict.get(network_share.host_name, None)
                    if host_osh:
                        vector.addAll(network_share_reporter.report(network_share, host_osh, fsOshDict))
            return vector
    ...
    def getCifsSessions(self, hostOshDict):
        try:
            vector = ObjectStateHolderVector()
            cifs_session_discoverer = self._module.CifsSessionDiscoverer(self._framework, self._ws_connection)
            cifs_sessions = cifs_session_discoverer.discover()
            for remote_ip, vserver_name in cifs_sessions:
                remote_host_osh = modeling.createHostOSH(remote_ip)
                vector.add(remote_host_osh)
                if hostOshDict and hostOshDict.get(vserver_name, None):
                    dependency_link_osh = modeling.createLinkOSH('dependency', remote_host_osh, hostOshDict.get(vserver_name))
                    dependency_link_osh.setAttribute('dependency_name', 'CIFS')
                    dependency_link_osh.setAttribute('dependency_source', 'CIFS')
                    vector.add(dependency_link_osh)
            return vector
        except:
            excInfo = logger.prepareJythonStackTrace('')
            logger.warn('[' + SCRIPT_NAME + ':getCifsSessions] Exception: <%s>' % excInfo)
...
class NetAppCluster(NetApp):
    def __init__(self, framework, ws_connection):
        NetApp.__init__(self, framework, ws_connection)
        self._module = netapp_cluster_discoverer


    def getHostDetails(self, nodeOshDict, vserverOshDict):
        vector = ObjectStateHolderVector()
        cluster_osh, cluster_vector = self._getCluster(nodeOshDict)
        vector.addAll(cluster_vector)
        vector.addAll(self._getVservers(cluster_osh, vserverOshDict))
        #file_options =  self._framework.getParameter('filerOptions')
        # if file_options.lower() == "true":
        #     self.getFileOptions(vserverOshDict)
        vector.addAll(self._getInterfaces(nodeOshDict, vserverOshDict))
        return vector


    def getFileOptions(self, hostOshDict):
        try:
            options_result = {}
            if hostOshDict:
                file_options =  self._framework.getParameter('filerOptions')
                option_request_List = []
                if file_options.find(',') > 0:
                    option_request_List = file_options.split(',')
                else:
                    option_request_List.append(file_options)

                option_discoverer = netapp_cluster_discoverer.OptionDiscoverer(self._framework, self._ws_connection)
                for option_request in option_request_List:
                    conditions = {}
                    conditions['name'] = option_request
                    options = option_discoverer.discover(conditions)
                    for name, value, host in options:
                        option = name.strip() + ":" + value.strip()
                        if options_result.get(host, None):
                            options_result.get(host).append(option)
                        else:
                            options_list = []
                            options_list.append(option)
                            options_result[host] = options_list

            for host in options_result.keys():
                if options_result.get(host, None):
                    host_osh = hostOshDict.get(host, None)
                    if host_osh:
                        host_osh.setListAttribute('options', options_result.get(host, None))
        except:
            excInfo = logger.prepareJythonStackTrace('')
            logger.warn('[' + SCRIPT_NAME + ':getFileOptions] Exception: <%s>' % excInfo)
            pass
        return None


    def _getCluster(self, nodeOshDict):
        try:
            cluster_discoverer = netapp_cluster_discoverer.NetAppClusterDiscoverer(self._framework, self._ws_connection)
            cluster_discoverer.chunk_size = None
            cluster_name = cluster_discoverer.discover()

            node_discoverer = netapp_cluster_discoverer.NetAppNodeDiscoverer(self._framework, self._ws_connection)
            nodes = node_discoverer.discover()

            reporter = netapp_topology.NetAppClusterReporter()
            return  reporter.report(cluster_name, nodes, nodeOshDict)
        except:
            excInfo = logger.prepareJythonStackTrace('')
            logger.warn('[' + SCRIPT_NAME + ':_getCluster] Exception: <%s>' % excInfo)
            pass
        return None


    def _getVservers(self, cluster_osh, vserverOshDict):
        try:
            vector = ObjectStateHolderVector()
            reporter = netapp_topology.VServerReporter()
            vserver_discoverer = netapp_cluster_discoverer.VServerDiscoverer(self._framework, self._ws_connection)
            vservers = vserver_discoverer.discover()
            for vserver in vservers:
                vector.addAll(reporter.report(vserver, cluster_osh, vserverOshDict))
            return vector
        except:
            excInfo = logger.prepareJythonStackTrace('')
            logger.warn('[' + SCRIPT_NAME + ':_getVservers] Exception: <%s>' % excInfo)
            pass
        return None


    def _getInterfaces(self, nodeOshDict, vserverOshDict):
        try:
            vector = ObjectStateHolderVector()
            interface_reporter = netapp_topology.InterfaceReporter()
            interface_discoverer = netapp_cluster_discoverer.InterfaceDiscoverer(self._framework, self._ws_connection)
            interfaces = interface_discoverer.discover()
            for interface in  interfaces:
                host_osh = None
                if interface.role == "node_mgmt" and nodeOshDict and nodeOshDict.get(interface.host_name, None):
                    host_osh = nodeOshDict.get(interface.host_name)
                elif vserverOshDict and vserverOshDict.get(interface.host_name, None):
                    host_osh = vserverOshDict.get(interface.host_name)
                if host_osh:
                    vector.addAll(interface_reporter.report(interface, host_osh))
            return vector
        except:
            excInfo = logger.prepareJythonStackTrace('')
            logger.warn('[' + SCRIPT_NAME + ':_getInterfaces] Exception: <%s>' % excInfo)
            pass
        return None


class NetApp7Mode(NetApp):
    def __init__(self, framework, ws_connection):
        NetApp.__init__(self, framework, ws_connection)

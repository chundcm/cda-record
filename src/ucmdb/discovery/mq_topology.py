...
qTypeMap = {'QALIAS': 'Alias Queue', 'QLOCAL': 'Local Queue', 'QREMOTE': 'Remote Queue', 'XMITQ': 'Transmission Queue',
            'SYSTEM': 'System Queue', 'OTHER': 'Other', 'QMODEL': 'Other Queue'}
qOshTypeMap = {'QALIAS': 'mqaliasqueue', 'QLOCAL': 'mqlocalqueue', 'QREMOTE': 'mqremotequeue',
               'XMITQ': 'mqtransmitqueue', 'SYSTEM': 'mqqueue', 'OTHER': 'mqqueue', 'QMODEL': 'mqqueue'}
receiverChlTypeMap = {'CLUSRCVR': 'Cluster Receiver Channel', 'RCVR': 'Receiver Channel',
                      'CLNTCONN': 'Client Connection Channel', 'RQSTR': 'Requestor Channel'}
senderChlTypeMap = {'CLUSSDR': 'Cluster Sender Channel', 'SDR': 'Sender Channel',
                    'SVRCONN': 'Server Connection Channel', 'SVR': 'Server Channel'}
...
def fixIP(ip, localIp):
    try:
        ...
        if ip == None or ip == '' or len(ip) < 1 or ip == '127.0.0.1' or ip == '0.0.0.0' or ip == '*' or re.search('::',
                                                                                                                   ip):
            return localIp
...
def isValidString(theString):
    try:
        ...
        if theString == None or theString == '' or len(theString) < 1:
            ...
            return 0
        elif re.search('Syntax error detected', theString) or re.search('could not be processed',
                                                                        theString) or re.search('not found', theString):
            return 0
        else:
    ...
    try:
        if isValidString(theParameter) and isValidString(theBuffer):
            ...
            if match.find() == 1:
                ...
                debugPrint(4, '[' + SCRIPT_NAME + ':getBufferParameter] Got value <%s> for parameter <%s>' % (
                returnString, theParameter))
...
def getHostFromConnName(shell, connName):
    try:
        ...
        debugPrint(4, '[' + SCRIPT_NAME + ':getHostFromConnName] Got hostname/IP <%s> from connection name <%s>' % (
        ip, connName))
        return ip
    except:
        excInfo = logger.prepareJythonStackTrace('')
...
def getPortFromConnName(connName):
    try:
        ...
        debugPrint(4, '[' + SCRIPT_NAME + ':getPortFromConnName] Got remote port <%s> from connection name <%s>' % (
        port, connName))
        return port
    except:
        excInfo = logger.prepareJythonStackTrace('')
...
def getMqPortAndIp(mqOSH):
    try:
        ...
        if mqListenerPort and mqListenerIP:
            ipServiceEndpoint = modeling.createServiceAddressOsh(hostOSH, mqListenerIP, mqListenerPort, 1,
                                                                 'ibmmqseries')
            # returnOSHV.add(modeling.createServiceAddressOsh(hostOSH, mqListenerIP, mqListenerPort, 1, 'ibmmqseries'))
            debugPrint(4, '[' + SCRIPT_NAME + ':getMqPortAndIp] Got MQ listener port <%s> and IP <%s>' % (
            mqListenerPort, mqListenerIP))
        ...
        for mqscProgram in mqscProgramList:
            try:
                ...
                if not isValidString(commandOutput):
                    commandOutput = shell.execCmd(mqscProgram + qManagerName + ' ' + mqCommand, MQ_CMD_TIMEOUT,
                                                  Boolean.TRUE)
                if isValidString(commandOutput):
                    if string.find(commandOutput.strip().lower(), 'not available') != -1 or string.find(
                            commandOutput.strip().lower(), 'could not be processed') != -1:
                        debugPrint(4,
                                   '[' + SCRIPT_NAME + ':runMqscCommand] Q Manager is not running or MQSC command returned no results')
                        return
                    elif string.find(commandOutput.strip().lower(), 'not recognized') != -1 or string.find(
                            commandOutput.strip().lower(), 'not found') != -1 or string.find(
                            commandOutput.strip().lower(), 'permission denied') != -1 or string.find(
                            commandOutput.strip().lower(), 'unknown command') != -1:
                        debugPrint(4,
                                   '[' + SCRIPT_NAME + ':runMqscCommand] Command <%s> resulted in an error! Will try the next one...: <%s>' % (
                                   mqscProgram, commandOutput))
                        continue
                    else:
                        debugPrint(5, '[' + SCRIPT_NAME + ':runMqscCommand] Command <%s> ran successfully!!: <%s>' % (
                        mqscProgram, commandOutput))
                        return commandOutput
                else:
                    debugPrint(3,
                               '[' + SCRIPT_NAME + ':runMqscCommand] Command <%s> returned empty output! Will try the next one...' % mqscProgram)
                    continue
            except:
                excInfo = logger.prepareJythonStackTrace('')
                debugPrint(3,
                           '[' + SCRIPT_NAME + ':runMqscCommand] Command <%s> threw an exception! Will try the next one...: <%s>' % (
                           mqscProgram, excInfo))
                continue
    except:
        excInfo = logger.prepareJythonStackTrace('')
...
## Get MQ Version
##############################################
def parseMqVersion(versionString):
    versionNumber = None
    if versionString:
        numList = re.split('..', versionString)
        if len(numList) > 2:
            versionNumber = numList[0] + '.' + numList[1]
        else:
...
def getMqVersion(localShell):
    try:
        ...
        for mqVerPath in mqVerPaths:
            mqver = localShell.execAlternateCmds(SUDO + mqVerPath + 'dspmqver', SUDO + mqVerPath + 'mqver',
                                                 SUDO + 'dspmqver', SUDO + 'mqver', 'dspmqver', 'mqver',
                                                 mqVerPath + 'dspmqver', mqVerPath + 'mqver')
            if isValidString(mqver):
                debugPrint(4, '[' + SCRIPT_NAME + ':getMqVersion] Got mqver/dspmqver output: <%s>' % mqver)
                returnString = ''
                match = Pattern('Name:\s+(.*)\n.*Version:\s+(.*)\n.*[l|L]evel:\s+(.*)\n.*BuildType:\s+([^\n]+)',
                                REFlags.DOTALL).matcher(mqver)
                match.find()
...
def getIpAndPortFromMqManager(shell, qManagerName):
    ...
    if isValidString(qMgrPortDetails):
        ...
        try:
            ...
            if ip_addr.isValidIpAddress(ip_str.strip()):
...
def getQManagers(shell, ipAddress, localFramework, manager_to_endpoint_dict):
    try:
        ...
        if isValidString(MQVER_PATH) and MQVER_PATH.find(';') > 0:
            mqVerPaths = string.split(MQVER_PATH, ';')
        elif not isValidString(MQVER_PATH):
            mqVerPaths = [MQVER_PATH.strip()]
        else:
            mqVerPaths = [' ']

        for mqVerPath in mqVerPaths:
            temp = shell.execAlternateCmds(SUDO + mqVerPath + 'dspmq', mqVerPath + 'dspmq')
            if temp is not None:
                qManagersList.append(temp)

        if qManagersList is None:
            environment = shell_interpreter.Factory().create(shell).getEnvironment()
            environment.setVariable('LD_ASSUME_KERNEL', '2.4.19')
            qManagersList = [shell.execAlternateCmds(SUDO + 'dspmq', 'dspmq')]

        for qManager in qManagersList:
            if qManager.strip().lower().find('is not recognized as an internal or external command') > 0:
                localFramework.reportError(
                    'Unable to execute command "dspmq" . Please specify path using the "mqver_path" parameter')
                continue
            else:
                match = Pattern('QMNAME\((.*?)\).*?STATUS\(([\w\s]+)', REFlags.DOTALL).matcher(qManager)
                while match.find() == 1:
                    ## Get MQ Version
                    mqVersionString = getMqVersion(shell)
                    mqVersionNumber = parseMqVersion(mqVersionString[:mqVersionString.find(' ')]) or ''
                    debugPrint(2,
                               '[' + SCRIPT_NAME + ':getQManagers] Got WebSphere MQ version <%s> and version number <%s>' % (
                               mqVersionString.strip(), mqVersionNumber))
                    ...
                    if isValidString(qManagerName):
                        debugPrint(2, '[' + SCRIPT_NAME + ':getQManagers] Got Q manager <%s> with status <%s>' % (
                        qManagerName, qManagerStatus))
                        ...
                        if qManagerStatus.lower() != 'running':
                            logger.warn(
                                "[" + SCRIPT_NAME + ":getQManagers] Skipping MqManager %s since it's not in running state: got state %s" % (
                                qManagerName, qManagerStatus))
                            continue

                        qMgrDetails = runMqscCommand(shell, 'DISPLAY QMGR DESCR DEADQ DEFXMITQ REPOS CCSID',
                                                     qManagerName)
                        if isValidString(qMgrDetails):
                            ...
                            ipWithPort = getIpAndPortFromMqManager(shell, qManagerName)
                            if ipWithPort:
                                ip_address, listen_port = ipWithPort
                            else:
                                proc_ip, proc_port = manager_to_endpoint_dict.get(qManagerName, [None, None])
                                ip_address = proc_ip or ipAddress
                                listen_port = proc_port
                            ...
                            debugPrint(5,
                                       '[' + SCRIPT_NAME + ':getQManagers] Got Q manager: <%s>' % qManagerOSH.toXmlString())
        ...
        if isValidString(qDetails):
            ...
            for queue in queues:
                ...
                if not isValidString(qName):
                    debugPrint(4,
                               '[' + SCRIPT_NAME + ':getQueues] Invalid Q name on qManager <%s>! Skipping...' % qManagerName)
                    continue
                if qName.strip() == '*':
                    debugPrint(4, '[' + SCRIPT_NAME + ':getQueues] Skipping Q <%s> on qManager <%s>' % (
                    qName, qManagerName))
                    continue
                ...
                if qType.strip() == 'QLOCAL' and isValidString(DISCOVER_DYNAMIC_QUEUES) and isValidString(
                        qDefinitionType) and DISCOVER_DYNAMIC_QUEUES.strip().lower() not in ['true', 'yes', 'y', '1']:
                    if qDefinitionType.strip() != 'PREDEFINED':
                        debugPrint(4,
                                   '[' + SCRIPT_NAME + ':getQueues] Non-static Local Q <%s> found on qManager <%s>! Skipping...' % (
                                   qName, qManagerName))
                        continue
                ...
                if qOSH == None:
                    debugPrint(4,
                               '[' + SCRIPT_NAME + ':getQueues] Error building an OSH for Q <%s> on qManager <%s>' % (
                               qName, qManagerName))
                    continue
                ...
                if isValidString(qNamelist):
                    debugPrint(3, '[' + SCRIPT_NAME + ':getQueues] Got MQ Cluster NameList <%s> on Q <%s>' % (
                    qNamelist, qName))
                ...
                if qType == 'QALIAS':
                    ...
                    if isValidString(targetQ):
                        ...
                        if (targetQ + qManagerName) in qOshDict.keys():
                            debugPrint(3,
                                       '[' + SCRIPT_NAME + ':getQueues] Got Q <%s> already in dictionary for alias Q <%s>' % (
                                       targetQ, qName))
                            returnOSHV.add(
                                modeling.createLinkOSH('realization', qOSH, qOshDict[targetQ + qManagerName]))
                        else:
                            debugPrint(3, '[' + SCRIPT_NAME + ':getQueues] Got new target Q <%s> for alias Q <%s>' % (
                            targetQ, qName))
                ...
                ## If this is a REMOTE Q, link it with its TRANSMIT Q
                    ...
                    if isValidString(xmitQ):
                        debugPrint(4,
                                   '[' + SCRIPT_NAME + ':getQueues] Got xmit Q <%s> for remote Q <%s>' % (xmitQ, qName))
                        if (xmitQ + qManagerName) in qOshDict.keys():
                            debugPrint(3,
                                       '[' + SCRIPT_NAME + ':getQueues] Got xmit Q <%s> already in dictionary for remote Q <%s>' % (
                                       xmitQ, qName))
                            returnOSHV.add(modeling.createLinkOSH('use', qOSH, qOshDict[xmitQ + qManagerName]))
                        else:
                            debugPrint(3, '[' + SCRIPT_NAME + ':getQueues] Got new xmit Q <%s> for remote Q <%s>' % (
                            xmitQ, qName))
                    ...
                    if isValidString(xmitQ) and isValidString(remoteQMgrName) and isValidString(
                            DISCOVER_REMOTE_HOSTS) and DISCOVER_REMOTE_HOSTS.strip().lower() in ['true', 'yes', 'y',
                                                                                                 '1']:
                        debugPrint(3,
                                   '[' + SCRIPT_NAME + ':getQueues] Got new Q <%s> for remote Q <%s> with Q manager <%s>' % (
                                   remoteQName, qName, remoteQMgrName))
                        ...
                        ipserverOSH = None
                        remoteMqOSH = None
                        remoteHostDetails = runMqscCommand(shell,
                                                           'DISPLAY CHANNEL(*) WHERE(xmitq EQ ' + xmitQ + ') TYPE(SDR) CONNAME',
                                                           remoteQMgrName)
                        debugPrint(4,
                                   '[' + SCRIPT_NAME + ':getQueues] Got remote host details <%s> for remote Q <%s>' % (
                                   remoteHostDetails, qName))
                        ...
                        if isValidString(channelConnName):
                            ...
                            if remoteHost is not None:
                                ...
                                debugPrint(3,
                                           '[' + SCRIPT_NAME + ':getQueues] Creating remote host <%s> with port <%s> for remote Q <%s>' % (
                                           remoteHost, remotePort, qName))
                                remoteHostOSH = modeling.createHostOSH(remoteHost)
                                ipserverOSH = modeling.createServiceAddressOsh(remoteHostOSH, remoteHost,
                                                                               int(remotePort),
                        ...
                        if remoteHostOSH:
                            debugPrint(3,
                                       '[' + SCRIPT_NAME + ':getQueues] Creating remote Q manager <%s> for remote Q <%s>' % (
                                       remoteQMgrName, qName))
                            ## Build Websphere MQ OSH
                        ...
                        if remoteQManagerOSH and isValidString(remoteQName):
                            debugPrint(3, '[' + SCRIPT_NAME + ':getQueues] Creating Q <%s> for remote Q <%s>' % (
                            remoteQName, qName))
                        ...
                        ## This is an alias Q manager because the remote Q name is blank
                            ...
                            debugPrint(3,
                                       '[' + SCRIPT_NAME + ':getQueues] Adding realization link between Alias Q Manager <%s> and Q Manager <%s>' % (
                                       qName, remoteQMgrName))
...
def getChannels(shell, qManagerOSH):
    try:
        ...
        returnOSHV = ObjectStateHolderVector()
        qManagerName = qManagerOSH.getAttribute('data_name').getStringValue()
        channelDetails = runMqscCommand(shell, 'DISPLAY CHANNEL(*) CHLTYPE TRPTYPE DESCR CLUSTER CLUSNL CONNAME XMITQ',
                                        qManagerName)
        ...
        if isValidString(channelDetails):
            ...
            for channel in channels:
                ...
                if not isValidString(channelName) or not isValidString(channelType):
                    debugPrint(4,
                               '[' + SCRIPT_NAME + ':getChannels] Invalid channel name or type on Q manager <%s>! Skipping...' % qManagerName)
                    continue
                if channelName.strip() == '*':
                    debugPrint(4, '[' + SCRIPT_NAME + ':getChannels] Skipping channel <%s> on qManager <%s>' % (
                    channelName, qManagerName))
                    continue
                debugPrint(2, '[' + SCRIPT_NAME + ':getChannels] Got channel <%s> of type <%s>' % (
                ...
                if not isValidString(xmitQ):
                    ...
                    xmitQ = qManagerOSH.getAttribute('mqqueuemanager_defaultxmitqname').getStringValue()
                if isValidString(xmitQ):
                    debugPrint(4, '[' + SCRIPT_NAME + ':getChannels] Got xmit Q <%s> for channel <%s>' % (
                    xmitQ, channelName))
                    if (xmitQ + qManagerName) in qOshDict.keys():
                        debugPrint(3,
                                   '[' + SCRIPT_NAME + ':getChannels] Got xmit Q <%s> already in dictionary for remote Q <%s>' % (
                                   xmitQ, channelName))
                        returnOSHV.add(modeling.createLinkOSH('use', channelOSH, qOshDict[xmitQ + qManagerName]))
                    else:
                        debugPrint(3, '[' + SCRIPT_NAME + ':getChannels] Got new xmit Q <%s> for remote Q <%s>' % (
                        xmitQ, channelName))
                        xmitQOSH = ObjectStateHolder(qOshTypeMap['XMITQ'])
                        xmitQOSH.setStringAttribute('data_name', xmitQ.strip())
                        xmitQOSH.setStringAttribute('queue_type', qTypeMap['XMITQ'])
                ...
                if isValidString(channelConnName):
                    ...
                    if host is not None:
                        ...
                        debugPrint(3,
                                   '[' + SCRIPT_NAME + ':getChannels] Got xmit channel remote host <%s> and port <%s>' % (
                                   host, port))
                ...
                if isValidString(channelCluster):
                    debugPrint(3, '[' + SCRIPT_NAME + ':getChannels] Got MQ Cluster <%s> on channel <%s>' % (
                    channelCluster, channelName))
                    clusterOSH = ObjectStateHolder('mqcluster')
                ...
                if isValidString(channelNamelist):
                    debugPrint(3, '[' + SCRIPT_NAME + ':getQueues] Got MQ Cluster NameList <%s> on channel <%s>' % (
        ...
        if isValidString(clusterDetails):
            ...
            for cluster in clusters:
                ...
                if not isValidString(clusterName):
                    debugPrint(4,
                               '[' + SCRIPT_NAME + ':getClusters] Invalid cluster name on Q manager <%s>! Skipping...' % qManagerName)
                    continue
                if clusterName.strip() == '*':
                    debugPrint(4, '[' + SCRIPT_NAME + ':getClusters] Skipping cluster <%s> on Q manager <%s>' % (
                    clusterName, qManagerName))
                    continue
                debugPrint(2, '[' + SCRIPT_NAME + ':getClusters] Got cluster <%s> on Q manager <%s>' % (
                clusterName, qManagerName))
                clusterOSH = ObjectStateHolder('mqcluster')
                ...
                if isValidString(clusterConnName):
                    ...
                    if remoteHost is not None:
                        ...
                        debugPrint(4,
                                   '[' + SCRIPT_NAME + ':getClusters] Got host <%s> and port <%s> for cluster <%s>' % (
                                   remoteHost, remotePort, clusterName))
                        if clusterQMgr.lower().strip() != qManagerName.lower().strip() and isValidString(
                                DISCOVER_REMOTE_HOSTS) and DISCOVER_REMOTE_HOSTS.strip().lower() in ['true', 'yes', 'y',
                                                                                                     '1']:
                            debugPrint(3,
                                       '[' + SCRIPT_NAME + ':getClusters] Got remote host <%s> and port <%s> for cluster <%s>' % (
                                       remoteHost, remotePort, clusterName))
                            remoteHostOSH = modeling.createHostOSH(remoteHost)
                            ipserverOSH = modeling.createServiceAddressOsh(remoteHostOSH, remoteHost, int(remotePort),
                                                                           modeling.SERVICEADDRESS_TYPE_TCP,
                                                                           'ibmmqseries')
                            returnOSHV.add(ipserverOSH)
                            returnOSHV.add(remoteHostOSH)
                ...
                if remoteHostOSH and isValidString(clusterQMgr):
                    debugPrint(3, '[' + SCRIPT_NAME + ':getClusters] Got remote Q manager <%s> for cluster <%s>' % (
                    clusterQMgr, clusterName))
                    ## Build Websphere MQ OSH
                    remoteMqOSH = ObjectStateHolder('webspheremq')
                    remoteMqOSH.setAttribute('data_name', 'IBM WebSphere MQ')
                    remoteMqOSH.setAttribute('application_ip', remoteHost)
                ...
                ## Set port and IP info and add a link if this Q manager is the cluster Q manager
                    ...
                    debugPrint(3,
                               '[' + SCRIPT_NAME + ':getClusters] Got local Q manager <%s> for cluster <%s> with IP <%s> and port <%s>' % (
                               clusterQMgr, clusterName, remoteHost, remotePort))
                ...
                if isValidString(clusterChannel):
                    debugPrint(4, '[' + SCRIPT_NAME + ':getClusters] Got channel <%s> for cluster <%s>' % (
                    clusterChannel, clusterName))
                    if (clusterChannel + qManagerName) in senderChannelOshDict.keys():
                        ...
                        if isValidString(channelType) and channelType == senderChlTypeMap['CLUSSDR']:
                            debugPrint(3,
                                       '[' + SCRIPT_NAME + ':getClusters] Got cluster sender channel <%s> already in dictionary for cluster <%s>' % (
                                       clusterChannel, clusterName))
                            returnOSHV.add(modeling.createLinkOSH('use', clusterOSH, channelOSH))
                        else:
                            debugPrint(3,
                                       '[' + SCRIPT_NAME + ':getClusters] Got channel <%s> of type <%s> already in dictionary for cluster <%s>...Skipping channel creation!' % (
                                       clusterChannel, channelType, clusterName))
        else:
        ...
        if isValidString(namelistDetails):
            ...
            for namelist in namelists:
                ...
                if not isValidString(namelistName):
                    debugPrint(4,
                               '[' + SCRIPT_NAME + ':getNamelists] Invalid namelist name on Q manager <%s>! Skipping...' % qManagerName)
                    continue
                if namelistName.strip() == '*':
                    debugPrint(4, '[' + SCRIPT_NAME + ':getNamelists] Skipping namelist <%s> on Q manager <%s>' % (
                    namelistName, qManagerName))
                    continue
                ...
                debugPrint(2, '[' + SCRIPT_NAME + ':getNamelists] Got namelist <%s> on Q manager <%s>' % (
                namelistName, qManagerName))
                namelistOSH = ObjectStateHolder('mqnamelist')
...
def getMqManagerToMqServerEndpointRelation(shell):
    ...
    processes = discoverer.discoverProcessesByCommandLinePattern(
        'runmqlsr')  # main listener process of the server, one per manager
    result = {}
...
def DiscoveryMain(Framework):
    ...
    if isValidString(use_sudo) and use_sudo.strip().lower() in ['true', 'yes', 'y', '1'] and isValidString(
            sudo_command):
            ...
            for qManagerName in qManagerOshDict.keys():
                ...
                if isValidString(qManagerStatus) and qManagerStatus.lower() == 'running':
                    ...
                    if channelOSHV and channelOSHV.size() > 0:
                        ...
                        debugPrint(4,
                                   '[' + SCRIPT_NAME + ':DiscoveryMain] No CHANNELs found for Q Manager <%s>!' % qManagerName)
                    ################################
                    ## Get clusters
                    ...
                    if clusterOSHV and clusterOSHV.size() > 0:
                        ...
                        debugPrint(4,
                                   '[' + SCRIPT_NAME + ':DiscoveryMain] No CLUSTERs found for Q Manager <%s>!' % qManagerName)
                    ################################
                    ## Get namelists
                    ...
                    if namelistOSHV and namelistOSHV.size() > 0:
                        ...
                        debugPrint(4,
                                   '[' + SCRIPT_NAME + ':DiscoveryMain] No NAMELISTs found for Q Manager <%s>!' % qManagerName)
        else:

...
class WebDispatcherPlugin(BasePlugin):
    ...
    def process(self, context):
        ...
        if instPf:
            ...
            vec.addAll(_reportSAPSystem(servedSystems, processOsh, shell, connIp, serverOsh))
            vec.addAll(_reportHanaInstance(servedSystems, processOsh, shell, connIp, serverOsh))
...
def _reportSystems(servedSystems, processOsh, shell, connectionIp, applicationOsh=None):
    ...
    for servedSystem in servedSystems:
        ...
        endpointVector = map(third, results)
        if endpointVector:
            for v1 in endpointVector:
                vector.addAll(v1)
...
def _reportSAPSystem(servedSystems, processOsh, shell, connectionIp, applicationOsh):
    r'''in order to report sap system with sap web dispatcher, need to report:
    1. message server endpoint
    2. sap abap central service
    3. sap system
    4. sap message server host
    5. link between message server endpoint and host
    6. link between message server endpoint and abap central service
    7. link between sap system and abap central service
    8. sap web dispatcher
    9. link between sap system and web dispatcher
    '''
    resolveEndpoint = _getEndpResolveFn(shell, connectionIp)
    vector = ObjectStateHolderVector()
    linkR = sap.LinkReporter()
    ...
    for servedSystem in servedSystems:
        sources = servedSystem.getMetadataSources()
        #if source is message server, report sap ssytem and abap central service
        msgSources, sources = partition(isMessageServerSource, sources)
        endpoints = keep(sap_webdisp.HasEndpoint.getEndpoint, msgSources)
        abapCentralServiceBuilder = sap.ABAPCentralServiceBuilder()
        abapCentralServiceReporter = sap.CentralComponentReporter(abapCentralServiceBuilder)
        for e in flatten(keep(resolveEndpoint, endpoints)):
            e = netutils.createTcpEndpoint(e.getAddress(), e.getPort())
            endpOsh, hostOsh, eVector = _reportServerEndp(e, processOsh)
            vector.addAll(eVector)
            abapCentralServiceOsh = abapCentralServiceReporter.reportAnonymous(hostOsh)
            vector.add(abapCentralServiceOsh)
            vector.add(linkR.reportUsage(abapCentralServiceOsh, endpOsh))
            # report sap system if available
            sapSystem = servedSystem.system
            if sapSystem:
                sapSystemOsh = sap.Builder().buildSapSystem(sapSystem)
                vector.add(sapSystemOsh)
                vector.add(linkR.reportMembership(sapSystemOsh, abapCentralServiceOsh))
                vector.add(linkR.reportDependency(sapSystemOsh, applicationOsh))
        #if source is not message server and from ICR file, try to resove it as sap abap application server and j2ee application server
        abapApplicationServer = sap.ABAPApplicationServerBuilder()
        abapApplicationReporter = sap.CentralComponentReporter(abapApplicationServer)
        j2EEApplicationServer = sap.J2EEApplicationServerBuilder()
        j2EEApplicationReporter = sap.CentralComponentReporter(j2EEApplicationServer)
        ...
        for e in endpoints:
            ...
            vector.addAll(eVector)
            appServerOsh = None
            if e.getPortType() in (sap.PortTypeEnum.findByName('DIAG'), sap.PortTypeEnum.findByName('RFC')):
                appServerOsh = abapApplicationReporter.reportAnonymous(hostOsh)
            elif e.getPortType() in (sap.PortTypeEnum.findByName('J2EE'), sap.PortTypeEnum.findByName('J2EES')):
                appServerOsh =j2EEApplicationReporter.reportAnonymous(hostOsh)
            if appServerOsh:
                ...
                if sapSystem:
                    sapSystemOsh = sap.Builder().buildSapSystem(sapSystem)
                    vector.add(sapSystemOsh)
                    vector.add(linkR.reportMembership(sapSystemOsh, appServerOsh))
                    vector.add(linkR.reportDependency(sapSystemOsh, applicationOsh))
    return vector


def _reportHanaInstance(servedSystems, processOsh, shell, connectionIp, applicationOsh):
    r'''in order to report sap system with hana db instance, need to report:
    1. hana db instance
    2. hana db host
    3. hana db endpoint
    4. link between hana db instance and hana db host
    5. link between hana db instance and db endpoint
    6. link between hana db instance and web dispatcher
    '''
    resolveEndpoint = _getEndpResolveFn(shell, connectionIp)
    vector = ObjectStateHolderVector()
    linkR = sap.LinkReporter()
    #x) report external server as hana DB instance
    for servedSystem in servedSystems:
        # report endpoints of hana db instance
        endpoints = servedSystem.getExternalServersEndpoints()
        hanaDBInstanceBuilder = sap.HanaDBInstanceBuilder()
        HanaDBInstanceReporter = sap.CentralComponentReporter(hanaDBInstanceBuilder)
        for e in flatten(keep(resolveEndpoint, endpoints)):
            e = netutils.createTcpEndpoint(e.getAddress(), e.getPort())
            endpOsh, hostOsh, eVector = _reportServerEndp(e, processOsh)
            vector.addAll(eVector)
            HanaDBInstanceOsh = HanaDBInstanceReporter.reportAnonymous(hostOsh)
            if HanaDBInstanceOsh:
                vector.add(HanaDBInstanceOsh)
                vector.add(sap.LinkBuilder().buildLink('composition', hostOsh, HanaDBInstanceOsh))
                vector.add(sap.LinkBuilder().buildLink('composition', hostOsh, endpOsh))
                vector.add(linkR.reportUsage(HanaDBInstanceOsh, endpOsh))
                # report hana DB instance link with web dispatcher
                vector.add(linkR.reportDependency(applicationOsh, HanaDBInstanceOsh))
    return vector
...
class TelnetOutputHandler(OutputHandler):
    def handle(self, contents):
        '@types: jarray(char) -> jarray(char)'
        return String(StreamReaderThread.translateBackspace(contents, len(contents))).toCharArray()
...
class ShellFactory:
    ...
    def createShell(self, client, protocolName=None, skip_set_session_locale=None):
        ...
        try:
            ...
            self.isUnsetHist = self.globalSettings.getPropertyBooleanValue('unsetHistFile', False)
            ...
            if protocolName == PowerShell.PROTOCOL_TYPE:
                ...
                if self.isUnsetHist:
                    logger.debug('Unset HISTFILE')
                    self.unsetHist(client)
                shell = UnixShell(client, skip_set_session_locale)
        except:
            errMsg = str(sys.exc_info()[1])
            if errMsg.find('TimeoutException') != -1:

                for cls in (JunOsShell, CiscoIOSShell, NexusShell, F5Shell):
    ...
    def unsetHist(self, shellClient):

        ''' Determine unix shell trying to execute 'unset HISTFILE' command 

        @command: unset HISTFILE 

        @types: BaseClient -> bool 

        @raise Exception: Failed to unset HISTFILE 

        '''

        try:
            shellClient.executeCmd('unset HISTFILE', 8000)
        except:
            errMsg = 'Failed to unset HISTFILE. Exception received: %s' % (sys.exc_info()[1])
            logger.error(errMsg)
            raise Exception(errMsg)
...
class Shell:
    ...
    def createOutputHandler(self, cmd=None):
        ...
        elif self.getClientType() == 'uda' and not isinstance(self, WinShell):
            logger.debug("UDA for linux")
            return SSHOutputHandler()
        else:
    ...
    def checkSizeForCopy(self, localPath, remotePath):
        fs = file_system.createFileSystem(self)
        fo = fs.getFile(remotePath, [file_topology.FileAttrs.SIZE], 1)
        remoteSize = fo.sizeInBytes()
        localSize = os.path.getsize(localPath.strip())
        if localSize != remoteSize:
            logger.debug('The file sizes do not match, local file size: ', localSize, ' remote file size: ', remoteSize)
            # Copy failed, need to remove the corrupted file
            if fs.exists(remotePath):
                logger.debug('The corrupted file ', remotePath, ' will be removed.')
                fs.removeFile(remotePath)
            return False
        return True
...
class JunOsShell(CiscoIOSShell):
    'Basic class for JUNOS shell'
    _JUNOS_VER_MARKER = 'JUNOS'
    _OSTYPE = 'JUNOS'

    @classmethod
    def isJuniper(cls, client):
        ''' Check for JunOs shell.
        @types: Client -> bool
        @command: show version
        '''
    ...
    @staticmethod
    def _is_invalid_command(output):
        return output.find('unknown command') >= 0
...
class WinShell(Shell):
    ...
    def downloadFileFromRemote(self, remoteFileName, remoteFilePath):
        """ Copy file from the remote path, using downloading
        @types: str, str -> None
        """

        tempFileFolder = CollectorsParameters.PROBE_MGR_TEMPDOWNLOAD + CollectorsParameters.FILE_SEPARATOR
    ...
    def copyFileIfNeeded(self, localFile, checkSize=False, share=DEFAULT_WIN_SHARE):
        ...
        if rtCode != 0:
            if self.__client.putFile(localFile, share):
                ...
                if checkSize and not self.checkSizeForCopy(localFile, remoteFile):
                    return None
            else:
                return None
...
class UnixShell(Shell):
    ...
    def setSessionLocale(self):
        ...
        if locale:
            try:
                ...
                for i in locale:
                    if environment.setVariable('LANG', i) and environment.setVariable('LC_ALL', i):
                        break
            except:
            ...
            for sudoCommandPattern in self.__sudoCommandsArray:
                if re.match(sudoCommandPattern.lstrip(), command.strip()):
                    ...
                    if self._checkFileExistance:
                        ...
                        # Since it's possible to run a command like sudo sample_command which should not be tried to check for presence on the FS.
    ...
    def __shouldRunInPrivMode(self, command):
        ...
        if enter_cmd:
            enter_cmd = enter_cmd.strip()
        if exit_cmd:
            exit_cmd = exit_cmd.strip()
        cmd = command.line
        if cmd:
            cmd = cmd.strip()
    ...
    def __getSudoWithCmd(self, sudoPath, originalCommand, preserveSudoContext = 0):
        ...
        if not self.__sudoCommandsArray:
            ...
            if self.__shouldAllCommandsRunAsSuperUser():
                ...
                if m:
                    logger.debug('format command in perl:', originalCommand.replace(r'\"', '"'))
                    originalCommand = originalCommand.replace(r'\"', '"')
            ...
            if m:
                logger.debug('format command in perl:', originalCommand.replace(r'\"', '"'))
                return "%s %s" % (sudoPathWithParams, originalCommand.replace(r'\"', '"'))
            else:
                splitPattern = self.__sudoSplitPattern % re.escape(separator)
                commandElements = re.split(splitPattern, originalCommand)
                for i in range(0, len(commandElements)):

...
def discoverDiskFolderByWmic(shell, OSHVec, hostOSH):
    wmiProvider = wmiutils.getWmiProvider(shell)
    queryBuilder = wmiProvider.getBuilder('Win32_Volume')
    queryBuilder.usePathCommand(1)
    queryBuilder.addWmiObjectProperties('Name', 'DriveType', 'FreeSpace', 'Capacity', 'FileSystem',)
    queryBuilder.addWhereClause('DriveType=3')
    wmicAgent = wmiProvider.getAgent()
    diskItems = []
    try:
        ...
        logger.debugException('Failed getting disks mount point information via wmic')
        return 0

    for diskItem in diskItems:
        # size in MB
        diskSize = diskItem.Capacity and diskItem.Capacity.strip() or None
        if diskSize:
            diskSize = _bytesToMB(diskSize)
        diskFreeSize = diskItem.FreeSpace and diskItem.FreeSpace.strip() or None
        if diskFreeSize:
        ...
        diskName = diskItem.Name and diskItem.Name.strip() or None
        if diskName:
            if re.match(r'\w:\\.+', diskName):
                diskName = re.sub(r'\\$', '', diskName)
                logger.info('Get disk volume:', diskName)
            else:
                logger.info('Filter disk %s because of repeating volume' % diskName)
                continue
        # Win32_Volume don't have ProviderName
        diskProviderName = diskName
...
def discoverDiskMountPoints(wmiProvider, OSHVec, hostOSH, phyVolumeNumberToOshMap={}):
    ...
    for partition in partitionItems:
        ...
        if mountedTo:
            ...
            fsOsh = modeling.createFileSystemOSH(hostOSH, mountedTo, "FixedDisk",size=_bytesToMB(size))
            OSHVec.add(fsOsh)

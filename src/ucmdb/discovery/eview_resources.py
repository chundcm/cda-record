...
# Cp23 - Change to support SMT feature which changes D M=CPU to D M=CORE and command output change QCCR1H112414
...
def ev3_getCpulistOutput(ls):
    ...
    cmdError = output.getValuesFromLineList('s', output.cmdResponseList, 'IEE996I INVALID DISPLAY M=CPU COMMAND ISSUED WITH PROCVIEW CORE')
    if len(cmdError) > 0:
       output = ls.evMvsCmd(_CMD_D_M_CORE)
    if output.isSuccess() and len(output.cmdResponseList) > 0:
        ...
        # if cpuTable is empty then it could be the command output is in the D M=CORE format so try it that way
        if len(cpuTable) == 0:
            headerColumns = ['ID', 'ST', 'ID RANGE', 'VP', 'ISCM', 'CPU THREAD STATUS']
            tableBeginPattern = 'CORE STATUS'
            tableEndPattern = 'CPC ND ='
            firstColumnPaddingChar = ''
            includeRegexPattern = ''
            ignorePatterns = []
            cpuTable = output.getTableValues(output.cmdResponseList, headerColumns, tableBeginPattern, tableEndPattern, firstColumnPaddingChar, includeRegexPattern, ignorePatterns)
        if len(cpuTable) == 0:
           logger.warn("Unable to get individual CPU information")    
...
def DiscoveryMain(Framework):
    ...
    if eview_lib.isNotNull(hostId):
        ...
        lparOsh.setStringAttribute('platform_vendor', PlatformVendors.IBM)

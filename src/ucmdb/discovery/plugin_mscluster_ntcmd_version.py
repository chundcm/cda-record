...
class MSClusterInformationPluginByNTCMD(Plugin):
    ...
    def updateCurrentContainer(self, applicationOsh):
        if applicationOsh.getObjectClass() == 'node':
            # Since MS Cluster can run only on Windows we'll change the container CIT to NT in order to workaround recon. issues
            node_osh = applicationOsh.getAttribute("root_container").getObjectValue()

            logger.debug("applicationOsh: " + node_osh)
            object_id = node_osh.getCmdbId()
            nt_container_osh = modeling.createOshByCmdbId('nt', object_id)
            applicationOsh.setContainer(nt_container_osh)
    ...
    def process(self, context):
        ...
        self.updateCurrentContainer(self.applicationOsh)
    ...
    def parseIpsFromResorceParam(self, resourceProperties, ipPattern):
        ...
        lines = resourceProperties.splitlines()
        if not lines:
            return ips
        # Ignore the first line because it may be matched by the pattern
        for line in lines[1:]:

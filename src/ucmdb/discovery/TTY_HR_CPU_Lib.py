...
def makeCPUOSH(hostId, cid, vendor='', speed='', usage='', data_name='',
               coreNumber=0, descr='', is_virtual = False, logicalCpuCount=None):
    hostOsh = modeling.createOshByCmdbIdString('host', hostId)
    cpuOsh = modeling.createCpuOsh(cid, hostOsh, speed, coreNumber, vendor,
                                   descr, data_name, logicalProcessorCount=logicalCpuCount)
    if is_virtual:
...
class AixCpuDiscoverer:
    ...
    GET_CPU_COUNT_COMMAND = 'lparstat -i | grep "Online Virtual CPUs"'
    ...
    def get_cpus_from_sysplanar(self):
        ...
        cpu_index = 0
        for line in lines:
            m = re.match(r"(\d+).+WAY\s+PROC", line) or re.match(r"(\d*)PROCESSOR PLANAR", line) or re.match(r"SYS BP\s+(\d+)W", line)
            if m:
                coreCount = m.group(1) and int(m.group(1)) or None
                if coreCount:
                    for coreNumber in range(0, coreCount):
                        cpu = _Cpu('CPU%d' % cpu_index)
                        cpu.id = 'CPU%d' % cpu_index
                        cpu.name = 'CPU%d' % cpu_index
                        # For IBM, we treat a core as a physical cpu
                        cpu.coresCount = 1
                        cpus.append(cpu)
    ...
    def get_cpu_count_from_lparstat(self):
        '''
        Discovers CPU count as below:
        lparstat -i | grep "Online Virtual CPUs"
        Output will be like:
        #Online Virtual CPUs          : 2
        @types: -> int
        '''
        r = self.client.execCmd(self.GET_CPU_COUNT_COMMAND)
        if r == None:
            r = ''
        m = re.match('Online Virtual CPUs\s*: (\d+)', r)
        cpu_count = 0
        if (m):
            cpu_count = m.group(1)
        return cpu_count
    ...
    def get_cpus(self):
        ...
        if r:
            ...
            if matches:
                core_count = 1
                cpu_count = self.get_cpu_count_from_lparstat()
                for match in matches:
                    ...
                    cpu.logicalCpuCount = cpu_count
                    cpu_list.append(cpu)
                    cpuNumber = cpuNumber + 1
                    if cpu_count and len(cpu_list) >= cpu_count:
                        break
        return cpu_list
    ...
    def get_frame_cpus(self):
        ...
        vcpu_count = self.get_cpu_count_from_lparstat()
        if cpu_list:
            cpu_vendor = self.get_cpu_vendor()
            ...
            for cpu in cpu_list:
                ...
                cpu.logicalCpuCount = vcpu_count
        if not cpu_list:
...
def disAIX(host_obj, client, Framework, langBund, host_is_virtual = False):
    ...
    for cpu in cpu_list:
        myVec.add(makeCPUOSH(host_obj, cpu.id, cpu.vendor, cpu.speed, '',
                             cpu.description, cpu.coresCount, is_virtual=host_is_virtual, logicalCpuCount=cpu.logicalCpuCount))
    return myVec
...
def _parseCpuinfo(buffer):
    ...
    logical_cpu = buffer.count('processor')
    ...
    for block in blocks:
        ...
        try:
            cpu = _Cpu(number)
            cpu.logicalCpuCount = logical_cpu
            for line in lines[1:]:
                if not line:
                    continue
                # split(':) is not used
                ...
                if key == 'physical id':
                    ...
                    ## this shows up for Linux on IBM Power7 hardware
                    # clock    : 4024.000000MHz
                    m = re.search('(\d+\.\d+)MHz', value)
                    cpu.speed = m and float(m.group(1))
                elif key == 'core id':
                    cpu.coreId = value
...
def disLinux(hostId, shell, Framework, langBund=None, host_is_virtual = False):
    ...
    for cpu in cpus:
        ...
        myVec.add(makeCPUOSH(hostId, cpuId, cpu.vendor, cpu.speed, '',
                             ...
                             logicalCpuCount=cpu.logicalCpuCount / cpuNum))
    return myVec
...
def cstm_supports_current_locale(shell, locale_name):
    output = shell.execCmd('ls -l /usr/sbin/cstm/catalog', useCache=1)
    if output and shell.getLastCmdReturnCode() == 0:
        for i in locale_name:
            if output.find(i) != -1:
                return True
        return False

def setLocale(shell, locale_name):
    if locale_name:
        try:
            ...
            logger.debug('Locale set to: %s' % locale_name)
...
def disHPUX(host_obj, client, Framework=None, langBund=None, host_is_virtual = False):
    ...
    if newAppr == 0:
        ...
        locale_name = client.getAvailableEngLocale()
        if not cstm_supports_current_locale(client, locale_name):
            logger.warn('Current locale %s is not supported by the cstm message catalog. Falling back to C locale for cstm calls' % locale_name)
            setLocale(client, ["C"])
        ...
        logger.debug("Restoring session locale to %s" % locale_name)
        setLocale(client, locale_name)
    else:
#        CPU info:
...
#          L3 Unified cache:           6 MB, 12-way
        ...
        if re.search('identification', full_cpuInfo):
            ...
            if re.search('Bus speed', full_cpuInfo):
                compiled = re.compile('\s*CPU info\s*:\s*.*?(\d+).*?Clock speed = (.*MHz)\r?\n\s*?Bus speed\s*= (.*MT/s)\r?\n\s+.*vendor information =\s*\"?(.*)\".*\r?\n\s+processor family:\s*\d+\s+(.*?)\r?\n', re.S)
                ver = 1
            else:

...
class SQLClient :
    ...
    # This function inserts a batch of sql statement to DB
    def executeInsertBatchList(self, listOfInsertStatement):
          stmt = None
          #logger.error('executeInsertBatch entered')
          try:
              stmt = self.conn.createStatement()
              for entry in listOfInsertStatement:
                 #logger.error('sql:',entry)
                 stmt.addBatch(entry)
              logger.debug('start send batch')
              stmt.executeBatch();
              logger.debug('end send batch')

          except Exception, e:
              logger.error('executeInsertBatchList:')
              logger.error(e)
              #reportStatus(Severity.FAILURE, ADD, 'id', 'updateStatus', e)
          finally:
              if (stmt != None) :
                  stmt.close()
...
def getInsertStatement(table, attributesMap, updateStatus, externalId):
...
def getUpdateStatement(table, attributesMap, idValue, updateStatus, externalId):
           iter = attributesMap.entrySet().iterator()
           ...
           while iter.hasNext():
               ...
               if counter != attributesMap.size():
                    stmtStr = stmtStr +' , '
           stmtStr = stmtStr + ' WHERE ' + ID_KEY + ' = ' + '\'' + idValue.upper() + '\''
           #logger.info('stmtStr=',stmtStr)
           return stmtStr

 # This function creates a delete statement according to the given parameters.
def getDeleteStatement(table, attributesMap, idValue, updateStatus, externalId):
    iter = attributesMap.entrySet().iterator()
...
def processCis(allObjectChildren, client, ciDictionary, objectMappings, action, updateStatus):
    ...
    while iter.hasNext():
        ...
        if (newId is None):
            ...
            #in case the ucmdb id will be pushed comment above line and uncomment the next line
            #newId=cmdbId
        ...
        if (action == ADD):
            listOfSQLStatements.append(getInsertStatement(table, attributesMap, updateStatus, externalId))
            if (len(listOfSQLStatements) == BATCH_SIZE):
                client.executeInsertBatchList(listOfSQLStatements)
                del listOfSQLStatements[:]
                batchCount=0
        elif (action == UPDATE):
            listOfSQLStatements.append(getUpdateStatement(table, attributesMap, newId, updateStatus, externalId))
            if (len(listOfSQLStatements) == BATCH_SIZE):
                client.executeInsertBatchList(listOfSQLStatements)
                del listOfSQLStatements[:]
                batchCount=0
        elif (action == DELETE):
            listOfSQLStatements.append(getDeleteStatement(table, attributesMap, newId, updateStatus, externalId))
            if (len(listOfSQLStatements) == BATCH_SIZE):
                client.executeInsertBatchList(listOfSQLStatements)
                del listOfSQLStatements[:]
                batchCount=0
    ...
    #sending last CIs
    if not listOfSQLStatements:
       logger.info('List is empty for action ',action)
    else:
       logger.error('end batchCount= ',batchCount)
       client.executeInsertBatchList(listOfSQLStatements)
       del listOfSQLStatements[:]
       batchCount=0
...
def doAction(client, allChildren, objectMappings, linkMappings, refXml, action, updateStatus):
    ...
    try:
        ...
        #logger.error('objectMappings',objectMappings)

        #logger.error("###CPE:",allObjectChildren)

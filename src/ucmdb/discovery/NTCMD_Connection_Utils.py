...
from cloud_shell import WindowsAWSDiscoverer

def doHPCmd(client, ntcmd_obj, ip_address, langBund, Framework, host_cmdbid=None, host_key=None, host_macs=None,  uduid=None, nat_ip = None, doDnsHostNameMethod=None):
    ...
    try:
        ...
        if not interfaceDiscoverer.getResults():
            try:
                ...
                interfaceDiscoverer.raw_ips.extend(shellIfaceDiscoverer.raw_ips)
            except:
    ...
    if enableAWS == None or enableAWS.lower().strip() == 'true':
        try:
            aws_discoverer = WindowsAWSDiscoverer(client)
            if aws_discoverer.is_applicable():
                hostDo.cloudInstanceId = aws_discoverer.discoverInstanceId()
        except:
            msg = logger.prepareFullStackTrace('')
            logger.debugException(msg)
    ...
    topoBuilder.build()
    # get hostOsh and add primary_dns_name
    try:
        hostOsh = topoBuilder.hostOsh
        if hostOsh and doDnsHostNameMethod:
            doDnsHostNameMethod(Framework, ip_address, client, hostOsh, resultVector)
    except:
        msg = logger.prepareFullStackTrace('')
        logger.debugException(msg)

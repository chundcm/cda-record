...
class WmiHostDiscoverer:
    ...
    def discover(self):
        ...
        try:
            installedDate = self.getOsInstalledDate()
            if installedDate:
                hostDo.osinstalledDate = installedDate
        except Exception, ex:
            logger.warn('Failed getting OS installed date. %s' % ex.message)
        ...
        try:
            hostDo.biosVersion, hostDo.biosDate = self.getBiosInfo()
        except Exception, ex:
            logger.warn('Failed getting BIOS Date. %s' % ex.message)
        ...
        try:
            hostDo.partOfDomain = self.getPartOfDomain()
        except Exception, ex:
            logger.warn('Failed getting PartOfDomain value. %s' % ex.message)
    ...
    def getPartOfDomain(self):
        r''' Get Part of Domain
        @note: part of domain can be 'true' or 'false'
        @types: -> str
        @raise Exception: WMI query failed. No data returned
        '''
        queryBuilder = self._wmiProvider.getBuilder('Win32_ComputerSystem')
        queryBuilder.addWmiObjectProperties('PartOfDomain')
        items = self._wmiProvider.getAgent().getWmiData(queryBuilder)
        if items and items[0]:
            if items[0].PartOfDomain.lower() in ['0', 'false']:
                return False
            else:
                return True
        raise Exception("WMI query failed. No data returned.")
    ...
    def getOsInstalledDate(self):
            queryBuilder = self._wmiProvider.getBuilder('Win32_OperatingSystem')
            queryBuilder.addWmiObjectProperties('InstallDate')
            wmiResults = self._wmiProvider.getAgent().getWmiData(queryBuilder)
            result = wmiResults and wmiResults[0].InstallDate
            if result:
                from java.text import SimpleDateFormat
                from java.text import ParsePosition
                sfDate = SimpleDateFormat('yyyyMMddHHmmss')
                osInstalledDate = sfDate.parse(str(result)[:14], ParsePosition(0))
                return osInstalledDate

            raise Exception("Failed to get OS installed Date")
    ...
    def getBiosInfo(self):
        biosVersion = None
        biosDate = None
        queryBuilder = self._wmiProvider.getBuilder('Win32_BIOS')
        queryBuilder.addWmiObjectProperties('biosversion')
        queryBuilder.addWmiObjectProperties('releasedate')
        wmiResults = self._wmiProvider.getAgent().getWmiData(queryBuilder)
        version = ''
        if wmiResults:
            if wmiResults[0].biosversion:
                if isinstance(wmiResults[0].biosversion, list):
                    for item in wmiResults[0].biosversion:
                        version = version + ' ' + item
                    biosVersion = version.strip()
                else:
                    biosVersion = wmiResults[0].biosversion.strip()
            if wmiResults[0].releasedate:
                biosDatelist = re.findall(r'\d+', wmiResults[0].releasedate.strip())
                sfDate = SimpleDateFormat('yyyyMMdd')
                biosDate = sfDate.parse(str(biosDatelist[0][:8]), ParsePosition(0))
    ...
    def isWin2003(self):
        return self.__isWindowsVersion(('2003',))

    def isWin7(self):
        return self.__isWindowsVersion(('7',))

    def isWindows10_2016(self):
        return self.__isWindowsVersion(('10', '2016'))

...
def get_ha_runtime_state(shell):
    result = {}
    output = shell.execAlternateCmdsList(['/opt/VRTS/bin/hagrp -state', 'hagrp -state'])
    if not output:
        logger.warn('Failed to get the HA runtime state. No Execution Environment will be reported.')
        return result
    lines = re.split('[\r\n]+', output)
    for line in lines:
        if line.find('#Group') != -1:
            continue
        #sample line
        #MultiNIC_psg State                 serverName    |ONLINE|
        attributes = line.split()
        if len(attributes) < 4:
            continue 
        ha_name = attributes[0].strip()
        ha_attribute = attributes[1].strip()
        ha_server = attributes[2].strip()
        ha_value = attributes[3].strip()
        if ha_attribute == 'State' and ha_value == '|ONLINE|':
            servers = result.get(ha_name, [])
            servers.append(ha_server)
            result[ha_name] = servers
    return result
...
def mainFunction(resBuffer, cfPath, shUtils, OSHVResult, protocol, Framework, lastUpdateTime):
    ...
    ha_runtime_state_map = get_ha_runtime_state(shUtils)
...
#                OSHVResult.add(runOSH)
            ...
            m = re.search('SystemList\s*\=\s*\{(.*?)\}', element_prop, re.DOTALL)
            if m:
            ...
            server_names = ha_runtime_state_map.get(groupName)
            if server_names:
                for server_name in server_names:
                    clusterSoftware = nodeNameToFSSWOSH.get(server_name)
                    if clusterSoftware and clusterDeviceOSH:
                        exec_link_osh = modeling.createLinkOSH('execution_environment', clusterSoftware, clusterDeviceOSH)
                        OSHVResult.add(exec_link_osh)
    ...
    for match in matches:
        ...
        try:
            if cfFileName and cfFileName.startswith("/"):
                includeFilePath = cfFileName
            else:
...
def DiscoveryMain(Framework):
    ...
    try:
        ...
        # Get main.cf path
        if ((main_cf_path == None) or (main_cf_path == 'NA')) and (not clientShUtils.isWinOs()):

...
class TopologyDiscoverer(_HasClient, _HasCrossClientHelper):
    ...
    def getVirtualMachinesInComputeResource(self, computeResource):
        ...
        while vmQuery.hasNext():
            ...
            if resultObject:
                ...
                vm.id = str(vm.reference.getValue())
                ...
                if not (vm.guest and vm.guest.getHostName()):
                    if self.config.ignoreHostsWithoutHostnames():
                        _countNoHostName += 1
                        if _LOG_VERBOSE_FILTERING:
                            logger.debug(" ......... VM '%s': can not find host name, VM is skipped" % vm.name)
                        continue

                vmsByReference[vm.reference] = vm

        logger.debug(" ......... VMs total = %s, powered off = %s, no host key = %s, no host name = %s, skipped = %s" % (_countTotal, _countPoweredOff, _countNoHostKey, _countNoHostName, _countTotal - len(vmsByReference)))
    ...
    def retrieveDiskPartitionInfo(self, hostStorageSystem, devicePathArray):
        try:
            ...
            logger.debug('Failed to retrieve disk partition info, ESX server may be in maintenance mode')
            return None
    ...
    def _resolveEsxHostnameToIp(self, host):
        ...
        if host is not None and host.virtualNicsByKey is not None:
            ...
            for key in host.virtualNicsByKey:
                VNic_IP = None
                try :
                    VNic_IP = host.virtualNicsByKey[key].getSpec().getIp().getIpAddress()
                except :
                    logger.debug("failed to get IPs from vitual Nic")
                if VNic_IP:
                    vmKenerlPorts.append(VNic_IP)
        logger.info('...Read IPs from vCenter for VMkernel: ', vmKenerlPorts)

        vmKenerlPorts = filter(None, vmKenerlPorts)
...
class VcenterReportingTopologyListener(BaseReportingTopologyListener):
    ...
    def onDatacenter(self, datacenter):
        ...
        self.topologyReporter.reportManagedDatacenter(datacenter, resultsVector, self._vcIpAddress)
...
class TopologyReporter(_HasCrossClientHelper):
    ...
    def reportDatacenter(self, datacenter, resultVector, vcIpAddress):
        dcBuilder = self.getDatacenterBuilder()
        dcOsh = dcBuilder.build(datacenter)
        dcOsh.setStringAttribute('vcenter_ip', vcIpAddress)
    ...
    def reportExtents(self, datastore, datacenter, resultVector):
        for extent in datastore.info.extents:
            ...
            if extent and extent.diskDevice:
                extent.diskDeviceOsh = self.reportDiskDevice(datastore.osh, extent.diskDevice, extentOsh, resultVector)
    ...
    def reportScsiTopology(self, host, resultVector):
        ...
        for scsi in host.iScsiHbaByIqn:
            scsiOsh = ObjectStateHolder('iscsi_adapter')
            scsiOsh.setContainer(host.hostOsh)
            slot_iqn = scsi.iqn
            scsiOsh.setStringAttribute('iqn', slot_iqn)
                ...
                for target in interface.targets:
                    ...
                    if target.transport:
                        if target.type == 'HostInternetScsiTargetTransport'and target.transport.address:
                            ...
                            remoteScsiAdaterOsh = ObjectStateHolder('iscsi_adapter')
                            remoteScsiAdaterOsh.setContainer(remoteHostOsh)
                            target_iqn = target.transport.iqn
                            key = target_iqn
                            remoteScsiAdaterOsh.setStringAttribute( 'iqn', target_iqn )
                            ...
                            targetOsh = remoteScsiAdaterOsh
                        elif target.type == 'HostFibreChannelTargetTransport' and target.transport.remoteWWNP:
                            remoteFCPortOsh = ObjectStateHolder('fcport')
                            remoteFCPortOsh.setStringAttribute('fcport_wwn', target.transport.remoteWWNP)
                            key = target.transport.remoteWWNP
                            resultVector.add(remoteFCPortOsh)
                            linkOsh = modeling.createLinkOSH('fcconnect', localPortOsh, remoteFCPortOsh)
                            resultVector.add(linkOsh)
                            targetOsh = remoteFCPortOsh

                        if localPortOsh.getObjectClass() == 'iscsi_adapter':
                            str_wwn = localPortOsh.getAttributeValue("iqn")
                        else:
                            str_wwn = localPortOsh.getAttributeValue("fcport_wwn")
                        note = "LUN for initiator:" + str(str_wwn) + " target:" + key
                        if targetOsh:
                            for lun in target.LUNs:
                                lunId = lun.getLun()
                                lunDiskId = lun.getScsiLun()
                                if lunId >= 0:
                                    lunOsh = ObjectStateHolder('LUN')
                                    lun_id = isinstance(lunId, basestring) and int(lunId) or lunId
                                    lunOsh.setIntegerAttribute('lun_id', lun_id)
                                    lunOsh.setStringAttribute('data_note', note)
                                    resultVector.add(lunOsh)

                                    linkOsh = modeling.createLinkOSH('iscsi_initiator', lunOsh, localPortOsh)
                                    resultVector.add(linkOsh)
                                    linkOsh = modeling.createLinkOSH('iscsi_target', lunOsh, targetOsh)
                                    resultVector.add(linkOsh)
                                    lunOshs = []
                                    scsiLun = host.scsiLunByKey.get(lunDiskId)
                                    diskName = scsiLun.getCanonicalName()
                                    if host.lunOshByDiskId.get(diskName):
                                        lunOshs = host.lunOshByDiskId.get(diskName)
                                    lunOshs.append(lunOsh)
                                    host.lunOshByDiskId[diskName] = lunOshs
        ...
        for datastore in datacenter._datastoresByReference.values():
            if datastore.type == 'vmfs':
                for extent in datastore.info.extents:
                    for host in computeResource._hostsByReference.values():
                        for interface in host.hostScsiTopologyInterfaces.values():
                            hbaOsh = host.adapterOshByKey.get(interface.adapter)
                            hbaInterface = host.lunDiskToInterface.get(extent.name + interface.adapter)
                            if hbaInterface and hbaOsh:
                                link = modeling.createLinkOSH('dependency', datastore.osh, hbaOsh)
                                resultVector.add(link)

                        if extent.diskDeviceOsh:
                            lunOshs = host.lunOshByDiskId.get(extent.name)
                            if lunOshs:
                                for lunOsh in lunOshs:
                                    if extent.diskDeviceOsh:
                                        linkOsh = modeling.createLinkOSH('dependency', extent.diskDeviceOsh, lunOsh)
                                        resultVector.add(linkOsh)
            ...
            if host and host.hypervisorOsh and host.vmReferences:
                for vmReference in host.vmReferences:
                    ...
                    if vm and vm.hostOsh:
                        ...
                        hypervisor_name = host.hypervisorOsh.getAttribute('hypervisor_name').getStringValue()
                        runLink.setStringAttribute('name', hypervisor_name)
                        resultVector.add(runLink)
    ...
    def reportEsxCpus(self, host, resultVector):
        hyperThread = host.hyperThread

        if host.cpuById and host.hostOsh:
            cpuBuilder = self._getEsxCpuBuilder()
            for cpu in host.cpuById.values():
                if hyperThread is not None:
                    cpu.hyperThread = hyperThread
    ...
    def reportConnectedSwitchPort(self, host, resultVector):
        ...
        if host.physicalNicCdpByphysicalNicDevice:
            for device in host.physicalNicCdpByphysicalNicDevice.keys():
                ...
                if device in  host._physicalNicDeviceToKey.keys():
                    if host._physicalNicDeviceToKey[device] in host.physicalNicOshByKey.keys():
                        interfaceOsh = host.physicalNicOshByKey[host._physicalNicDeviceToKey[device]]
                        hash_id = str(hash(str(datetime.datetime.now()) + connectedInterfaceOsh.getAttribute("interface_name").getStringValue() + interfaceOsh.getAttribute("interface_name").getStringValue()))
                        layer2_osh.setAttribute('layer2_connection_id', hash_id)
    ...
    def reportManagedDatacenter(self, datacenter, resultVector, vcIpAddress):
        '''
        ...
        if not self.config.reportBasicTopology():
            ...
            self.reportDatacenter(datacenter, resultVector, vcIpAddress)
...
class VmCreatedOrChangedEvent(Event):
    ...
    def __repr__(self):
        return "VM '%s' created or changed on host '%s', timestamp %s" % (self.virtualMachine.name, self.targetHost.name, self.creationTime)
        ...
        try:
            ...
            while self.__shouldContinueRunning(retryAttempt):
                ...
                try:
                    ...
                    logger.debug("Exception, details is:", ex)
                    lastMessage = ex.getMessage()
                    retryAttempt += 1
                except:
                    msg = logger.prepareJythonStackTrace('')
            ...
            lastMessage and self._framework.reportError(lastMessage)
        ...
        if events:
            ...
            for event in vmToEventMap.values():
                try:
                    ...
                    logger.debug("Failed to handle migrate event, going to skip this event")
                    continue
...
#                logger.warn("Cannot find parent virtual switch by key '%s' for port group '%s'" % (switchKey, portGroupKey))
    ...
    def createVmOnHostExecutionLink(self, host, vm):
        runLink = None
        if host.hypervisorOsh and vm.hostOsh:
            runLink = modeling.createLinkOSH('run', host.hypervisorOsh, vm.hostOsh)
            hypervisor_name = host.hypervisorOsh.getAttribute('hypervisor_name').getStringValue()
            runLink.setStringAttribute('name', hypervisor_name)
        return runLink
    ...
    def reportEvent(self, migratedEvent):
        ...
        if reportUnknownIPs(self._framework, vm):
            ...
            logger.debug("target Vm '%s': is skipped for its IP(%s) is out of Probe IP range" % (vm.name, list(vm._ipAddressesByIpString.keys())))


class VmCreatedOrChangedEventListener(EventListener):
    def __init__(self, client, crossClientHelper):
        EventListener.__init__(self, client, crossClientHelper)
    ...
    def _getEventTypes(self):
        return ['VmPoweredOnEvent', 'DrsVmPoweredOnEvent', 'VmCreatedEvent', 'VmReconfiguredEvent']
        ...
        if events:
            ...
            for event in events:
                try:
                    ...
                    logger.debug("Failed to handle create or change event, going to skip this event")
                    continue
...
#            logger.warn("Failed querying Host details for reference '%s'" % destinationHostRef)
    ...
    def onEvent(self, apiEvent):
        ...
        createdOrChangedEvent = VmCreatedOrChangedEvent()
        createdOrChangedEvent.creationTime = self._getCreatedTimeFromEvent(apiEvent)
        createdOrChangedEvent.virtualMachine = vm
        createdOrChangedEvent.targetHost = host

        self._reportEvent(createdOrChangedEvent)


class VmCreatedOrChangedEventReporter(EventReporter):
    def __init__(self, crossClientHelper, framework):
        EventReporter.__init__(self, crossClientHelper)
    ...
    def reportEvent(self, VmCreatedOrChangedEvent):
        vectorForAdd = ObjectStateHolderVector()

        vm = VmCreatedOrChangedEvent.virtualMachine
        host = VmCreatedOrChangedEvent.targetHost
        if not reportUnknownIPs(self._framework, vm):
            logger.debug("Host '%s': is skipped for its IP(%s) is out of Probe IP range" % (vm.name, list(vm._ipAddressesByIpString.keys())))
        else:
            self.reportVirtualMachine(vm, vectorForAdd)
...
def reportUnknownIPs(framework, vm):
    discoverUnknownIPs = 1
    try:
        ...
        pass
    isIpInRange = hasIpInProbeIpRanges(list(vm._ipAddressesByIpString.keys()))
    reportUnknownIps = isIpInRange or discoverUnknownIPs
    return reportUnknownIps

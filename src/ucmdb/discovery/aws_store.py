...
class Ebs(Volume, aws.HasId, entity.HasOsh):
    ...
    def __init__(self, id_, volumeType=None, createTime=None, encrypted=None, iops=None,  sizeInMb=None,
                 snapshotId=None, state=None, availabilityZoneName=None, arn=None):
...
class Builder:
    ...
    def visitAwsEbs(self, ebs):
        ...
        osh = ObjectStateHolder('amazon_ebs')
        osh.setAttribute('volume_id', ebs.getId())
        if ebs.volumeType:
            osh.setStringAttribute('volume_type', ebs.volumeType)
        if ebs.createTime:
            if isinstance(ebs.createTime, Date):
                osh.setDateAttribute('volume_creation_time', ebs.createTime)
        if ebs.encrypted is not None:
            osh.setBoolAttribute('volume_encrypted', bool(ebs.encrypted))
        if ebs.iops:
            osh.setIntegerAttribute('volume_iops', ebs.iops)
        if ebs.sizeInMb and ebs.sizeInMb.value() is not None:
            size = Double.valueOf(ebs.sizeInMb.value())
            osh.setAttribute('volume_size', size)
        if ebs.getState():
            osh.setAttribute('volume_state', ebs.getState())
        if ebs.arn:
            osh.setAttribute('amazon_resource_name', ebs.arn)
        return osh
...
class Reporter:
    ...
    def reportEbs(self, ebs):
        r'@types: aws_store.EBS-> ObjectStateHolderVector'
        if not ebs:
            raise ValueError("EBS is not specified")
        ebsOsh = ebs.build(self.__builder)
        return ebsOsh

    def linkEbsToAvailabilityZone(self, ebsOsh, zoneOsh):
        r'@types: ObjectStateHolder, ObjectStateHolder -> ObjectStateHolder'
        if not ebsOsh:
            raise ValueError("EBS is not specified")
        if not zoneOsh:
            raise ValueError("Availability Zone is not specified")
        return modeling.createLinkOSH('membership', zoneOsh, ebsOsh)

    def linkEbsToMappedVolume(self, ebsOsh, volumeOsh):
        r'@types: ObjectStateHolder, ObjectStateHolder -> ObjectStateHolder'
        if not ebsOsh:
            raise ValueError("EBS is not specified")
        if not volumeOsh:
            raise ValueError("Mapped volume is not specified")
        return modeling.createLinkOSH('usage', volumeOsh, ebsOsh)

...
def createJavaClient(framework, ipAddress, credentialsId):
    ...
    NA1040 = framework.getParameter("NAVersion10.40")
    if NA1040:
        if NA1040.lower() == 'true':
            try:
                BouncyCastleFipsProviderClass = Class.forName("org.bouncycastle.jcajce.provider.BouncyCastleFipsProvider")
                bfProvider = BouncyCastleFipsProviderClass.newInstance()
                Security.insertProviderAt(bfProvider, 1)
            except:
                logger.debug("Failed to load BouncyCastleFipsProvider")
...
def _parseDevice(row):
    ...
    device.lastSnapshotAttemptDate = row.getValue('lastSnapshotAttemptDate')
...
def parse_groupInfo(row):

    groupName = row.getValue('deviceGroupName')
    moduleId = row.getValue('deviceGroupID')
    createTime = row.getValue('createDate')
    parentDeviceGroupID = row.getValue('parentDeviceGroupID')
    isDynamic = row.getValue('isDynamic')
    group_item = na.DeviceGroup(str(moduleId), groupName, str(createTime),str(parentDeviceGroupID),str(isDynamic))
    return group_item

def getDevicesGroup(client):
    parse_group_itemsId = lambda row: row.getValue('deviceID')
    device_groupSet = client.executeCommandAndParse("list groups -type device", parse_groupInfo)
    device_groups = dict()
    for device_group in filter(lambda x: "Default Site" not in str(x.name) ,device_groupSet):
        #create device_group_osh
        device_groups.setdefault(device_group,[])
        subDevice_item_id = client.executeCommandAndParse("list device -group \"%s\"" % str(device_group.name), parse_group_itemsId)
        if subDevice_item_id:
            #get device_id from device_group and add into map
            device_groups.get(device_group).extend(subDevice_item_id)
...
class BaseNaDiscoverer:
    ...
    def setDicoverDeviceGroupConfig(self, discoverDeviceGroup):
        ''' int -> None '''
        if discoverDeviceGroup:
            self.discoverDeviceGroup = discoverDeviceGroup
...
class NaDiscovererPerDevice(BaseNaDiscoverer):
    ...
    def discover(self):
        ...
        deviceGroups = None
        if self.discoverDeviceGroup:
            try:
                deviceGroups = getDevicesGroup(self.client)
            except:
                logger.warn("Failed to discover device group.")

        return devicesById,connectivitiesByDeviceId,deviceGroups

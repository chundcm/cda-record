...
class AppCmdDiscover(object):
    ...
    def discover(self, shell, webservice_ext_filter=[]):
        ...
        app_of_sites, _ = self.discover_apps(vdirs, shell, webservice_ext_filter)
    ...
    def discover_apps(self, vdirs, shell, webservice_ext_filter):
        ...
        result = self.shell.execCmd(COMMAND)
        if self.shell.getLastCmdReturnCode() == 0 and result:
            apps, uri_endpoints_list = self.parse_apps(result, vdirs, shell, webservice_ext_filter)
            logger.debug('Apps: ', apps)
            logger.debug('Uri Endpoints: ', uri_endpoints_list)
            return apps, uri_endpoints_list
        else:
            return {}, {}
    ...
    def parse_apps(self, result, all_vdirs, shell, webservice_ext_filter):
        ...
        for app_element in app_elements:
            ...
            uri_endpoints = self.get_uri_endpoints(physical_path_of_app, shell, webservice_ext_filter)
            if uri_endpoints:
                uri_endpoints_list.append(uri_endpoints)

            app = WebApplication(appPool, appPath, physical_path_of_app, config_files, virtual_dirs=vdirs_of_app)
            apps = app_of_sites[siteName]
            apps.append(app)
        return app_of_sites, uri_endpoints_list

    def get_uri_endpoints(self, physical_path_of_app, shell, webservice_ext_filter):
        fs = file_system.createFileSystem(shell)
        uri_endpoints = {}
        try:
            logger.debug("try to get url from path:", physical_path_of_app)
            ...
            for webservicefile in files:
                match = re.match(r'[:\w\.\\]+\\(\w+)\\(\w+)(\.\w+)', webservicefile.path)
                if match:
                    key = match.group(2)
                    uri = match.group(1)+ '/' + match.group(2) + match.group(3)
                    logger.debug('match uri: %s' %uri)
                    uri_endpoints[key] = uri
        except:
...
def discover(shell, iisOSH, hostIPs, iis_version, webservice_ext_filter=[]):
    ...
    apps, uri_endpoints_list = appCmdDiscover.discover_apps(vdirs, shell, webservice_ext_filter)
    sites = appCmdDiscover.discover_sites(app_pools, apps)
    topology = reporter.reportTopology(app_pools.values(), sites, iisOSH, iis_version, webservice_ext_filter, uri_endpoints_list)
    return topology

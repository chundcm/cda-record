...
class TomcatDiscoverer(file_mon_utils.FileMonitorEx):
    def __init__(self, Framework, OSHVResult, shellUtils=None):
        ...
        needOracle = Framework.getParameter('discoverOracle')
        if needOracle is None:
            self.discoverOracle = True
        else:
            self.discoverOracle = Boolean.parseBoolean(needOracle)
    ...
    def discoverTomcat(self, configFile):
        ...
        # discover JVM
        installPath = configFile.strip().split('conf')[0]
        self.getJVM(installPath)
    ...
    def getJdbcResources(self, env, jdbcOshMap, globalJdbcResources=None):
        ...
        for jdbc in jdbcResources.values():
            ...
            dataSources = reporter.reportDatasources(container, datasource)

            logger.debug("data sources: ", dataSources )
            logger.debug("discoverOracle: ", self.discoverOracle )
            if not self.discoverOracle:
                logger.debug( "To remove oracle CI... " )
                dsToRemove = ObjectStateHolderVector()
                for ds in dataSources:
                    logger.debug("[", ds.getObjectClass(), "]: " ,ds.getAttributeAll() )
                    if ds.getObjectClass() == "oracle":
                        dsToRemove.add(ds)
                        logger.debug("Found oracle")
                    elif ds.getObjectClass() == "dependency":
                        end2 = ds.getAttribute("link_end2").getObjectValue();
                        if end2.getObjectClass() == "oracle":
                            logger.debug("found oracle link")
                            dsToRemove.add(ds)
                for dsOra in dsToRemove:
                    dataSources.remove(dsOra)

            logger.debug("data sources new: ", dataSources )
            self.OSHVResult.addAll(dataSources)
    ...
    def getJVM(self, installPath):
        shell = self.shellUtils
        tomcatProcesses = []
        tomcatCommandLinePattern = 'org.apache.catalina.startup.Bootstrap'

        processDiscoverer = process_discoverer.getDiscovererByShell(shell)
        for process in processDiscoverer.discoverAllProcesses():
            if str(process.commandLine).find(tomcatCommandLinePattern) != -1 \
                    and str(process.commandLine).find(installPath) != -1:
                logger.info('Find Tomcat process: ', process)
                tomcatProcesses.append(process)

        if not tomcatProcesses:
            logger.reportWarning("No Tomcat processes currently running")

        for process in tomcatProcesses:
            commandLineDescriptor = jee.JvmCommandLineDescriptor(process.commandLine)
            serverRuntime = jee_discoverer.ServerRuntime(commandLineDescriptor, self.ipaddress)
            jvmDiscoverer = jee_discoverer.JvmDiscovererByShell(shell, None)
            jvm = jvmDiscoverer.discoverJvmByServerRuntime(serverRuntime)
            if jvm:
                jvmOsh = jee.ServerTopologyBuilder().buildJvmOsh(jvm)
                jvmOsh.setContainer(self.tomcatOsh)
                self.OSHVResult.add(jvmOsh)

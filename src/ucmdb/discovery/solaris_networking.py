        ...
        try:
            ...
            for ifconfigRecord in ifconfigRecords:
                ...
                physicalInterface = self.networking.getInterfaceByName(
                    ifconfigRecord.physicalInterfaceName.split(':')[0] + (zloginZoneName or u''))
...
class NonGlobalZoneNetworkingDiscovererByShell(NetworkingDiscovererByShell):
    ...
    def _discoverIps(self, zloginZoneName=None):
        ...
        try:
            ...
            for ifconfigRecord in ifconfigRecords:
                interface = None
                physicalInterface = self.networking.getInterfaceByName(ifconfigRecord.physicalInterfaceName.split(':')[0] + (zloginZoneName or u''))
                if physicalInterface is not None:
                    interface = physicalInterface
                    if physicalInterface.name not in interfacesNameList:
                        interfacesNameList.append(physicalInterface.name)
                    ...
                    if ifconfigRecord.physicalInterfaceName != ifconfigRecord.fullInterfaceName:
                        ...
                        logicalRole = self._getOrCreateLogicalRole(logicalInterface)
                        logicalRole.setParent(physicalInterface)
                        if logicalInterface.name not in interfacesNameList:
                            interfacesNameList.append(logicalInterface.name)
...
def getInterfacesViaNetstat(shell, commandPrefix=""):
    ...
    ipToInterfaceName = {}
    ifconfigRecords = getIpsViaIfconfig(shell)
    for ifconfigRecord in ifconfigRecords:
        ipToInterfaceName[ifconfigRecord.ip] = ifconfigRecord.fullInterfaceName
    netstatOutput = getCommandOutput(command, shell)
    return parseNetstatOutput(netstatOutput, ipToInterfaceName)


def parseNetstatOutput(netstatOutput, ipToInterfaceName):
    # There are more names than macs, names can appear several times
    netstatRecordsByName = {}
    results = re.findall(r"\n(\S+)\s+(\d+\.\d+\.\d+\.\d+).+SP[A-Za-z]*\s+([0-9a-f:]+)", netstatOutput)
    resultsIpv6 = re.findall(r"\n(\S+)(\s+)([\da-f:]+)\s\s(?!other)", netstatOutput)
    results.extend(resultsIpv6)
    for row in results:
        if ipToInterfaceName.get(row[1]):
            interfaceName = ipToInterfaceName.get(row[1])
        else:
            interfaceName = row[0]
...
def parseIfconfigOutput(ifconfigOutput):
    splitList = re.split(r"([a-zA-Z0-9_-]+(?::\d+)?):\s+flags=\d+<([\w,-]+)>", ifconfigOutput)
    if len(splitList) < 2:
...
def resolveHostnameToIp(hostname, shell):
    ...
    try:
        ...
        resolvedIP = parseNslookupOutput(nslookupOutput, hostname)
    except:
        fileBasedResolver = netutils.DNSResolver(shell)
        resolvedIP = fileBasedResolver.resolveHostIpByHostsFile(hostname)
    return resolvedIP

...
from com.hp.ov.nms.sdk.iface import NmsInterfaceBindingStub
...
class CmdbConnection:
    ...
    def __init__(self, serverHost, username, password, serverIp = None, port = "8080", protocol = "http"):
...
class NnmConnection:
    ...
    def getNodeBeanServiceUrl(self):
        urlStr = '%s://%s:%s/NodeBeanService/NodeBean' % (self.protocol, self.serverIp, self.port)
        return URL(urlStr)

    def getInterfaceBeanServiceUrl(self):
        urlStr = '%s://%s:%s/InterfaceBeanService/InterfaceBean' % (self.protocol, self.serverIp, self.port)
        return URL(urlStr)
...
def _createNodeRequestObject(nnmServerIp, cmdbContext, nnmiHostName=None):
    if nnmiHostName:
        uidConditionValue = ID_SEPARATOR.join(["%", nnmiHostName])
    else:
        uidConditionValue = ID_SEPARATOR.join(["%", nnmServerIp])
    return _createRequestObject(nnmServerIp, cmdbContext, NODE, "host_nnm_internal_key", uidConditionValue)


def _createInterfaceRequestObject(nnmServerIp, cmdbContext, nnmiHostName=None):
    if nnmiHostName:
        uidConditionValue = ID_SEPARATOR.join(["%", nnmiHostName, "%"])
    else:
        uidConditionValue = ID_SEPARATOR.join(["%", nnmServerIp, "%"])
    return _createRequestObject(nnmServerIp, cmdbContext, INTERFACE, "interface_nnm_internal_key", uidConditionValue)


def _createRequestObject(nnmServerIp, cmdbContext, cit, keyAttribute,uidConditionValue):
    ...
    propertyNamesList.add(keyAttribute)
...
def getUCMDBIDs(cmdbConnection, nnmServerIp, framework, cmdbContext, request):
...
def _processCIs(cis):
    ...
    try:
        ...
        logger.debug('Retrieved UCMDB IDs of %d CIs from the UCMDB server' % ciSize)
...
def _createNnmStub(nnmConnection, cit):
    ''' NnmConnection -> NmsNodeBindingStub '''
    try:
        if cit == INTERFACE:
            url = nnmConnection.getInterfaceBeanServiceUrl()
            stub = NmsInterfaceBindingStub(url, None)
        else:
            url = nnmConnection.getNodeBeanServiceUrl()
...
def _getMethodByCIT(cit):
    if cit == INTERFACE:
        return _getNnmInterfaceById
    else:
        return _getNnmNodeById
...
def _getNnmInterfaceById(nnmStub, nnmId):
    ''' Stub, string -> list(Interface) '''
    try:
        interfaceFilter = FF.EMPTY | FF.CONDITION('id', '==', nnmId)
        return nnmStub.getInterfaces(interfaceFilter.nr()).getItem()
    except:
        logger.warn("Failed getting NNM interface by ID %s" % nnmId)

    return []

def _updateNnmNode(nnmConnection, nnmStub, cmdbId, nnmId, framework, cit):
    ...
    customAttributes = [CustomAttribute("UCMDB_ID", cmdbId), CustomAttribute("ODB_ID", cmdbId)]
...
def updateNNM(cit, cmdbToNnmIds, nnmConnection, framework):
    ...
    stub = _createNnmStub(nnmConnection, cit)
    _getNnmItemById = _getMethodByCIT(cit)
    ...
    for cmdbId, nnmId in cmdbToNnmIds.items():
        ...
        nodes = _getNnmItemById(stub, nnmId)
        if not nodes:
        ...
        try:
            _updateNnmNode(nnmConnection, stub, cmdbId, nnmId, framework, cit)
...
def getCmdbConnectionDetails(framework, protocolObject, serverHost, serverIp):
    ...
    try:
        ...
        errorMsg = 'UCMDB Web Service username is not defined in the NNM Protocol.'
    ...
    try:
        ...
        errorMsg = 'UCMDB Web Service password is not defined in the NNM Protocol.'
        logger.error(errorMsg)
        framework.reportError(errorMsg)

    return CmdbConnection(serverHost, username, password, serverIp, port, protocol)
...
def updateNNMByCIT(cmdbConnection, nnmConnection, Framework, cmdbContext, request, cit):
    logger.debug("Starting to update %s" % cit)
    cmdbToNnmIds = getUCMDBIDs(cmdbConnection, nnmConnection.serverIp, Framework, cmdbContext, request)
    updateNNM(cit, cmdbToNnmIds, nnmConnection, Framework)
...
def DiscoveryMain(Framework):
    try:
        ...
        nnmiHostName = Framework.getParameter('NNMiHost')

        request = _createNodeRequestObject(nnmConnection.serverIp, cmdbContext, nnmiHostName)
        updateNNMByCIT(cmdbConnection, nnmConnection, Framework, cmdbContext, request, NODE)

        request = _createInterfaceRequestObject(nnmConnection.serverIp, cmdbContext, nnmiHostName)
        updateNNMByCIT(cmdbConnection, nnmConnection, Framework, cmdbContext, request, INTERFACE)

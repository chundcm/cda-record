...
PortTypeEnum = netutils.PortTypeEnum.merge(
    netutils._PortTypeEnum(
        ...
        J2EE=netutils._PortType('J2EE'),
        J2EES=netutils._PortType('J2EES'),
        DIAG=netutils._PortType('DIAG'),
        RFC=netutils._PortType('RFC')
    ))
...
class System(entity.HasName, entity.Immutable):
    def __init__(self, name, globalHost=None, type_=None, defaultPfPath=None,
                 uuid_=None, extendedSystemID=None):
        ...
        self.extendedSystemID = extendedSystemID
        try:
...
def isCorrectSystemName(name):
    ...
    if name and SYSTEM_NAME_PATTERN.match(name) is not None:
        return True
    logger.debug('SAP System name is not valid, name is: ', name)
    return False
...
class Client(entity.Immutable):
    ...
    def __init__(self, name, roleType, description=None,
                                       cityName=None):
        r'@types: str, str'
        Client.checkClientArgument(name, roleType)
        self.name = name
    ...
    @classmethod
    def checkClientArgument(cls, name, roleType):
        assert name
        if roleType and roleType not in cls.RoleType.values():
...
class Builder:
    ...
    def buildSapSystem(self, system):
        ...
        if system.extendedSystemID:
            osh.setAttribute("extended_system_id", system.extendedSystemID)
        self.updateUuid(osh, system.uuid)
        modeling.setAppSystemVendor(osh)
        return osh

    def buildSapFrunInstance(self, ipAddress, credential, hostOsh):
        r'@types: sap.SapFrunInstance -> ObjectStateHolder'
        osh = ObjectStateHolder("sap_frun_instance")
        osh.setStringAttribute('application_ip', ipAddress)
        osh.setStringAttribute('credentials_id', credential)
        osh.setStringAttribute('discovered_product_name', "SAP FRUN")
        osh.setStringAttribute('vendor', "SAP")
...
class Reporter:
    ...
    def reportFrunInstance(self, ipAddress, credential, hostOsh):
        r'''@types: sap.SapFrunInstance -> ObjectStateHolder
        @raise ValueError: SapFrunInstance is not specified
        '''
        if not credential:
            raise ValueError("SapFrunInstance is not specified")
        return self.__builder.buildSapFrunInstance(ipAddress, credential, hostOsh)
...
class ABAPCentralServiceBuilder(BaseCentralComponentBuilder):
    CIT = 'abap_sap_central_services'
    DISCOVERED_PRODUCT_NAME = 'SAP ABAP Central Services'
    PRODUCT_NAME = 'abap_sap_central_services'


class ABAPApplicationServerBuilder(BaseCentralComponentBuilder):
    CIT = 'sap_r3_server'
    DISCOVERED_PRODUCT_NAME = 'SAP ABAP Application Server'
    PRODUCT_NAME = 'sap_abap_application_server'


class J2EEApplicationServerBuilder(BaseCentralComponentBuilder):
    CIT = 'sap_j2ee_app_server'
    DISCOVERED_PRODUCT_NAME = 'SAP J2EE Application Server'
    PRODUCT_NAME = 'sap_j2ee_application_server'

class HanaDBInstanceBuilder():
    CIT = 'hana_instance'
    DISCOVERED_PRODUCT_NAME = 'SAP HanaDB'
    PRODUCT_NAME = 'hana_database'
...
class HostReporter(_HasBuilder):
    ...
    def reportHostWithIps(self, *ips):
        ...
        '''
        ips = filter(None, ips)
        ips = filter(lambda x: not x.is_loopback, map(ip_addr.IPAddress, ips))
        if not ips:
            raise ValueError("IPs are not specified")

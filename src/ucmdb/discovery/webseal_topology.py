...
class Reporter(object):
    ...
    def report_junction(self, junction_pdo, webseal_osh, webseal_container=None, application_endpoints=None):
        ...
        elif len(junction_pdo) == 3:
            junction_name, servers, server_state = junction_pdo
        else:
            junction_name, servers, server_state, virtual_host_name = junction_pdo
        oshs = []
        ...
        if virtual_host_name:
            junction_osh.setStringAttribute('virtual_host_name', virtual_host_name.strip().lower())

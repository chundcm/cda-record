    ...
    def parseServerEndpoint(self, value):
        ...
        return (valueByName
                and netutils.createTcpEndpoint(
                    ...
                    sap.PortTypeEnum.findByName(valueByName.get('PROT')) or netutils.PortTypeEnum.findByName(valueByName.get('PROT'))
                ) or None
        )
                ...
                if matchObj:
                    (appServerEndpointsBySystemName.setdefault(systemSid, []).
                     append(netutils.createTcpEndpoint(
                        matchObj.group(2),
                        matchObj.group(3),
                        sap.PortTypeEnum.findByName(matchObj.group(1)) or netutils.PortTypeEnum.findByName(matchObj.group(1)))))
        servedSystems = []

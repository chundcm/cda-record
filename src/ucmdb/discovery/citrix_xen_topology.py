...
class Discover(object):
    ...
    def doDiscover(self, hypervisor_osh):
        ...
        for host in hosts:
            ...
            virtual_osh.setStringAttribute('name', 'Citrix Xen Hypervisor_%s' % host.getHostName())
            virtual_osh.setStringAttribute('product_name', 'xen_hypervisor')
            virtual_osh.setContainer(host_osh)
            ...
            for vm in vms:
                ...
                vector.add(vm_osh)
                vector.add(modeling.createLinkOSH('execution_environment', virtual_osh, vm_osh))
...
class Builder(object):
    ...
    def build_vm(self, vm):
        node_os = 'host_node'
        try:
            template = vm.data.get('other_config').get('base_template_name')
            node_os = self.get_node_type(template)
        except:
            if template:
                logger.debug('Cannot parse os type : '+ template)
            else:
                logger.debug('Cannot get os type ')
        vmOsh = ObjectStateHolder(node_os)
        ...
        vmOsh.setStringAttribute('platform_vendor', PlatformVendors.Citrix)
        return vmOsh
    ...
    def get_node_type(self, template):
        node_type = 'host_node'
        if 'Windows' in template:
            node_type = 'nt'
        elif 'Other' in template:
            logger.debug('Other install media detected... report it to computer')
        else:
            node_type = 'unix'
        return node_type

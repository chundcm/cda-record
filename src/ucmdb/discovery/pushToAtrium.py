...
def processProtocol(Framework):
    ...
    if remedyProtocol != None:
        ...
        if isNoneOrEmpty(remedyProtocolName) or remedyProtocolName != 'remedyprotocol':
            ...
            logger.debug('remedyprotocol_user: %s' % targetUser)
            logger.debug( "Remedy Protocol pwd is empty: ", ( not targetPass) )

            if not targetUser:
                targetUser = remedyProtocol.getProtocolAttribute( 'protocol_username', '' )
                targetPass = remedyProtocol.getProtocolAttribute( 'protocol_password', '' )
                logger.debug('protocol_username: %s' % targetUser)
                logger.debug( "Protocol pwd is empty: ", ( not targetPass) )
...
def getPushProperties():
    ...
    try:
        ...
        try:
            tryDeleteRelations = True
            tryDeleteRelations = properties.getProperty("deleteRelations")
            if (not isNoneOrEmpty(tryDeleteRelations) ) and (string.lower(string.strip(tryDeleteRelations)) == 'false'):
                tryDeleteRelations = False
            else:
                tryDeleteRelations = True

            logger.debug("Try delete relations also: ", tryDeleteRelations)
        except:
            logger.debugException("Unable to read deleteRelations property from push.properties")

        fileInputStream.close()
    except:
        logger.debugException("Unable to process %s file." % PUSH_PROPERTIES_FILE)

    return debugMode, smartUpdateIgnoreFields, sortCSVFields, testConnNameSpace, testConnClass, tryDeleteRelations
...
def insertLinkToAtrium(CONTEXT, className, end1CMDBId, end1ClassName, end1superclass, end2CMDBId, end2ClassName, end2superclass, attributesMap, mode, mamId, keyAttributes, role1, role2, nameSpace, smartUpdateIgnoreFieldsList):
    ...
    logger.debug('insertLinkToAtrium: ', mode, ': link for Atrium: ', end1CMDBId, " -> ", end2CMDBId, ", mamId: ", mamId)
    ...
    if mode == UPDATE:
        ...
        if aRelBase == None:
            ...
            if ciNeedsUpdating(mamId, className, attributesMap, keyAttributes, nameSpace, aRelBase, smartUpdateIgnoreFieldsList):
                ...
                externalId = aRelBase.getId()
                logger.debug ('insertLinkToAtrium: update success ',mamId, ", Atrium ID: ", externalId)
        ...
        if (mode == INSERT or foundCi == None):
            ...
            if (not isNoneOrEmpty(end1CMDBId) and not isNoneOrEmpty(end2CMDBId)):
                ...
                logger.debug('insertLinkToAtrium: create relation success ', mamId, ", Atrium ID: ", externalId )
                slogger.debug("%s\t%s\t%s\t%s\t%s\t%s\t%s\n" % (str(mamId), mode, className, end1CMDBId,  nameLink1, end2CMDBId, nameLink2))
            else:
            ...
            try:
                if ciNeedsUpdating(mamId, className, attributesMap, keyAttributes, nameSpace, foundCi, smartUpdateIgnoreFieldsList):
                    ...
                    logger.debug ('insertLinkToAtrium: update relation success ', mamId, ", Atrium ID: ", externalId)
    ...
    if externalId:
        cacheExternalCiId(mamId, externalId)
...
'''
    Method to delete relations from the inbound XML from the UCMDB server
'''
def deleteRelations(CONTEXT, allRelationData):
    for linkElement in allRelationData:
        ...
        logger.debug('(Del-Relations) ClassName %s, mamID %s, nameSpace %s ' % (className, mamId, nameSpace) )
        ...
        try:
            remedyId = restoreExternalCiId(mamId)
            if remedyId:
                logger.info('(Del-Relatiosn) Atrium ID restored %s' % remedyId)
                CMDBInstance.delete(CONTEXT, aClassKey, 'TOPO.DDM', remedyId, CMDBInstance.CMDB_CASCADE_DELETE_FOLLOW_WEAK_RELATIONSHIPS)
            else:
                logger.error('(Del-Relations)Failed to delete relation. Atrium ID by UCMDB ID %s not found' % mamId)
        except:
            logger.errorException('(Del-Relations)Failed to delete Atrium Relation')
...
'''
def processInboundXml(operation, xmlResult, CONTEXT, debugMode, smartUpdateIgnoreFieldsList, sortCSVFieldsList, tryDeleteRelations = False):
    ...
    try:
        ...
        if operation in ('add', 'update'):
            ...
            if tryDeleteRelations:
                logger.debug( "Try to delete relations separately begin.")
                relationsData = xmlData.getRootElement().getChild('data').getChild('links').getChildren('link')
                deleteRelations(CONTEXT, relationsData)
                logger.debug( "Try to delete relations separately end.")
...
def testRemedyAtriumConnection(CONTEXT, testConnNameSpace, testConnClass):
    ...
    try:
        ...
        logger.debug("Test connection failed (Exception): ", ex )
        return 0
    except:
        excMsg = traceback.format_exc()
        logger.debug( "Test connection failed: ", excMsg )
        return 0
    return 1
...
def DiscoveryMain(Framework):
    ...
    (debugMode, smartUpdateIgnoreFields, sortCSVFields, testConnNameSpace, testConnClass, tryDeleteRelations) = getPushProperties()
    ...
    if validateAdapterDirs():
        ...
        if testConnection == 'true':
            if not success:
                ...
                logger.debug("Atrium DiscoveryMain finished")
                raise Exception, CONNECTION_FAILED
                return
            else:
                logger.debug("Test connection was successful. DiscoveryMain finished")
                return

        if not success:
            logger.errorException(CONNECTION_FAILED)
            logger.reportError(CONNECTION_FAILED)
            logger.debug("Atrium DiscoveryMain finished")
            return
        ...
        deleteStatus  = processInboundXml("delete", deleteResult, CONTEXT, debugMode, smartUpdateIgnoreFieldsList, sortCSVFieldsList, tryDeleteRelations)

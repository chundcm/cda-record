...
class HostDiscovererByShell:
    ...
    def discover(self):
        ...
        try:
            resultDo = self.getBiosFromWMIC()
            self.hostDo.biosVersion = self.hostDo.biosVersion or resultDo.biosVersion
            self.hostDo.biosDate = self.hostDo.biosDate or resultDo.biosDate

        except Exception, ex:
            logger.warn('Failed getting Bios  from WMIC %s' % ex)
        ...
        try:
            if not self.hostDo.osinstalledDate:
                self.hostDo.osinstalledDate = self.getOSinstalledDate()
        except Exception, ex:
            logger.warn('Failed getting host description from registry. %s' % ex)
    ...
    def getBiosFromWMIC(self):
        hostDo = HostDo()
        biosVersion = self.shell.execCmd('wmic path Win32_BIOS get BIOSVersion')
        biosDate = self.shell.execCmd('wmic path Win32_BIOS get ReleaseDate')
        if biosVersion:
            if biosVersion.count(',') == 0:
                matchResult = re.search(r'{"(.*?)"}', biosVersion)
                hostDo.biosVersion = matchResult.group(1)
            elif biosVersion.count(',') == 1:
                matchResult = re.search(r'{"(.*?)", "(.*)"}', biosVersion)
                hostDo.biosVersion = matchResult.group(1) + ' ' + matchResult.group(2)
            else:
                logger.warn('Cannot get BiosVersion.')

        if biosDate:
            biosDatelist = re.findall(r'\d+', biosDate)
            sfDate = SimpleDateFormat('yyyyMMdd')
            biosDate = sfDate.parse(str(biosDatelist[0][:8]), ParsePosition(0))
            hostDo.biosDate = biosDate

        return hostDo

    def getOSinstalledDate(self):
        installedDateStr = self.shell.execCmd('wmic path Win32_OperatingSystem get InstallDate')
        if installedDateStr:
            from java.text import SimpleDateFormat
            from java.text import ParsePosition
            sfDate = SimpleDateFormat('yyyyMMddHHmmss')
            installedDatelist = re.findall(r'\d+', installedDateStr)
            osinstalledDate = sfDate.parse(str(installedDatelist[0])[:14], ParsePosition(0))
    ...
    def getDefaultGateway(self):
        ...
        if output and self.shell.getLastCmdReturnCode() == 0:
            ...
            strWindowsNetstatDefaultGwInRouter = self.langBund.getString('windows_netstat_default_gateway_in_router')
            for line in output.split('\n'):
                matched = re.match(strWindowsNetstatDefaultGwInRouter, line)
                if matched:
                    return matched.group(1).strip()
...
def separateCaption(caption, OtherTypeDescription = None):
    ...
    hostNameMatch = re.search('(Windows)([^\d]+)(NT|2000|XP|2003|Vista|2008|7|8\.\d|8|10|2012|2016)(.+|)', caption)
    osName = caption

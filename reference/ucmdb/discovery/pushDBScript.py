    ...
    while iter.hasNext():
        ...
        if (newId is None):
            ...
            if ((action == ADD) or (action == UPDATE)):
                ...
                logger.info('try to delete CI item which not exist, skip it!')                
        else:
        ...
        if (newId is None):
            if((action == ADD) or (action == UPDATE)):
                ...
                try: 
                    ...
                    logger.error('Failed to add or update linkage due to externalId creation error')                    
                    reportStatus(Severity.FAILURE, ADD, linkObjId, updateStatus, e)                
            elif (action == DELETE):
                logger.info('try to delete linkage which not exist, skip it!')                
        else:
...
def DiscoveryMain(Framework):
    ...
    if (credentialsId == '' ):
        raise RuntimeError('Credentials are empty. Please fill the credentials for the database')
    ...
    try:
        ...
        logger.error('Failed process for credential information is not complete: ', e)
        raise Exception(e)

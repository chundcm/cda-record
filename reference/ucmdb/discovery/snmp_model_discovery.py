...
class ModelDiscover(SnmpHelper):
    ...
    def add_resource_to_model(self, attribute, type, dvlsn=None):
        ...
        if attribute.ShortName:
            resource.Child[tr].ShortName = attribute.ShortName
        else:
            resource.Child[tr].ShortName = ''

        if attribute.MacAddr:
            resource.Child[tr].MacAddr = attribute.MacAddr
        else:
            resource.Child[tr].MacAddr = None

        if attribute.Model:
            resource.Child[tr].Model = attribute.Model
        else:
            resource.Child[tr].Model = ''

        if attribute.Vendor:
            resource.Child[tr].Vendor = attribute.Vendor
        else:
            resource.Child[tr].Vendor = ''

        if attribute.Size:
            resource.Child[tr].Size = attribute.Size
        else:
            resource.Child[tr].Size = 0

        if attribute.SerialNumber:
            resource.Child[tr].SerialNumber = attribute.SerialNumber
        else:
            resource.Child[tr].SerialNumber = ''

        if attribute.Version:
            resource.Child[tr].Version = attribute.Version
        else:
            resource.Child[tr].Version = ''
...
@BuildFor('interface')
class InterfaceBuilder(CIBuilder):
    def build(self, module, container, node):
        if module.MacAddr or module.Name:
            osh = modeling.createInterfaceOSH(module.MacAddr, hostOSH=container, description=module.Description, name=module.Name)
            return osh, node
        return None, None

@BuildFor('remote_management_card')
class RMCBuilder(CIBuilder):
    def build(self, module, container, node):
        if module.MacAddr or module.Name:
            osh = modeling.createInterfaceOSH(module.MacAddr, hostOSH=node, description=module.Description, name=module.Name, interfaceClass="remote_management_card")
            if osh:
                osh.setStringAttribute("name", module.ShortName)
                osh.setStringAttribute("vendor", module.Vendor)
                osh.setStringAttribute("version", module.Version)
                return osh, node
        return None, None

@BuildFor('disk_device')
class DiskBuilder(CIBuilder):
    def build(self, module, container, node):
        if module.Name:
            osh = ObjectStateHolder('disk_device')
            osh.setStringAttribute('name', module.Name)
            osh.setStringAttribute('serial_number', module.SerialNumber)
            osh.setIntegerAttribute('disk_size', module.Size)
            osh.setStringAttribute('vendor', module.Vendor)
            osh.setStringAttribute('model_name', module.Model)
            if module.Container:
                container = module.Container
            else:
                container = node
...
class CIBuilderHelper(object):
    ...
    def buildCIs(self):
        ...
        if model.Model:
            self.node.setStringAttribute('discovered_model', model.Model)
        if model.DnsName:
            self.node.setStringAttribute('primary_dns_name', model.DnsName)
    ...
    def buildPorts(self, ports):
        ...
        for child in ports.values():
            osh = builder.build(child, self.node, self.node)
            self.vector.add(osh)

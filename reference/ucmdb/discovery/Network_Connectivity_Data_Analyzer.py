...
class ConnectionDao(Dao):
    def getUniqueSrcIps(self, protocol=6):
        ...
        srcAddrSqlBuilder.where('srcAddr not like %s and prot=%d' % (invalidIp, protocol))
    ...
    def getConnections(self, srcIps=None, hostId=None, protocol=6):
        moreInvalidIp = '\'0%\''
        invalidIpClause = 'dstAddr not like %s' % moreInvalidIp
        srcIps = srcIps or []
...
def DiscoveryMain(Framework):
    try:
        #TODO delete expired connection record from database
        import tcpdbutils
        from com.hp.ucmdb.discovery.library.communication.downloader.cfgfiles import GeneralSettingsConfigFile
        tcpdbutils = tcpdbutils.TcpDbUtils(Framework)
        DELETESQL = 'delete from %s where stamp < NOW() - INTERVAL \'%s hours\''
        try:
            tcpdbutils.executeUpdate(DELETESQL % ('Agg_V5', GeneralSettingsConfigFile.getInstance().getPropertyIntegerValue('tcpExpirationTime', 24)))
            logger.debug("Delete expired connection record from database successfully")
        except:
            logger.warn("Failed to delete expired records from probe database")

from arxview_decorators import mandatory_attribute, abstract_method
from com.hp.ucmdb.discovery.library.credentials.dictionary import ProtocolDictionaryManager
from arxview_client import ArxviewClient
import logger
...
class FrameworkBasedConnectionDataManager(AbstractConnectionDataManager):
    ...
    @mandatory_attribute
    def getConnectionUrl(self):
        url = self.__framework.getParameter('Arxview URL')
    ...
    def getClient(self):
        if not self.__client:
            avc = ArxviewClient(self.getConnectionUrl(), self.trustAllCerts())
            logger.info("Arxview version:", avc.getVersion())
            avc.login(self.getUsername(), self.getPassword())
            self.__client = avc
        return self.__client

    def closeClient(self):
        if self.__client:
            logger.info("Arxview Logout.")
            try:

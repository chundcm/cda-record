...
DISABLE_AGENT_CALL_HOME = 'DisableCallHomeInAgent'
...
#AgentDrivenFlow/ADID: Agent Driven Inventory Discovery, previously named as Smart Agent
AGENT_DRIVEN_FLOW_OPTION_ENABLE = 'SCHEDULER_Enabled'
AGENT_DRIVEN_FLOW_OPTION_PROBEBASEURL = 'SCHEDULER_ProbeBaseURL'
AGENT_DRIVEN_FLOW_OPTION_CHECKINTERVALSECONDS = 'SCHEDULER_CheckIntervalSeconds'
AGENT_DRIVEN_FLOW_OPTION_ALLOWPREPOSTSCRIPT = 'SCHEDULER_AllowScanScript'
AGENT_DRIVEN_FLOW_OPTION_CPU_THRESHOLD = 'SCHEDULER_CPUThreshold'
AGENT_DRIVEN_FLOW_OPTION_MEM_THRESHOLD = 'SCHEDULER_MemThreshold'
AGENT_DRIVEN_FLOW_OPTION_KEY_PASS = 'SCHEDULER_KeyPass'

JOB_PARAM_AGENT_DRIVEN_FLOW_ENABLED = 'EnableAgentDrivenFlow'
JOB_PARAM_AGENT_DRIVEN_FLOW_ALLOWPREPOSTSCRIPT = 'AllowPrePostScriptForAgentDrivenFlow'
JOB_PARAM_AGENT_DRIVEN_FLOW_CHECKINTERVALSECONDS = 'CheckIntervalSecondsForAgentDrivenFlow'
JOB_PARAM_AGENT_DRIVEN_FLOW_MEM_THRESHOLD = 'ADIScanPostponeMemoryThreshold'
JOB_PARAM_AGENT_DRIVEN_FLOW_CPU_THRESHOLD = 'ADIScanPostponeCpuThreshold'
JOB_PARAM_AGENT_DRIVEN_FLOW_ALLOW_PROBE_URL_OVERRIDE = 'updateProbeUrl'
JOB_PARAM_AGENT_DRIVEN_FLOW_ALLOW_PROBE_NAMES_TO_OVERWRITE = 'allowOverwriteProbeUrlNames'
...
def agentInstallRoutine(Framework):
    ...
    logger.debug('Installation command before adding call home command:', installationCmd)
    installationCmd = handleCallHomeURLsVariables(Framework, installationCmd, agentPlatformConfig)
    logger.debug('Installation command after adding call home command:', installationCmd)
    ...
    for ip in ipList:
        ...
        if client:
            ...
            if uduid: # in case of update job
                ...
                if (str(hostnameOnClient).lower() == str(hostname).lower()) or (len(hostname) == 15):
...
def getCallHomeUrl0(Framework):
    callHomeUrl0 = Framework.getParameter(ZERO_CH_PARAM)
    disableCallHomeInAgent = Boolean.parseBoolean(Framework.getParameter(DISABLE_AGENT_CALL_HOME))
    if (not disableCallHomeInAgent) and (callHomeUrl0 is None):
        env = EnvironmentImpl.getInstance()
        probeGatewayIP = env.getProbeGatewayIP()
        probeGatewayCallhomePort = 80
        try:
            probeGatewayCallhomePort = env.getProbeGatewayCallhomePort()
        except:
            logger.debug('No such API getProbeGatewayCallhomePort(), defaulting to 80')
        import ip_addr
        if ip_addr.isValidIpAddress(probeGatewayIP) and ip_addr.IPAddress(probeGatewayIP).get_version() == 6:
            callHomeUrl0 = "[%s]:%s" % (probeGatewayIP, probeGatewayCallhomePort)
        elif not ip_addr.isValidIpAddress(probeGatewayIP) and ( (not probeGatewayIP is None) and (probeGatewayIP.find(".") < 0) ) :
            logger.debug("Looks like the probe gateway host name is used, will replace it with FQDN of probe gateway: ", probeGatewayIP)
            probeGatewayIP = socket.getfqdn()
            logger.debug("The FQDN of probe gateway is: ", probeGatewayIP)
            callHomeUrl0 = "%s:%s" % (probeGatewayIP, probeGatewayCallhomePort)
        else:
            callHomeUrl0 = "%s:%s" % (probeGatewayIP, probeGatewayCallhomePort)
        logger.debug(
    ...
    if disableCallHomeInAgent:
        callHomeUrl0 = '';
        logger.debug( 'DisableCallHomeInAgent is true, call home functionality would be disabled in agent side, no ' + ZERO_CH_PARAM + ' would be provided ')
...
def handleCallHomeURLsVariables(Framework, installCommand, agentPlatformConfig):
    ...
    url0Param = getCallHomeUrl0(Framework)
    logger.debug( 'HandleCallHomeURLsVariables, url0Param:', url0Param)
...
def calculateCallhomeFrequency(Framework):
    ...
    disableCallHomeInAgent = Boolean.parseBoolean(Framework.getParameter(DISABLE_AGENT_CALL_HOME))
    logger.debug('Configured callhome frequency days:', callHomeFreqParam, ", callhome disabled: ", disableCallHomeInAgent)
...
def calculateCallhomeValue(callHomeFreqParam):
    ...
    if callHomeFreqSecondsInt == 0:
        logger.warn('Call home would be disabled because call home frequency was configured as 0, setting ', str(MINIMUM_FREQUENCY_CH_DAYS), ' day')
...
def updateCallHomeParams(Framework):
    ...
    try:
        ...
        # In case there's a url0 placeholder we either delete it (if url0 is empty) or replace it with an actual
        # platform specific url0 parameter (or a probe's gateway address if the parameter is not specified)
        callHomeUrl0 = getCallHomeUrl0(Framework)
...
def getProbePortForAgentDrivenDiscovery( ):
    ''' returns localUseHttp, localADIDPort '''

    # is union mode, then just use gateway port
    # otherwise, use probe mgr port
    httpsEnabled = getProbeJettyHttpsEnabled()
    port = getADIDPort()

    return not httpsEnabled, port

def getProbeInUnionMode():
    isUnion = True
    try:
        isUnion = Boolean.parseBoolean(CollectorsParameters.getValue(CollectorsParameters.KEY_AGENT_UNION_LOCAL_PROBE))
    except:
        pass

    return isUnion

def getProbeJettyHttpsEnabled():
    httpsEnabled = True
    try:
        httpsEnabled = Boolean.parseBoolean(CollectorsParameters.getValue(CollectorsParameters.KEY_JETTY_ENABLED_HTTPS))
    except:
        pass
...
def getADIDKeyPass():
    ...
    return globalSettings.getPropertyStringValue('ADIDKeyPass', '')

def getADIDPort():
    port = 9443
    try:
        port = Integer.parseInt(CollectorsParameters.getValue(CollectorsParameters.KEY_AGENT_DRIVEN_DISCOVERY_CUSTOM_PORT))
    except:
        pass

    return port

def getSmartAgentEnabledLocally(Framework):
    enabledInProbe = False
    enabledInJob = Boolean.parseBoolean(Framework.getParameter(JOB_PARAM_AGENT_DRIVEN_FLOW_ENABLED))
    # check whether enabled in probe
    if enabledInJob:
        try :
            enabledInProbe = Boolean.parseBoolean(CollectorsParameters.getValue(CollectorsParameters.KEY_AGENT_DRIVEN_DISCOVERY_ENABLED))
        except:
            pass
    return enabledInJob and enabledInProbe

def _safeParsePercentValue(value):
    try:
        intVal = Integer.parseInt(value)
        if intVal > 0 and intVal <= 100:
            return intVal
        logger.warn('Provided value is not in a valid percent range. Value is %s' % intVal)
    except JavaException, e:
        logger.warn('Provided value is not a valid integer %s' % value)
    return None
    
def buildProbeBaseUrl(localUseHttp, localProbePort):
    if localUseHttp:
        probeBaseUrl = "http://"
    else:
        probeBaseUrl = "https://"
    import ip_addr
    probeManagerIP = EnvironmentImpl.getInstance().getProbeManagerIP()
    if ip_addr.isValidIpAddress(probeManagerIP) and ip_addr.IPAddress(probeManagerIP).get_version() == 6:
        probeManagerIP = '[' + probeManagerIP + ']'
    elif not ip_addr.isValidIpAddress(probeManagerIP) and ( (not probeManagerIP is None) and (probeManagerIP.find(".") < 0) ) :
        logger.debug("Looks like the probe manager host name is used, will replace it with FQDN of probe manager: ", probeManagerIP)
        probeManagerIP = socket.getfqdn()
        logger.debug("The FQDN of probe manager is: ", probeManagerIP)
    return probeBaseUrl + probeManagerIP + ":" + str(localProbePort) + "/rest/agent/"

def updateAgentDrivenFlowOptions(Framework, agentClient):
    try:
        ucmdb_version = modeling.CmdbClassModel().version()
        logger.debug('UCMDB Version is: ', ucmdb_version )

        if ucmdb_version < 10.34:
            logger.debug('UCMDB Version should be 10.34 or later, skip updating the agent driven flow options')
            return

        newOptions = HashMap()
        remoteOptions = agentClient.getOptionsMap()

        remoteEnableSmartAgent = Boolean.parseBoolean(remoteOptions.get(AGENT_DRIVEN_FLOW_OPTION_ENABLE))
        localEnableSmartAgent = getSmartAgentEnabledLocally(Framework)
        localUseHttp, localProbePort = getProbePortForAgentDrivenDiscovery()
        localAdidKeyPass = getADIDKeyPass()

        logger.debug('Check agent driven flow option, enabled remotely: ', remoteEnableSmartAgent, ", enabled locally: ", localEnableSmartAgent )

        newOptions[AGENT_DRIVEN_FLOW_OPTION_ENABLE] = str(localEnableSmartAgent)
        newOptions[AGENT_DRIVEN_FLOW_OPTION_KEY_PASS] = str(localAdidKeyPass)
        if localEnableSmartAgent:
            remoteBaseUrl = None
            try:
                remoteBaseUrl = remoteOptions.get(AGENT_DRIVEN_FLOW_OPTION_PROBEBASEURL)
            except JavaException, e:
                pass
            allowUpdateProbeBaseUrl = True
            try:
                allowUpdateProbeBaseUrl = Boolean.parseBoolean(Framework.getParameter(JOB_PARAM_AGENT_DRIVEN_FLOW_ALLOW_PROBE_URL_OVERRIDE))
            except:
                logger.warn("Failed to parse boolean param % assuming true by default" % Framework.getParameter(JOB_PARAM_AGENT_DRIVEN_FLOW_ALLOW_PROBE_URL_OVERRIDE))
                
            
            if not remoteBaseUrl:
                logger.debug('Remote Agent seems to be a fresh install without ProbeBaseURL set. Updating to current probe.')
                newOptions[AGENT_DRIVEN_FLOW_OPTION_PROBEBASEURL] = buildProbeBaseUrl(localUseHttp, localProbePort)
                
            if remoteBaseUrl and allowUpdateProbeBaseUrl:
                allowedNamesForUpdateStr = Framework.getParameter(JOB_PARAM_AGENT_DRIVEN_FLOW_ALLOW_PROBE_NAMES_TO_OVERWRITE)
                if allowedNamesForUpdateStr and allowedNamesForUpdateStr.strip().lower() != 'none':
                    allowedNamesPatternList = [x.strip() for x in allowedNamesForUpdateStr.split(',') if x and x.strip()]
                    for pattern in allowedNamesPatternList:
                        if pattern.lower() == 'all':
                            logger.debug('Will update ProbeBase URL since "all" encountered URLs have to be updated according to %s parameter setting ' % JOB_PARAM_AGENT_DRIVEN_FLOW_ALLOW_PROBE_NAMES_TO_OVERWRITE)
                            newOptions[AGENT_DRIVEN_FLOW_OPTION_PROBEBASEURL] = buildProbeBaseUrl(localUseHttp, localProbePort)
                            break
                        if re.search(pattern, remoteBaseUrl):
                            logger.debug('Will update ProbeBase URL since value %s matched current ProbeBase URL %s of the agent' % (pattern, remoteBaseUrl))
                            newOptions[AGENT_DRIVEN_FLOW_OPTION_PROBEBASEURL] = buildProbeBaseUrl(localUseHttp, localProbePort)
                            break

            if not Framework.getParameter(JOB_PARAM_AGENT_DRIVEN_FLOW_CHECKINTERVALSECONDS) is None:
                newOptions[AGENT_DRIVEN_FLOW_OPTION_CHECKINTERVALSECONDS] = Framework.getParameter(JOB_PARAM_AGENT_DRIVEN_FLOW_CHECKINTERVALSECONDS)
            if not Framework.getParameter(JOB_PARAM_AGENT_DRIVEN_FLOW_ALLOWPREPOSTSCRIPT) is None:
                newOptions[AGENT_DRIVEN_FLOW_OPTION_ALLOWPREPOSTSCRIPT] = Framework.getParameter(JOB_PARAM_AGENT_DRIVEN_FLOW_ALLOWPREPOSTSCRIPT)
            
            cpuThreshold = _safeParsePercentValue(Framework.getParameter(JOB_PARAM_AGENT_DRIVEN_FLOW_CPU_THRESHOLD))
            if cpuThreshold is not None:
                newOptions[AGENT_DRIVEN_FLOW_OPTION_CPU_THRESHOLD] = str(cpuThreshold)
            memThreshold = _safeParsePercentValue(Framework.getParameter(JOB_PARAM_AGENT_DRIVEN_FLOW_MEM_THRESHOLD))
            if memThreshold is not None:
                newOptions[AGENT_DRIVEN_FLOW_OPTION_MEM_THRESHOLD] = str(memThreshold)
                
        logger.debug('Sync agent options for agent driven flow feature: ', newOptions )

        agentClient.setOptionsMap(newOptions)

    except JavaException, je:
        msg = str(je.getMessage())
        logger.info(logger.prepareFullStackTrace(msg))
...
def isAgentInstalled(Framework):
    ...
    return output and output[-5:].find('false') < 0 #only check the last false

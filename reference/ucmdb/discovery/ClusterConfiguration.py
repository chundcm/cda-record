...
class ClusterConfiguration:
    ...
    def getDistributors(self,oshv,sqlServerId):
        ...
        while rs.next():
            ...
            if(name is None):
                rs.close()
                return None
            databaseName = self.getStringResult(rs, 'distribution database')
            max = int(self.getIntResult(rs, 'max distrib retention'))
            min = int(self.getIntResult(rs, 'min distrib retention'))
            history = int(self.getIntResult(rs, 'history retention'))
            ...
            cleanup = String(self.getStringResult(rs, 'history cleanup agent'))
            idx = cleanup.indexOf('Agent history clean up:')
            if (idx >= 0):
                cleanup = cleanup.substring(idx + len("Agent history clean up:"))
    ...
    def getStringResult(self, rs, name=None):
        try:
            if name:
                return rs.getString(name)
        except:
            logger.debug('cannot get column: %s' %name)
            msg = logger.prepareFullStackTrace('')
            logger.warn(msg)

    def getIntResult(self, rs, name=None):
        try:
            if name:

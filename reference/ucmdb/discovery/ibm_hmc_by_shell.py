        ...
        if managedSystem:
            ...
            for (lparId, lpar) in lpars.items():
                if lparId and lpar:
                    ...
                    if poolAssignment.get(lparId, {}).get('curr_shared_proc_pool_id'):
                    ...
                    if lpar.lparProfile:
                        ...
                        lpar.lparProfile.onlineVirtualCpu = ibm_hmc_lib.toInteger(poolAssignment.get(lparId, {}).get('online_virtual_cpu'))
                    else:
        ...
        if buffer:
            ...
            if propertiesDict.has_key('curr_shared_proc_pool_id'):
                return {lparId: {'lparId': propertiesDict.get('curr_shared_proc_pool_id'),
                                 ...
                                 'online_virtual_cpu': propertiesDict.get('run_procs'),
                                 'curr_shared_proc_pool_id': "exist"}}
            else:
                return {lparId: {'lparId': "", 'online_virtual_cpu': propertiesDict.get('run_procs')}}
    ...
    if hmcHost and ipAddress:
        ...
        if hmcHost.hmcSoftware:
            ...
            if hmcHost.hmcSoftware.bios:
                hostOsh.setStringAttribute('bios_version', hmcHost.hmcSoftware.bios)
...
def createLparProfileOsh(lparProfile, hostOsh, pool_obj):
    ...
    if lparProfile:
        ...
        if lparProfile.lparProfile.onlineVirtualCpu is not None:
            lparProfileOsh.setFloatAttribute('online_virtual_cpu', lparProfile.lparProfile.onlineVirtualCpu)
...
def doHmcSideInformation(shell, shellOsh, ip, reportHostName, reportCpus = True, reportHostNameWhenNoIp = False):
    """
            ...
            if managedSystem.lparProfilesDict:
                for lpar in managedSystem.lparProfilesDict.values():
                    ...
                    if lpar.type not in ('aixlinux', 'os400', 'vioserver'):
                        continue
                    if lpar.lparProfile.lparIpAddress or (reportHostName and reportHostName.lower().strip() == 'true') or reportHostNameWhenNoIp:
                        #creating Lpar
#                        virtualHostOsh = modeling.createHostOSH(lpar.lparProfile.lparIpAddress, 'host', lpar.osName)
                        ...
                        if reportHostNameWhenNoIp and lpar.lparProfile.lparIpAddress is None:
                            virtualHostOsh.setStringAttribute('name', lpar.lparName)
                        if reportHostName and reportHostName.lower().strip() == 'true':
                            virtualHostOsh.setStringAttribute('name', lpar.lparName)
                        if managedSystem.genericParameters.serialNumber:
                        #if lpar.logicalSerialNumber:
                            #normalize the serial number to the one visible on machine.
                            #hmc represents the serial number as <hw serial> + <lpar_id>
                            #virtualHostOsh.setStringAttribute('serial_number', lpar.logicalSerialNumber[:len( '%s' % lpar.lparId) * (-1) ] )
                            virtualHostOsh.setStringAttribute('serial_number', managedSystem.genericParameters.serialNumber)
...
def doDiscovery(Framework, shellUtils, ip, credentialId, codepage, shellName, warningsList, errorsList):
    ...
    reportHostNameWhenNoIp =  Boolean.parseBoolean(Framework.getParameter('setHostNameIfNoIpDefined'))
    reportCpus = True
    if reportCpusStr:
        ...
        try:
            ...
            try:
                ...
                if ibm_hmc_lib.isIvm(shellUtils):
                    ...
                    return result
                elif ibm_hmc_lib.isWpar(shellUtils):
                    hostOsh = modeling.createHostOSH(ip)
                    hostOsh.setStringAttribute('discovered_os_name', 'AIX')
                    shellOsh.setContainer(hostOsh)
                    result.add(hostOsh)
                    ...
                    result.add(hypervisorOSH)
                    logger.debug('WPAR host detected!')
                    return result
                result = doHmcSideInformation(shellUtils, shellOsh, ip, reportHostName, reportCpus, reportHostNameWhenNoIp)
    ...
    if not shellUtils:
        ...
        for shellClientName in SHELL_CLIENT_PROTOCOLS:
            ...
            if shellUtils:
                ...
                break
            elif not (errorsList or warningsList):
                logger.reportWarning('No credential defined for the triggered IP.')

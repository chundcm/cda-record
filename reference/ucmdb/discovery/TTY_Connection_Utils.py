...
from cloud_shell import UnixAWSDiscoverer
...
class HostDataObject(DataObject):
    ...
    ATTRIBUTE_BIOS_VERSION = 'bios_version'
    ATTRIBUTE_BIOS_DATE = 'bios_date'
    ATTRIBUTE_OS_INSTALL_DATE = 'os_installed_date'
    ...
    def setBiosDate(self, biosDate):
        self.biosDate = biosDate

    def getOSInstallDate(self):
        return self.osInstallDate

    def setOSInstallDate(self, osInstallDate):
        self.osInstallDate = osInstallDate

    def getBiosVersion(self):
        return self.biosVersion

    def setBiosVersion(self, biosVersion):
    ...
    def build(self):
        ...
        if self.biosVersion:
            osh.setStringAttribute(HostDataObject.ATTRIBUTE_BIOS_VERSION, self.biosVersion)
        if self.biosDate:
            osh.setAttribute(HostDataObject.ATTRIBUTE_BIOS_DATE, self.biosDate)
        if self.osInstallDate:
            osh.setAttribute(HostDataObject.ATTRIBUTE_OS_INSTALL_DATE, self.osInstallDate)
...
class UnixOsDomainNameDiscoverer:
    ...
    def discover(self, hostName, ipAddressObj):
        ...
        try:
            ...
            # if all the above failed, try domainname command
            if not domainName:
                try:
...
class UnixDiscoverer:
    ...
    def get_bios_by_shell(self,command):
        result = None
        try:
            output = self.client.execAlternateCmdsList(command)
            lines = []
            if ':' in output and self.client.getLastCmdReturnCode() == 0:
                lines = output.split(':')
                result = lines[-1]
            else:
                logger.warn('Failed to obtain bios release date information')
        except:
            logger.warn('Failed to obtain bios release date information')
        return result

    def getInstallOSDateByFolder(self,command_data):
        result_str = self.client.execCmd(command_data)
        if result_str and self.client.getLastCmdReturnCode() == 0:
            result_list = result_str.split('\n')
            modified_str = result_list[-4]
            date_str = str(modified_str.split(': ')[-1])[:19]
            logger.debug("date_str:" + date_str)
            sfDate = SimpleDateFormat('yyyy-MM-dd HH:mm:ss')
            dateObj = sfDate.parse(str(date_str))
            self.hostDataObject.setOSInstallDate(dateObj)
        else:
            raise Exception

    def discoverOSInstallDate(self):
        try:
            command_data = 'rpm -qi basesystem | grep "Install Date"'
            result_str = self.client.execCmd(command_data, useCache=1)
            if ':' in result_str and self.client.getLastCmdReturnCode() == 0:
                index = result_str.find(':')
                result = result_str[index + 1:]
                result_list = result.split(' ')
                if 'PM' in result_list:
                    temp = int(result_list[5].split(':')[0]) +12
                    time = str(temp) + result_list[5][2:]
                else:
                    time = result_list[5]

                sfDate = SimpleDateFormat("dd MMM yyyy HH:mm:ss")
                date_str = result_list[2]+' '+result_list[3]+' '+result_list[4]+' '+time
                dateObj = sfDate.parse(str(date_str))
                logger.debug(str(dateObj))
                self.hostDataObject.setOSInstallDate(dateObj)
        except:
            try:
                command_data = "stat /var/log/installer/"
                self.getInstallOSDateByFolder(command_data)
            except:
                try:
                    command_data = "stat /lost+found"
                    self.getInstallOSDateByFolder(command_data)
                except:
                    logger.debug("cannot get linux OS install Date")

    def discoverBiosVersionAndDate(self):
            # get bios release date
            command_date = ["dmidecode -t 0 | grep 'Release Date'","/usr/sbin/dmidecode  -t 0 | grep 'Release Date'"]
            bios_date = self.get_bios_by_shell(command_date)
            sfDate = SimpleDateFormat('MM/dd/yyyy')
            if bios_date:
                dateObj = sfDate.parse(bios_date, ParsePosition(0))
                self.hostDataObject.setBiosDate(dateObj)
            # get bios version
            command_version = ["dmidecode -t 0 | grep 'Version'","/usr/sbin/dmidecode  -t 0 | grep 'Version'"]
            bios_version = self.get_bios_by_shell(command_version)
            if bios_version:
                self.hostDataObject.setBiosVersion(str(bios_version).strip())
            else:
                try:
                    #it belongs to mac
                    output_os = self.client.execCmd('uname')
                    if 'Darwin' in output_os and self.client.getLastCmdReturnCode() == 0:
                        command_line = 'system_profiler SPHardwareDataType'
                        output_bios_version_raw = self.client.execCmd(command_line)
                        output_bios_version_process = None
                        for index in output_bios_version_raw.split('\n'):
                            if 'Boot ROM Version' in index:
                                output_bios_version_process = (index.split(':')[1]).strip()
                        if output_bios_version_process:
                            self.hostDataObject.setBiosVersion(str(output_bios_version_process).strip())
                            command_line = 'ioreg -c AppleSMBIOS -w 0 | grep  "SMBIOS"'
                            output_bios_date_raw = self.client.execCmd(command_line).split('\n')[4]
                            # output_bios_date_reg = output_bios_version_process + '\d{2}\\\d{2}\\\d{4}'
                            output_bios_date_reg = '\d{2}[/]\d{2}[/]\d{4}'
                            index_start = output_bios_date_raw.find('<') + 1
                            index_end = output_bios_date_raw.find('>')
                            logger.debug("line1:"+output_bios_date_raw[index_start:index_end])
                            line = (output_bios_date_raw[index_start:index_end]).decode('hex')
                            matcher = re.search(output_bios_date_reg, line, re.I)
                            if matcher:
                                sfDate_Time = SimpleDateFormat('MM/dd/yyyy')
                                dateObj = sfDate_Time.parse(str(matcher.group(0)), ParsePosition(0))
                                self.hostDataObject.setBiosDate(dateObj)

                except Exception,e:
                    logger.error(str(e))
                    logger.debug('error occured during getting bios version and date in MAC OS')
    ...
    def getSerialNumberByDmidecode(self):
        ...
        '''
        dmiDecodeCommand = 'dmidecode -t system'
        dmiAltDecodeCommand = '/usr/sbin/dmidecode -t system'
        output = self.client.execAlternateCmdsList([dmiDecodeCommand, dmiAltDecodeCommand], timeout=90000)
        if output:
            m = re.search('(System Information.*[\r\n]*(?:.+[\r\n]+){0,6})', output)
            if m:
                sn = re.search('\s*Serial Number:\s*(.*)', m.group(1), re.IGNORECASE)
                if sn:
                    return sn.group(1).strip()
        raise Exception("Command execution failed")

    def discoverSerialNumberLshw(self):
        sn = None
        try:
            sn = self.getSerialNumberLshw()
            if sn and sn != 'None' and sn != 'Not Specified':
                self.hostDataObject.setSerialNumber(sn)
                return sn
        except Exception:
            logger.warn("Failed getting host serial number by lshw")

    def getSerialNumberLshw(self):
        lshwCommand = 'lshw -C system'
        output = self.client.execCmd(lshwCommand, timeout=90000)
        if output and self.client.getLastCmdReturnCode() == 0:
            found_sn = re.search("serial:\s*(.*)", output, re.IGNORECASE)
            if found_sn:
                return found_sn.group(1).strip()
        raise Exception("Command execution failed")
    ...
    def isExceptionalCase(self):
            ...
            if shell.getLastCmdReturnCode() == 0:
                ...
                if output:
                    if re.search(r"Authorization failed", output, re.I) or re.search(r"insufficient privileges", output, re.I):
                        raise ValueError("Not enoguh permissions %s" % command)
        ...
        for ldmPath in LDM_PATHS:
            ver_cmd = '/'.join([ldmPath, LDM_NAME]) + " -V"
            try:
                ldm = getCommandOutput(ver_cmd, self.client)
                return True
    ...
    def addResultsToVector(self, vector, ttyObj, nat_ip = None):
        ...
        if self.hostDataObject.getIpsCount() > 0:
            ...
            # add single IP to vector with virtual attribute set
                ...
                # Do not create
                if not self.hostDataObject.isIpNATed(nat_ip) and not self.isExceptionalCase():
...
class JuniperDiscoverer(NexusDiscoverer):
    DISCOVERED_VENDOR = 'Juniper'
    VENDOR = 'Juniper'
    def __init__(self, framework, client, langBund, osName, uName, machineName):
        UnixDiscoverer.__init__(self, framework, client, langBund, osName,
                                uName, machineName)
    ...
    def makeDataObject(self, osName, uName, machineName, hostIpObj, domain):
        self.hostDataObject = JunHostDataObject(osName, uName, machineName,
                                                hostIpObj, domain, self.host_cmdbid,
                                                self.host_key, self.host_macs)

    def discover(self):
        self.hostDataObject.setDiscoveredOsVendor(JuniperDiscoverer.DISCOVERED_VENDOR)
        self.hostDataObject.setDiscoveredVendor(JuniperDiscoverer.DISCOVERED_VENDOR)
        self.hostDataObject.setOsVendor(JuniperDiscoverer.VENDOR)
        self.hostDataObject.setGenericVendor(JuniperDiscoverer.VENDOR)


class F5LbTmshDiscoverer(NexusDiscoverer):
    DISCOVERED_VENDOR = 'F5'
    VENDOR = 'F5'
    def __init__(self, framework, client, langBund, osName, uName, machineName):
        UnixDiscoverer.__init__(self, framework, client, langBund, osName,
                                uName, machineName)
    ...
    def makeDataObject(self, osName, uName, machineName, hostIpObj, domain):
        self.hostDataObject = F5LbHostDataObject(osName, uName, machineName,
                                             hostIpObj, domain, self.host_cmdbid,
                                             self.host_key, self.host_macs)
    
    def _parseVersion(self, output):
        if output:
            reStr = 's*BIG-IP\s+Version\s+([\d\.\-]+)\s+([\d\.\-]+)|\s*Version\s+([\d\.\-]+)'
            for line in output.strip().split('\n'):
                matcher = re.search(reStr, line)
                if matcher:
                    logger.debug('BIG IP version is %s' % matcher.group(1))
                    return matcher.group(1)
    
    def _parseManagementIps(self, output):
        results = []
        if output:
            matchers = re.findall('sys\s+management\-ip\s+([\d\.]/(\d+))', output)
            for matcher in matchers:
                ip = ip_addr.IPAddress( matcher[0] )
                mask = matcher[1]
                netmask = None
                if ip.get_version() == 4:
                    length = int(mask)
    ...
    def discoverManagementIPs(self):
        output = self.getCommandOutput('list /sys management-ip')
        ips = self._parseManagementIps(output)
        for ip, netmask in ips: 
            self.hostDataObject.addIp(ip, netmask)
    
    def discoverVersion(self):
        output = self.getCommandOutput('show sys version')
        osVersion = self._parseVersion(output)
        if osVersion:
            self.hostDataObject.setOsVersion(osVersion)
    
    def _parseSerialNumber(self, output):
        '''  Host Board Serial          pca0226z005d'''
        if not output:
            return
        m = re.search('Host\s+Board\s+Serial\s+([\w\.\-]+)', output)
        return m and m.group(1)
    
    def _parseMemoryData(self, output):
        '''Host memory size       49152'''
        if not output:
            return
        m = re.search('Host\s+memory\s+size\s+(\d+)', output)
        return m and long(m.group(1)) * 1024
    
    def discoverMachineInformation(self):
        output = self.getCommandOutput('show sys hardware')

        serialNumber = self._parseSerialNumber(output)
        if serialNumber:
            self.hostDataObject.setSerialNumber(serialNumber)
        
        memory = self._parseMemoryData(output)
        if memory:
            self.hostDataObject.setMemorySize(memory)
    
    def discover(self):
        self.hostDataObject.setDiscoveredOsVendor(F5LbTmshDiscoverer.DISCOVERED_VENDOR)
        self.hostDataObject.setDiscoveredVendor(F5LbTmshDiscoverer.DISCOVERED_VENDOR)
        self.hostDataObject.setOsVendor(F5LbTmshDiscoverer.VENDOR)
        self.hostDataObject.setGenericVendor(F5LbTmshDiscoverer.VENDOR)
        self.hostDataObject.setModel('F5')
        
        self.discoverVersion()
        self.discoverManagementIPs()
        self.discoverMachineInformation()
...
class FreeBsdDiscoverer(UnixDiscoverer):
    ...
    def discover(self):
        ...
        self.discoverBiosVersionAndDate()
        self.discoverOSInstallDate()

    def discoverBiosUUID(self):
        output = self.client.execAlternateCmdsList(['dmidecode -t system ','/usr/sbin/dmidecode -t system '],timeout=90000)
        if output:
    ...
    def _parseIfConfig(self, output):
        interfacesWithIps = []
        patternToParseInterfaces = ((self.ifaceNameMacPattern, self._handleInterfacePattern),)
        patternToParseIps = ((self.ipv4Pattern, self._handleIpPattern),
                                  (self.ipv6Pattern, self._handleIpPattern))

        for interfaceBuffer in self._splitOutputPerInterface(output):
            for pattern, parseFunc in patternToParseInterfaces:
                try:
                    ifaceDo = self._parseInterface(interfaceBuffer, pattern, parseFunc)
                    if not ifaceDo:
                        continue
                    for ipPattern, ipParseFunc in patternToParseIps:
...
class F5LbHostDataObject(HostDataObject):
    HOST_CLASS = 'lb'

class JunHostDataObject(HostDataObject):
    HOST_CLASS = 'node'
...
class AixDiscoverer(UnixDiscoverer):
    ...
    def discoverBiosVersionAndDate(self):
        bios_output = self.client.execCmd('prtconf | grep \"Firmware Version\"')
        if bios_output and self.client.getLastCmdReturnCode() == 0:
            bios_version = bios_output.split(':')
            self.hostDataObject.setBiosVersion(bios_version[1].strip())
    ...
    def discoverOsVersionWithMaintenanceLevel(self):
        ...
        if self.client.getLastCmdReturnCode() == 0:
            ...
            if arr:
                os_ver = arr[0]
                if len(os_ver) == 4:
                    os_version = '%s.%s.%s.%s' % (os_ver[0], os_ver[1], os_ver[2], os_ver[3])
                else:
                    logger.debug('Incorrect OS version: %s' % os_ver)
                ...
                if len(arr) >= 2:
                    if arr[1].isdigit():
                        ...
                        os_release = arr[1].strip()
                    for j in range(2, len(arr)):
                        os_release = os_release + '.' + arr[j].strip()
            ...
            if self.client.getLastCmdReturnCode() == 0:
                ...
                if arr:
                    ...
                    if len(os_ver) == 4:
                        ...
                        logger.debug('Incorrect OS version: %s' % os_ver)
                    if len(arr) >= 2:
                        m = re.search('0*([0-9]{1,2})', arr[1])
                        if m != None:
                            os_release = m.group(1)
                        versionDiscovered = True
        if not versionDiscovered:
    ...
    def discoverOsArchitecture(self):
        osArchOut = self.getCommandsOutput(['getconf KERNEL_BITMODE', '/usr/bin/getconf KERNEL_BITMODE', '/usr/sbin/getconf KERNEL_BITMODE'])
        if self.client.getLastCmdReturnCode() == 0:
            if re.search('64', osArchOut) :
                platform = self.getCommandsOutput(['getconf MACHINE_ARCHITECTURE', '/usr/bin/getconf MACHINE_ARCHITECTURE', '/usr/sbin/getconf MACHINE_ARCHITECTURE'])
                if self.client.getLastCmdReturnCode() == 0:
                    if str(platform).strip() == "chrp":
                        self.hostDataObject.setOsArchitecture('ppc64')
                    else:
                        self.hostDataObject.setOsArchitecture('64-bit')
                else:
                    logger.debug("Failed to get machine architecture")
            else:
                self.hostDataObject.setOsArchitecture('32-bit')
        else:
            logger.debug('Failed to get OS architecture')
...
class LinuxDiscoverer(UnixDiscoverer):
    ...
    XENSERVER_OS_NAME = "XenServer"
    XENSERVER_OS_VENDER = "Citrix"

    def __init__(self, framework, client, langBund, osName, uName, machineName):
        self._isEsx = 0
        self._isXenServer = 0
        self._lsbNotExist = False
        UnixDiscoverer.__init__(self, framework, client, langBund, osName,
                                uName, machineName)
    ...
    def discover(self):
        ...
        if linux_networking:
            ...
            self.discoverHostKey()
            self.discoverMachineBootDate()
            self.discoverSerialNumberByDmiDecode() or self.discoverSerialNumberLshw() or self.discoverSerialNumberLshal()
    ...
    def checkDistribution(self):
        output = self.client.execCmd('lsb_release -a')
        pattern = re.compile(ur'Release:(.*)')
        if output and self.client.getLastCmdReturnCode() == 0:
            if pattern.search(output):
                self.hostDataObject.setOsVersion(pattern.search(output).group(1).strip())
            self.setDistribution(output)
        else:
            self._lsbNotExist = True

    @staticmethod
    def mappingLinuxDistribution(strDistribution):
        if re.search('Ubuntu', strDistribution):
            return 'Ubuntu(Linux)'
        elif re.search('Red Hat', strDistribution):
            return 'Red Hat(Linux)'
        elif re.search('Debian', strDistribution):
            return 'Debian(Linux)'
        elif re.search('SUSE', strDistribution):
            return 'SuSE(Linux)'
        return 'Linux'

    def setDistribution(self, output):
        mappingResult = self.mappingLinuxDistribution(output)
        self.hostDataObject.osName = mappingResult
        self.hostDataObject.osDescription = mappingResult
        if re.search('Ubuntu', output):
            self.hostDataObject.setDiscoveredOsVendor('Canonical Ltd')

    def discoverOsArchitecture(self):
        output = self.getCommandsOutput(['uname -a', '/usr/bin/uname -a', '/usr/sbin/uname -a'])
        ...
        elif output and re.search('ppc64', output):
            self.hostDataObject.setOsArchitecture('ppc64')
        else:
    ...
    def discoverInterfaceSpeedViaDmesg(self, interfaceName):
        if interfaceName:
            output = self.getCommandsOutput(['dmesg | grep %s | grep -i Up' % interfaceName])
    ...
    def checkESXExecutable(self):
        ...
        if output:
            ...
            if index == -1:
                index = output.find('VMware Server')
                if index == -1:
                    return 1
    ...
    def checkF5BasePackage(self):
        ...
        if output and re.search('\s*f5base\-', output):
            return 1
    ...
    def discoverOsFlavor(self):
        for (fileName, regex) in LinuxDiscoverer.OS_FLAVOR_FILE_TO_REGEX:
            ...
            if output and output.strip() and output.strip().split('\n'):
                ...
                if flavor:
                    osInstallationType = flavor.group(1)
                    osInstallationType = string.replace(osInstallationType, 'Linux ', '')
                    self.hostDataObject.setOsFlavor(osInstallationType)
                    osRelease = flavor.group(2)
                    if osInstallationType.find('XenServer')>-1:
                        self._isXenServer = 1
                        self.hostDataObject.setDiscoveredOsVendor( LinuxDiscoverer.XENSERVER_OS_VENDER )
                        self.hostDataObject.setOsName( LinuxDiscoverer.XENSERVER_OS_NAME )
                        osXenRelease = re.search("([0-9\.]+)", osRelease)
                        if osXenRelease:
                            osRelease = osXenRelease.group(0)
                        self.hostDataObject.setOsVersion(osRelease) #same number in release.
                    if "release" not in osRelease:
                        osRelease = "release " + osRelease
                    self.hostDataObject.setOsRelease(osRelease)
                    if self._lsbNotExist:
                        self.setDistribution(output)
                        osVersion = re.search("(\d+\.?\d+)", flavor.group(2))
                        if self._isXenServer:
                            osVersion = re.search("([0-9\.]+)", flavor.group(2))
                        if osVersion:
                            self.hostDataObject.setOsVersion(osVersion.group(0))
                    #  QCCR1H117891 Flippling of attributes when running DDMA and DDMI discovery - discoveredOsVendor
                    osVender = flavor.group(0)
                    if osVender.find('Red Hat')>-1:
                        self.hostDataObject.setDiscoveredOsVendor('Red Hat')
                    if osVender.find('SUSE') >-1 or osVender.find('Novell')>-1:
                        self.hostDataObject.setDiscoveredOsVendor('Micro Focus')
                    if osVender.find('Oracle Linux')>-1:
                        self.hostDataObject.setDiscoveredOsVendor('Oracle')
                    if osVender.find('CentOS')>-1:
                        self.hostDataObject.setDiscoveredOsVendor('CentOS')
                else:
                    self.hostDataObject.setOsFlavor(output)
                break

    def discoverHostModel(self):
        output = self.client.execAlternateCmdsList(['dmidecode -t system',
                                               '/usr/sbin/dmidecode -t system'],timeout=90000)
        if output:
    ...
    def discoverManufacturer(self):
        succeeded = 1
        try:
            res = self.client.execAlternateCmdsList(['dmidecode -t system','/usr/sbin/dmidecode -t system'],timeout=90000)
            if res:
...
class HpNonStopDiscoverer(UnixDiscoverer):
    def makeDataObject(self, osName, uName, machineName, hostIp, domain):
        ...
        self.hostDataObject = _HpNonStopHostDataObject(osName, uName,
                                ...
                                self.host_key, self.host_macs)
        self.hostDataObject.setVendor('Hewlett-Packard')
        self.hostDataObject.setManufacturer('Hewlett-Packard')
...
class HpuxDiscoverer(UnixDiscoverer):
    ...
    def discover(self):
        ...
        self.discoverOsFlavor()
        self.discoverOsArchitecture()
        if not self.discoverBiosVersionByMachinfo():
            if not self.discoverBiosVersionByPdcinfo():
                if not self.discoverBiosVersionByCstm():
                    logger.warn('Could not discover bios version')
    ...
    def discoverOsArchitecture(self):
        ...
        if output and re.search('x86_64', output):
            ...
            output = self.getCommandsOutput(['getconf KERNEL_BITS', '/usr/bin/getconf KERNEL_BITS', '/usr/sbin/getconf KERNEL_BITS'])
            if output and re.search('64', output):
                self.hostDataObject.setOsArchitecture('64-bit')
            else:
                logger.warn('Failed detecting OS Architecture')
    ...
    def discoverBiosVersionByMachinfo(self):
        # machinfo |grep Firmware revision
        # Firmware revision:  46.34
        output = self.client.execAlternateCmds("machinfo |grep 'Firmware revision'", "/usr/contrib/bin/machinfo |grep 'Firmware revision'")
        if output:
            matcher = re.match('\s*Firmware revision:\s*([\w+\-\.]+)', output)
            if matcher:
                logger.debug("bios version:", matcher.group(1).strip())
                self.hostDataObject.setBiosVersion(matcher.group(1).strip())
                return 1
        else:
            logger.warn("Failed getting bios version by machinfo")

    def discoverBiosVersionByPdcinfo(self):
        # /usr/sbin/diag/contrib/pdcinfo -no_banner | grep 'PDC Version'
        # -----------------  Processor 0 HPMC Information - PDC Version: 46.34  ------
        output = self.getCommandsOutput(["/usr/sbin/diag/contrib/pdcinfo -no_banner | grep 'PDC Version'"])
        if output:
            matcher = re.search('\s*PDC Version:\s*([\w+\-\.]+)', output)
            if matcher:
                logger.debug("bios version", matcher.group(1).strip())
                self.hostDataObject.setBiosVersion(matcher.group(1).strip())
                return 1
        else:
            logger.warn("Failed getting host bios verion by pdcinfo")

    def discoverBiosVersionByCstm(self):
        # echo "selclass qualifier cpu;info;wait;il"| cstm | grep "Firmware Revision"
        #   PDC Firmware Revision:     46.34        IODC Revision:            0
        # echo "selclass qualifier cpu;info;wait;il"|/usr/sbin/cstm | grep "Firmware Revision"
        #   PDC Firmware Revision:     46.34        IODC Revision:            0
        output = self.client.execAlternateCmds('echo "selclass qualifier cpu;info;wait;il"| cstm | grep "Firmware Revision"', 'echo "selclass qualifier cpu;info;wait;il"|/usr/sbin/cstm | grep "Firmware Revision"')
        if output and (self.client.getLastCmdReturnCode() == 0):
            matcher = re.search('PDC Firmware Revision: \s*([\w+\-\.]+)', output)
            if matcher:
                logger.debug("bios version", matcher.group(1).strip())
                self.hostDataObject.setBiosVersion(matcher.group(1).strip())
                return 1
        else:
            logger.warn("Failed getting host bios version by cstm")
...
class SunDiscoverer(UnixDiscoverer):
    def __init__(self, framework, client, langBund, osName, uName, machineName):
        ...
        self.hostDataObject.setVendor('Oracle')
    ...
    def discover(self):
        ...
        serialNumber = (fptools.safeFunc(self.getSerialNumberBySneep)()
              ...
              or fptools.safeFunc(self.getSerialNumberFromPrtPicl)()
    ...
    def discoverBiosVersionAndDate(self):
        #sparc
        if str(self.client.execCmd('ls /dev/smbios')).upper().find("NO SUCH FILE") > -1:
            outputList = self.client.execCmd('/usr/sbin/prtconf -V').split(' ')
            if len(outputList) > 3:
                bios_version = outputList[1]
                self.hostDataObject.setBiosVersion(bios_version)
                bios_date = outputList[2]
                dateObj = SimpleDateFormat('yyyy/MM/dd').parse(bios_date, ParsePosition(0))
                self.hostDataObject.setBiosDate(dateObj)
            else:
                logger.warn("Failed getting bios version and date by /usr/sbin/prtconf -V")
        #smbios
        else:
            bios_version = self.client.execCmd('smbios |grep \'Version String\'').split(':')[1].strip()
            if bios_version:
                self.hostDataObject.setBiosVersion(bios_version)
                bios_date = self.client.execCmd('smbios |grep \'Release Date\'').split(':')[1].strip()
                if bios_date:
                    dateObj = SimpleDateFormat('MM/dd/yyyy').parse(bios_date, ParsePosition(0))
                    self.hostDataObject.setBiosDate(dateObj)
                else:
                    logger.warn("Failed getting bios date by smbios")
            else:
                logger.warn("Failed getting bios version by smbios")
    ...
    def getSerialNumberBySneep(self):
        ...
        if output and re.search('unknown|couldn\'t|bad string', output, re.I):
            output = ''
    ...
    def getSerialNumberFromPrtPicl(self):
        r'''Get Serial Nuber by executing prtpicl command
        @command: prtpicl
        @types: -> str
        @raise Exception: Command execution failed
        '''
        output = self.getCommandsOutput(['/usr/sbin/prtpicl -v -c chassis | grep SerialNumber'])
        if output:
            foundSerialNumber = re.search('SerialNumber\s+(.*)$', output, re.IGNORECASE)
            if foundSerialNumber:
                return foundSerialNumber.group(1).strip()
        raise Exception("Command execution failed")
    ...
    def discoverManufacturer(self):
        ...
        try:
            res = self.client.execAlternateCmds(
                    ...
                    'prtpicl -v -c chassis | grep MfgName',
                    '/usr/sbin/prtpicl -v -c chassis | grep MfgName')
            if (not res or (self.client.getLastCmdReturnCode() != 0)):
                raise Exception()
            m = re.search(':\s+(.+)', res)
            if not m:
                m = re.search('MfgName\s+(.*)$', res, re.IGNORECASE)
            if m:
                manufacturer = m.group(1).strip()
                self.hostDataObject.setManufacturer(manufacturer)
        except:
    ...
    def discoverOsArchitecture(self):
        #output from 'isainfo -b', should be 64 or 32
        archBuf = self.getCommandsOutput(['isainfo -b',
                                                '/usr/bin/isainfo -b'])
        if archBuf:
            if re.search('64', archBuf) :
                self.hostDataObject.setOsArchitecture('64-bit')
            else:
                self.hostDataObject.setOsArchitecture('32-bit')
        else:
            logger.debug('Failed to discover os architecture')
    ...
    def discoverOsVersionAndRelease(self):
        ...
        if versionBuf:
            ...
            osVer = re.search("(\d+)\.(\d+)", versionBuf)
            if osVer and (osVer.group(1) == '5'):
                # Solaris is sold under a different version number:
                # SunOS 5.10 -> Solaris 10
                self.hostDataObject.setOsVersion(osVer.group(2))
            else:
...
class MacOsDiscoverer(FreeBsdDiscoverer):
    ...
    def discoverOsArchitecture(self):
        output = self.getCommandsOutput(['uname -m', '/usr/bin/uname -m', '/usr/sbin/uname -m'])
        if output and re.search('x86_64', output):
            self.hostDataObject.setOsArchitecture('64-bit')
        elif output and re.search('i686|i386', output):
            self.hostDataObject.setOsArchitecture('32-bit')
...
class DiscovererFactory:
    ...
    def __init__(self, langBundle):
        ...
        self.osNameToClass = {
            ...
            'IOS': NexusDiscoverer,
            'TMOS':F5LbTmshDiscoverer,
            'JUNOS': NexusDiscoverer,
        }
...
def getOSandStuff(shell, shellOsh, Framework, langBund, uduid=None, nat_ip=None, ip=None, doDnsHostNameMethod=None):
    ...
    # Add DNS info
    try:
        if doDnsHostNameMethod:
            doDnsHostNameMethod(Framework, ip, shell, hostOsh, OSHVResult)
    except:
        msg = logger.prepareFullStackTrace('')
        logger.debugException(msg)

    enableAWS = Framework.getParameter('enableAWSDiscovery')
    # will enable AWS discovery by default
    if enableAWS == None or enableAWS.lower().strip() == 'true':
        try:
            aws_discoverer = UnixAWSDiscoverer(shell)
            if aws_discoverer.is_applicable():
                instance_id = aws_discoverer.discoverInstanceId()
                if instance_id:
                    hostOsh.setAttribute("cloud_instance_id", aws_discoverer.discoverInstanceId())
        except:
            msg = logger.prepareFullStackTrace('')
            logger.debugException(msg)

...
def createHostAndIPOSHV(ip):
    """
    Creates an incomplete node and an IP connected with a C{containment} link.
    ...
    link = createLinkOSH('containment', hostOSH, ipOSH)
...
def createDatabaseOSH(dbType, dbName, dbPort, ip, hostOSH, credentialsID=None,
    ...
    if databaseName:
        databaseOSH.setAttribute('database_dbsid', databaseName.strip())
        databaseOSH.setStringAttribute('name', databaseName.strip())
    ...
    if applicationVersionNumber != None and applicationVersionNumber != 'NA':
...
__J2EE_SERVERS = {
    ...
    'weblogic': __J2eeServerDefinition('weblogicas', 'WebLogic AS', 'oracle_corp'),

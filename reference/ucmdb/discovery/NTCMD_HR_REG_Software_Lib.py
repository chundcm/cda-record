...
def processSoftware(keys, buffer, hostOSH, OSHVResults, softNameToInstSoftOSH = None, filterlist=None):
    '''
    ...
    for key in keys:
        ...
        m = re.search('\n\s*Comments\s+REG_SZ\s+?([^\n]+)', key)
        if (m):
            softwareComment = m.group(1).strip()
        ...
        if softwareName:
            if ((softwareName in swList) == 0) :
                ...
                if softwareVendor == 'Microsoft Corporation' :
                    softwareVendor = 'Microsoft'

                softwareOSH = hostresource.makeSoftwareOSH(softwareName, softwarePath, softwareVer, hostOSH, softwareInstallDate, softwareProductId, softwareProductCode, softwareVendor, softwareComment)
...
def filterinstalledsoftware(filterlist, softwareName):
    if filterlist:
        for filtername in filterlist:
            if re.match(filtername, softwareName):
                logger.warn('Ignoring filter software with filter condition %s' % softwareName)
                return None
    return softwareName
...
def executeSoftwareQueryByPath(client, reg_path, prefix=''):
    ...
    if (reg_mamRc != 0) or (buffer.find(ntcmdErrStr) != -1):
        if (reg_mamRc == 1) and (buffer.find('ERROR: More data is available.') != -1):
            ...
            remoteFile = client.copyFileIfNeeded(localFile, True)
            if not remoteFile:
...
def doSoftware(shell, hostOsh, vector, softNameToInstSoftOSH = None, filterlist=None):
    ...
    return processSoftware(keys, buffer, hostOsh, vector, softNameToInstSoftOSH, filterlist)

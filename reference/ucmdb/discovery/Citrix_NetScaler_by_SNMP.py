...
OID_TABLE_VSERVERCSPOLICY = '1.3.6.1.4.1.5951.4.1.3.3.1'
...
class VServer(object):
    def __init__(self, name, ip=None, port=None, server_type=None):
...
class NetScalerDiscover(object):
    ...
    def getTopology(self):
        self.lbOsh, ipOsh, linkOsh = self.discoveryLB()
        ...
        # link vserver if exist
        v_server_link_map = self.getVServersLink()
        logger.debug("vserver_link_map", v_server_link_map)
        ...
        _framework = ScriptsExecutionManager.getFramework()
        destIp = _framework.getDestinationAttribute('ip_address')
        dstIpOsh = modeling.createIpOSH(destIp)
        linkOsh = modeling.createLinkOSH('containment', self.lbOsh, dstIpOsh)
        ...
        for v_server in v_server_map.values():
            ...
            v_server_dict[vServerBuilder.vServer.name] = vServerBuilder.osh

            for service in v_server.services:
                serviceBuilder = ServiceBuilder(service, lbc_osh)
                vector.addAll(serviceBuilder.build())
        for key,value in v_server_link_map.items():
            if v_server_dict.has_key(key) and v_server_dict.has_key(value):
                dependency_link = modeling.createLinkOSH("dependency", v_server_dict[key], v_server_dict[value])
                vector.add(dependency_link)
    ...
    def discoveryLB(self):
        ...
        if data:
            ...
            netscalerOsh = modeling.createCompleteHostOSH('lb', netscaler.ip)
            ipOsh = modeling.createIpOSH(netscaler.ip)
            linkOsh = modeling.createLinkOSH('containment', netscalerOsh, ipOsh)
            ...
            return netscalerOsh, ipOsh, linkOsh
        else:
    ...
    def getVServers(self):
        ...
        queryBuilder.addQueryElement(59, 'vsvrFullName')
        vservers = self.snmpAgent.getSnmpData(queryBuilder)
        v_server_map = {}
        for row in vservers:
            # in case ip 0.0.0.0 and port 0, it should link to content switching virtual server
            if row.vsvrIpAddress == '0.0.0.0' and row.vsvrPort == '0':
                if len(row.vsvrName)>=31:
                    v_server_map[row.meta_data] = VServer(row.vsvrFullName, server_type=serviceTypeToName(row.vsvrType))
                else:
                    v_server_map[row.meta_data] = VServer(row.vsvrName, server_type=serviceTypeToName(row.vsvrType))
            if isValidIP(row.vsvrIpAddress):
                if len(row.vsvrName)>=31:
                    v_server_map[row.meta_data] = VServer(row.vsvrFullName, row.vsvrIpAddress, row.vsvrPort,
    ...
    def getVServersLink(self):
        queryBuilder = snmputils.SnmpQueryBuilder(OID_TABLE_VSERVERCSPOLICY)
        queryBuilder.addQueryElement(2, 'cspolicyDestVserverName')
        queryBuilder.addQueryElement(6, 'csIndexVserverFullName')
        links = self.snmpAgent.getSnmpData(queryBuilder)
        vlink = {}
        for row in links:
            vlink[row.csIndexVserverFullName] = row.cspolicyDestVserverName
        return vlink
...
class VServerBuilder(Builder):
    ...
    def build(self):
        ...
        if self.vServer.ip and self.vServer.port:
        ...
        return self.newVector(vServerOsh, ownership_link)
...
class ServiceBuilder(Builder):
    ...
    def build(self):
        ...
        backend_server_osh = modeling.createHostOSH(self.service.ip)

...
class TopologyReporter:
    ...
    def reportWebDir(self, web_app, parent_osh, app_pools_map=None, webservice_ext_filter = [], bindings= None, uri_endpoints_list= None):
        """
        ...
        name = osh.getAttributeValue('data_name')
        if name.lower() == 'root':
            osh = parent_osh
        else:
            osh.setContainer(parent_osh)
        ...
        if web_app.config_files:
            ...
            oshs.extend(self.reportWebServices(web_app.config_files, osh, webservice_ext_filter, bindings, uri_endpoints_list))
    ...
    def reportTopology(self, app_pools_list, web_sites_list, parentOsh, iis_version=None, webservice_ext_filter=[], uri_endpoints_list=None):
        """
        ...
        for site in web_sites_list:
            ...
            if not site.is_ftp():
                ...
                report_web_application = partial(self.reportWebDir, parent_osh=root_osh, app_pools_map=apppool_map, webservice_ext_filter=webservice_ext_filter,
                                                 bindings = site.bindings, uri_endpoints_list = uri_endpoints_list)
    ...
    def reportWebServices(self, config_files, parent_osh, webservice_ext_filter = [], bindings= None, uri_endpoints_list= None):
        oshs = []
        for config_file in config_files:
            osh, name = self._builder.buildAppWebService(config_file, webservice_ext_filter)
            if osh and name:
                osh.setContainer(parent_osh)
                oshs.append(osh)
                oshs.append(modeling.createLinkOSH("dependency",osh, parent_osh))

                if uri_endpoints_list:
                    endpoint = bindings[0][2][0]
                    type = bindings[0][1]
                    ipaddress = endpoint.getAddress()
                    port = endpoint.getPort()
                    for uriEndpoints in uri_endpoints_list:
                        try:
                            if name:
                                uriEndpoint = uriEndpoints[name]
                                uriEndpoint = type+'://'+ipaddress+':'+str(port)+'/'+uriEndpoint+'?WSDL'
                                urlOSH = modeling.createUrlOsh(parent_osh, uriEndpoint, 'wsdl')
                                ipOSH = ipaddress and modeling.createIpOSH(ipaddress)
                                oshs.append(urlOSH)
                                oshs.append(modeling.createLinkOSH("dependency", osh, urlOSH))
                                if ipOSH:
                                    oshs.append(ipOSH)
                                    urlToIpOSH = modeling.createLinkOSH('depend', urlOSH, ipOSH)
                                    oshs.append(urlToIpOSH)
                        except KeyError, ex:
                            logger.info(ex)
                            logger.info('no related URI on Web service: %s' %name)
        return oshs

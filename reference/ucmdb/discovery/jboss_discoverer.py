...
class VersionLayout(jee_discoverer.Layout):
    ...
    def getServerLogPath(self):
        '''@types: -> str'''
        return self.path().join(self.getJbossHomePath(), 'standalone', 'log', 'server.log')
    ...
    def getModuleXmlPathForNewVersion(self):
        '''@types: -> str'''
        return self.path().join(self.getJbossHomePath(), 'modules', 'system', 'layers', 'base', 'org', 'jboss', 'as',
                                'server', 'main',
                                'module.xml')
...
class StandaloneModeLayout(ServerLayoutV7):
    ...
    def __init__(self, fs, installDirPath, customStanaloneXmlPath = None, serverBaseDir = None, serverConfigDir = None):
        ServerLayoutV7.__init__(self, fs, installDirPath)
        path = self.path()
        if customStanaloneXmlPath:
            if serverBaseDir and not serverConfigDir:
                serverConfigDir = path.join(serverBaseDir, 'configuration')
            if not serverConfigDir:
                self.__configPath = path.isAbsolute(customStanaloneXmlPath) and customStanaloneXmlPath or path.join(self.getInstallDirPath(), 'standalone', 'configuration', customStanaloneXmlPath)
            else:
                self.__configPath = (path.isAbsolute(customStanaloneXmlPath) and customStanaloneXmlPath or path.join(serverConfigDir, customStanaloneXmlPath))
...
class ServerRuntime(jee_discoverer.ServerRuntime):
    ...
    def findServerBaseDirPath(self):
        return self._getCommandLineDescriptor().extractProperty(r'jboss.server.base.dir')

    def findServerConfigDirPath(self):
        return self._getCommandLineDescriptor().extractProperty(r'jboss.server.config.dir')
...
class VersionInfoDiscovererByShell(jee_discoverer.DiscovererByShell):
    ...
    def _discoverVersionFromLog(self, path, parser):
        try:
            return parser(self._getShell().safecat(path))
        except (Exception, JException):
            logger.debug('Failed to discover JBoss version from log: %s' % path)
    ...
    def discoverVersion(self):
        ''' JBoss full version (like 4.0.0 or 4.2.3.GA, at least 3 digits) can be discovered:
            ...
            - by <jbossHomeDir>/bin/standalone.bat --v (suitable for 8.2, 9.0, 10.0, 10.1) '''
        return (# get version from licenses.xml
                ...
                or self._discoverVersionFromXml(self.getLayout().getModuleXmlPathForNewVersion(), self.__parseVersionInModuleXml)
                ...
                # get version from server.log
                or self._discoverVersionFromLog(self.getLayout().getServerLogPath(), self.__parseVersionInServerLog))
    ...
    def __parseVersionInModuleXml(self, content):
        ''' Parse JBoss version from jboss-as-server-7.x.x.Final.jar or wildfly-server-x.x.x.Final.jar version in module.xml
        ...
        if not m:
            m = re.search('wildfly-server-(.*)\.jar', content)
            if m:
                logger.debug('wildfly-server-x.x.x.jar found : ', m.group(1))
                matchObj = re.match('.*?(\d+)(?:\.(\d+))?.*?', m.group(1))
                major, minor = matchObj.groups()
                if int(major) <> 8:
                    major = int(major) + 8
                    logger.debug('new version :', str(major) + m.group(1)[1:])
                    return str(major) + m.group(1)[1:]
    ...
    def __parseVersionInServerLog(self, content):
        ''' Parse JBoss version from the server log file
        @resource-file: server.log
        @types: str -> str or None
        '''
        m = re.search('\[org.jboss.as\] \(MSC service thread (.*) starting', content)
        if m:
            content = m.group(0)
            m = re.search('(\d+\.){2}\d+\.{0,1}(\w)*', content) # example for search string: 8.2.1.Final
...
class ServerConfigParserV30(BaseConfigParser):
    ...
    def parseBindingsFromBindingManagerConfig(self, content, bindingConfigName):
        ...
        try:
            ...
            logger.debugException("jboss: No data collected. Missing configuration file <file missing or no permission parse the file>")
            logger.reportWarning("jboss: No data collected. Missing configuration file <file missing or no permission parse the file>")
        return bindings
...
class ServerConfigDescriptorV7:
    ...
    class Profile:
        ...
        def __init__(self, name = None, datasources = None, \
                     jmsResources = None, includedProfilesNames = None, connectorResources = None):
            ...
            self.__connectorResources = {}
            if connectorResources: self.__connectorResources = connectorResources
        ...
        def getConnectorResources(self):
            return self.__connectorResources
...
#        logger.debug('+++ managementBindings: %s' % managementBindings)
    ...
    def parseProfiles(self, profileNodes):
        ...
        for profileIndex in range(0, profileNodes.getLength()):
            ...
            connectorResources = self.parseConnectorRessource(profileNode)
            profile = ServerConfigDescriptorV7.Profile(name, datasources, jmsResources, includedProfilesNames, connectorResources)
...
#        logger.debug('+++ datasources: %s' % datasources)
    ...
    def parseConnectorRessource(self, profileNode):
        connectorResources = {}
        xpath = self._getXpath()
        XAconnectorResourceNodes = xpath.evaluate(r'subsystem/connector', profileNode, XPathConstants.NODESET)
        for XAConnectorResourceIndex in range(0, XAconnectorResourceNodes.getLength()):
            XAConnectorResourceNode = XAconnectorResourceNodes.item(XAConnectorResourceIndex)
            connectorResourceName = XAConnectorResourceNode.getAttribute('name')
            connectorResourceSocketBinding = XAConnectorResourceNode.getAttribute('socket-binding')
            connectorResources[connectorResourceName] = connectorResourceSocketBinding
...
#        logger.debug('=== parseDomainController ||| found domainController: %s' % domainControllerNodes.getLength())
        for domainControllerIndex in range(0, domainControllerNodes.getLength()):
            ...
            if dcType == ServerConfigDescriptorV7.DomainController.Type.REMOTE and resolvePropertyfromNode(domainControllerNode.getAttribute('port')):
...
class ServerDiscovererByShellV7(ServerDiscovererByShell):
    ...
    def _isApplicablePlatformTrait(self, trait):
        ...
        @types: entity.PlatformTrait -> bool
        '''
        VERSIONLIST = [7, 8, 9, 10, 11]
        if trait.majorVersion.value() in VERSIONLIST:
            return True
        else:
            return False
...
def discoverHomeDirPath(fs, serverSystemProperties, cmdLineElements):
    ...
    classPathDefinitions = map(lambda e: e.getValue(), filter(isClassPathElement, cmdLineElements))

...
ACTIVE = 'Active'
BACKUP = 'Backup'
FSM_ROLES = [ACTIVE, BACKUP]

class CMM:
    '''
    Data Object represent the IBM CMM
    '''
    def __init__(self, name, chassis_name, slot_number=''):
        self.name = name
        self.slot_number = slot_number
        self.chassis_name = chassis_name
    def __str__(self):
        return 'CMM(name="%s", chassis_name="%s", slot_number="%s")' % (self.name, self.chassis_name, self.slot_number)
    def __repr__(self):
        return self.__str__()
...
class IbmLpar:
    def __init__(self, lparProfile = None):
        ...
        self.osVersion = None
        self.rmcIp = None
        self.isLparNameResolvable = False
...
########################################################################
def createCmm(cmm, container):
    cmm_osh = ObjectStateHolder('ibm_cmm')
    if cmm.name:
        cmm_osh.setStringAttribute('name', cmm.name)
    cmm_osh.setContainer(container)
    if cmm.slot_number:
        cmm_osh.setStringAttribute('chassis_slot_number', str(cmm.slot_number))
    return cmm_osh
...
#            fsmOsh.setStringAttribute("fsm_bios", fsmSoftware.bios)
        ...
        if fsmSoftware.slot_number:
            fsmOsh.setStringAttribute("fsm_slot_number", str(fsmSoftware.slot_number))
        return fsmOsh
...
def buildGenericHostObject(host_class, hostDo,  report_serial = True):
    ...
    key = hostDo.hostname or (hostDo.ipList and str(hostDo.ipList[0])) or hostDo.serial or hostDo.sys_obj_id
    if key:
        host_osh.setStringAttribute('host_key', key)
...
def reportCmmLinks(chassis, cmm_osh):
    vector = ObjectStateHolderVector()
    if not (chassis and cmm_osh):
        return vector
    if not chassis.ipList:
        return vector
    for ip in chassis.ipList:
        ip_osh = modeling.createIpOSH(ip)
        link_osh = modeling.createLinkOSH('containment', cmm_osh, ip_osh)
        vector.add(ip_osh)
        vector.add(link_osh)
    return vector
...
def reportVirtualMachine(vm_map, server, server_osh, host_class, lpar, server_serial_to_interfaces):
    ...
    if hostDo.model and hostDo.machineType:
        vm_osh.setStringAttribute('discovered_model', "%s-%s" % (hostDo.machineType, hostDo.model))
        
    if lpar.logicalSerialNumber:
        serial_number = lpar.logicalSerialNumber
        if lpar.lparId:
            int_lpar_id = None
            try:
                int_lpar_id = int(str(lpar.lparId))
            except:
                # perhaps the value is present in a hex form
                int_lpar_id = int(str(lpar.lparId), 16)
            serial_number = lpar.logicalSerialNumber[:len(lpar.logicalSerialNumber) - len(str('%x' % int_lpar_id))]
        vm_osh.setStringAttribute('serial_number', serial_number)
#    elif hostDo.serial:
#        vm_osh.setStringAttribute('serial_number', 'derived:%s' % hostDo.serial)
    ...
    if not (hostDo.ipList or hostDo.macs):
        vm_osh.setStringAttribute('host_key', key)
...
def reportFsmHost(hostDo, chassis_map):
    vector = ObjectStateHolderVector()
    server_osh = buildGenericHostObject('unix', hostDo)
...
def buildSwitch(hostDo):
    ...
    if hostDo.switch_fqdn:
        switch_osh.setStringAttribute('primary_dns_name', hostDo.switch_fqdn)
    logger.debug('Processing switch %s' % hostDo)
...
def buildStorageSystem(hostDo):
    ...
    if hostDo.description:
        storage_osh.setStringAttribute('discovered_description', hostDo.description)
    if hostDo.discovered_model:
        storage_osh.setStringAttribute('discovered_model', hostDo.discovered_model)
    return storage_osh
...
def ReportTopology(chassises, servers, system_pools, vms, switches, storages, fsm_osh, cmms, fsms, reportHostName):
    vector = ObjectStateHolderVector()
    vector.add(fsm_osh)
    fsm_osh.setStringAttribute('fsm_role', BACKUP)
    vm_map = {}
    for vm in vms:
    ...
    chassis_name_to_chassis = {}
    if chassises:
        for chassis in chassises:
            chassis_name_to_chassis[chassis.name] = chassis
    ...
    if cmms:
        for cmm in cmms:
            chassis_osh = chassis_name_to_chassis_osh.get(cmm.chassis_name)
            if not chassis_osh:
                logger.warn('Failed to find chassis for CMM with name %s and chassis name %s' % (cmm.name, cmm.chassis_name))
                continue
            cmm_osh = createCmm(cmm, chassis_osh)
            chassis = chassis_name_to_chassis.get(cmm.chassis_name)
            vector.addAll(reportCmmLinks(chassis, cmm_osh))
            vector.add(cmm_osh)
#    if fsms:
#        for fsm_host in fsms:
#            logger.debug('Processing FSM hosts')
#            vect, fsm_server_osh, _ = reportFsmHost(fsm_host, chassis_name_to_chassis_osh)
#            fsm_host.fsmSoftware.build(fsm_server_osh)
#            if fsm_host.fsmSoftware.osh:
#                vect.add(fsm_host.fsmSoftware.osh)
#            vector.addAll(vect)
            
    if servers:
        fsm_osh.setStringAttribute('fsm_role', ACTIVE)
...
#                        virtualHostOsh = modeling.createHostOSH(lpar.lparProfile.lparIpAddress, 'host', lpar.osName)
                    ...
                    if (reportHostName and reportHostName.lower().strip() == 'true') or lpar.isLparNameResolvable:

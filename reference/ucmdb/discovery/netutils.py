...
def parseMac(origmac):
    ...
    macRe = r'\A([0-9A-F]{1,2}[-]?){4,8}\Z|\A([0-9A-F]{1,2}[:]?){4,8}\Z|\A([0-9A-F]{1,2}[ ]?){4,8}\Z'
...
def isUrlAvailable(url, acceptedStatusCodeRange, timeout=10000):
    ...
    with _create_http_client_wrapper() as client:
        ...
        try:
            ...
            logger.warn('Failed parsing url %s ' % url)
        try:
...
def updateEndpointAddress(endpoint, address):
    ...
    return Endpoint(endpoint.getPort(), endpoint.getProtocolType(), address, endpoint.isListen(),
...
class DNSResolver:
    ...
    def resolveDnsNameByNslookup(self, ipAddr, dnsServer=''):
        ...
        """
        resolver = dns_resolver.NsLookupDnsResolver(self.shell, dnsServer)

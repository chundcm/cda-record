...
class UnixBuilder(ComputerBuilder):
    CIT = 'unix'

class NtBuilder(ComputerBuilder):
    CIT = 'nt'
...
def build_vm_unix(hostname=None):
    '@types: str -> osh'
    pdo = Builder.create_pdo(name=hostname, host_key=None, is_virtual=True)
    return UnixBuilder().build(pdo)

def build_vm_nt(hostname=None):
    '@types: str -> osh'
    pdo = Builder.create_pdo(name=hostname, host_key=None, is_virtual=True)
    return NtBuilder().build(pdo)

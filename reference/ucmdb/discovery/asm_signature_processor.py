...
def process(Framework, configSignature, applicationResult, shell, processMap, hostIPs, scpOsh, bizOsh):
    ...
    createPredefinedVariables(Framework, processMap if includeParentProcesses else None, applicationResult, variableResolver)
...
def createPredefinedVariables(Framework, processMap, applicationResult, variableResolver):
    ...
    urlString = Framework.getDestinationAttribute('BIZ_URL')
    if urlString:
        urlString = urlString[1:len(urlString) - 1]
        hostname = parseUrl(urlString)
        if hostname:
            variableResolver.add('biz.url', hostname)

    for process in applicationResult.application.getProcesses():
    ...
    for resource in applicationResult.applicationresources:
        if resource.getObjectClass() == 'j2eeapplication':
            path = resource.getAttributeValue('j2eeapplication_fullpath')
            variableResolver.add('scp.application_path', path)
            logger.debug('Add predefined variable: scp.application_path = ', path)
        elif resource.getObjectClass() == 'webapplication':
            path = resource.getAttributeValue('resource_path')
            variableResolver.add('scp.application_path', path)
            logger.debug('Add predefined variable: scp.application_path = ', path)
        ...
        for value in values:
            if not value:
                if hasattr(variableSignature, ATTR_DEFAULT_VALUE):
                    # let defaultValue support variable
                    value = ExpressionResolver.resolve(variableResolver, variableSignature.defaultValue)

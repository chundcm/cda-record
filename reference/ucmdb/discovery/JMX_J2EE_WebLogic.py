...
def DiscoveryMain(Framework):
    ...
    trustAll = Boolean.parseBoolean(Framework.getParameter('trustAllSSLCertificates'))
    try:
        if trustAll:
            logger.debug('TrustAllCert in Weblogic by Shell or JMX')
            jmx.TrustAllCert.trustAllCertificates()
...
def discoverDomainAdministrativeIps(allservers, dnsResolver):
    ...
    if adminServers:
        ...
        if not adminServers[0].ip.value():
            ...
            if domainIpAddresses and domainIpAddresses[0]:
                adminServers[0].ip.set(domainIpAddresses[0])
            else:
                raise ValueError("No IpAddress of %s" % adminServers[0])

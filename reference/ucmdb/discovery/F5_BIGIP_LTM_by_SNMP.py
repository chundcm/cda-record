...
class F5Discoverer:
    ...
    def convertToIp(self, encodedIp):
        #TODO: Make it Python
        ipParts = []
        if encodedIp and len(encodedIp) == 32:
            #seems to be an IPv6 value
            
            for i in range(8):
                ipParts.append(encodedIp[:4])
                encodedIp = encodedIp[4:]
            return ':'.join(ipParts)
    ...
    def resolveCrgIp2Url(self, ip_string):
        resolver = SocketDnsResolver()
        HostFQDN = None
        try:
            HostFQDN = resolver.resolve_hostnames(ip_string)
        except:
            logger.debug("failed to resolve ip to hostname")
        return HostFQDN
...
class F5v4Discoverer(F5Discoverer):
    ...
    def discoverVirtualServers(self, f5):
        ...
        for virtualServer in virtualServers:
            ...
            HostFQDN = self.resolveCrgIp2Url(virtualServer.ipAddress)
            if HostFQDN:
                virtualServerHelper.reportUrl(HostFQDN)
    ...
    def discoverPools(self, poolNameToVirtualServer, f5):
        ...
        for pool in pools:
            ...
            hash_str = str(hash(str(datetime.datetime.now()) + pool.name))
            poolOsh.setStringAttribute('lb_cluster_id', hash_str)
...
class F5v9Discoverer(F5Discoverer):
    ...
    def discoverVirtualServers(self, f5Osh):
        ...
        for virtualServer in virtualServers:
            ...
            HostFQDN = self.resolveCrgIp2Url(ipAddress)
            if HostFQDN:
                virtualServerHelper.reportUrl(HostFQDN)
...
class VirtualServerHelper:
    def __init__(self, name, ipAddress, f5Osh, port):
        self.name = name
        self.ipAddress = ip_addr.IPAddress(ipAddress)
        self.primary_dns_name = None
        self.port = port
    ...
    def _buildVirtualServerOsh(self):
        domainName = DomainScopeManager.getDomainByIp(str(self.ipAddress).strip())
    ...
    def _buildServiceAddress(self):
        serviceAddressOsh = modeling.createServiceAddressOsh(self.osh, str(self.ipAddress), self.port, modeling.SERVICEADDRESS_TYPE_TCP)
    ...
    def setDnsName(self, DnsName):
        self.primary_dns_name = DnsName

    def reportUrl(self, HostFQDN):
        self.setDnsName(HostFQDN[0])
        self.osh.setAttribute("primary_dns_name", self.primary_dns_name)
        self.ipOSH.setAttribute("authoritative_dns_name", self.primary_dns_name)
    ...
    def __str__(self):
        return "[Name:%s, IP: %s, Port: %s]" % (self.name, repr(self.ipAddress), self.port)

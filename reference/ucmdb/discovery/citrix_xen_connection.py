...
from citrix_xen_models import Pool, Host
        ...
        if credentials:
            ...
            for creId in credentials:
                ...
                try:
                    ...
                    pool = getPool(session)
                    host = None
                    if pool:
                        host = pool.getMaster()
                    if not host:
                        logger.debug('Cannot get pool, try another way to get host')
                        host = Host.getAll(session)[0]
                    if not host:
                        logger.debug('Cannot get pool or host, try another credential')
                        continue
                    ...
                    virtual_osh.setStringAttribute('name', 'Citrix Xen Hypervisor_%s' %hostname)

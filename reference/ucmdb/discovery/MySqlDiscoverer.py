...
class MySqlDiscoverer:
    ...
    def getConfigsFromHelp(self, fileMonitor):
        ...
        try:
            ...
            if not help:
                if self.shell.isWinOs():
                    for i in range(len(mysqlCommandList)):
                        mysqlCommandList[i] = mysqlCommandList[i].replace('mysqld', 'mysql')
                else:
                    for i in range(len(mysqlCommandList)):
                        mysqlCommandList[i] = mysqlCommandList[i].replace('/sbin/mysqld', '/bin/mysql')
                help = self.shell.execAlternateCmdsList(mysqlCommandList)
            if self.shell.getLastCmdReturnCode() != 0:
...
#        useLink = modeling.createLinkOSH('use', masterHostOsh, serviceAddressOsh)
    ...
    def parseOpts(self, processParams):
        paramGroupsList = re.findall('\s*("[^"]+")|([^"]\S*="[^"]+")|([^"]\S*=[\/\w\.-]+)|([^"]\w*)', processParams)
        paramList = []

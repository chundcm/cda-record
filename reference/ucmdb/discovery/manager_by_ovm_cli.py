...
def check_cred(framework, creds_manager, cred_id):
    ...
    if cred_port is not None:
        ...
        # in some new linux OS, port 10000 is listened on tcp6. we can not report ip service endpoint in this case. So, try default port 10000.
        else:
            if cred_port == '10000':
                return True
        return False
...
def report(host_id, mgr_version, server_configs, vm_configs):
    ...
    for server_config in server_configs:
        ...
        try:
            ...
            logger.debug('no server pool detected: ' + server_config.server.name )
        # report OVM server
        ...
        serverID = re.match('([\d\w]{2}:)+[\d\w]{2}', server_id)
        # report VMs
        if serverID:
            vms = vm_configs_by_server_id.get(serverID.group(0), ())
...
def report_vm_node(vm_config, hypervisor_osh):
    ...
    os = vm_config.vm.os_title
    # try to get os type so that we could report it accurately
    try:
        if os == 'None':
            host_osh = ovm_node.build_vm_node(name)
        elif 'Windows' in os:
            host_osh = ovm_node.build_vm_nt(name)
        else:
            host_osh = ovm_node.build_vm_unix(name)
    except:
        logger.debug('failed to discover VM due to unknown OS:' + name )

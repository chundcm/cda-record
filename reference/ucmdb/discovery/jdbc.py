...
class DnsEnabledJdbcTopologyReporter(JdbcTopologyReporter):
    ...
    def __mergeDatabaseServers(self, prefferedSource, anotherSource):
        ...
        port = anotherSource.getPort() or prefferedSource.getPort()
    ...
    def __reportOracleListenerAndServiceName(self, ip, port, serviceName, hostOsh, serverOsh):
        # report oracle TNS listener and oracle service name
        vector = ObjectStateHolderVector()
        ...
        listener = db.OracleListener(ip, port)
        vector.addAll(reporter.reportTnsListener(listener, hostOsh))

        if not serviceName:
            serviceName = "orcl"
        serviceNameOSH = ObjectStateHolder("oracle_servicename")
        serviceNameOSH.setAttribute('name', serviceName)
        serviceNameOSH.setContainer(listener.getOsh())
        vector.add(serviceNameOSH)
        vector.add(modeling.createLinkOSH('realization', serviceNameOSH, serverOsh))
        return vector
    ...
    def __reportParsedServer(self, platform, datasource, databaseServer):
        ...
        try:
            ...
            server_osh, _, _, hostOsh, oshs = databaseServerReporter.reportServerWithDatabasesFromAddress(server, (datasource.getOsh(), ))
            vector.addAll(oshs)

            # report oracle tns listener and oracle service name
            logger.debug("Database Platform: ", platform)
            if platform == db_platform.Oracle():
                pattern = re.compile("jdbc:(\w+):")
                m = pattern.search(datasource.url)
                if m and m.group(1).lower() == 'pointbase' and server.getPort() == None:
                    server.setPort('9092')
                vector.addAll(self.__reportOracleListenerAndServiceName(server.address,
                                                                        server.getPort(),
                                                                        server.instance,
                                                                        hostOsh,
                                                                        server_osh))
    ...
    def __reportOracleRac(self, platform, datasource, rac):
        ...
        for server in resolvedServers:
            ...
            # report TNS listener and service name for each server
            logger.debug("Database Platform: ", platform)
            if platform == db_platform.Oracle():
                vector.addAll(self.__reportOracleListenerAndServiceName(server.address,
                                                                 server.getPort(),
                                                                 rac.getName(),
                                                                 modeling.createHostOSH(server.address),
                                                                 server.getOsh()))
            ...
            if datasource.url:
                ...
                if server.address:
                    try:
                        ...
                        databaseServerReporter = db.getReporter(platform, databaseBuilder)
                        _, _, _, _, oshs = databaseServerReporter.reportServerWithDatabasesFromAddress(server, (datasource.getOsh(), ))
                        vector.addAll(oshs)
        return vector

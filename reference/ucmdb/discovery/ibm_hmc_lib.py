...
def isVio(shell):
    ...
    if shell.getLastCmdReturnCode() == 0 and output.find('restricted: cannot specify') == -1:
        return 1
...
def isWpar(shell):
    output = shell.execAlternateCmds('lswpar -q -d UCMDB')
    if output and shell.getLastCmdReturnCode() == 0 and output.find('UCMDB') != -1:
        return 1

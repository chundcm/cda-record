...
def ip_filterable(cls):
    t = cls.__init__
    def __init__(self, *a, **kwargs):
        self.TO_BE_FILTER_IPS = {}   #map, ip string as key
        self.FILTERED_IPS = []       #list, ip string
        t(self, *a, **kwargs)
    setattr(cls, '__init__', __init__)
    return cls

def applying_filter(func):
    def applying(self, *a, **kwargs):
        for it in self.TO_BE_FILTER_IPS.keys():
            if str(it) in self.FILTERED_IPS:
                self.TO_BE_FILTER_IPS.pop(it)
        func(self, *a, **kwargs)
    return applying
...
class UnixNetworking:
    def __init__(self):
        ...
        self.TO_BE_FILTER_IPS = self.ipStringToIp

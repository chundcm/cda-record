...
def get_layer2_connections(local_devices, remote_devices, local_lldp_ports = None):
    '''
    Maps CDP/LLDP info with local device by local interface index
    @type :NetworkDevice, NetworkDevice ->[Layer2Connection]
    '''
    layer2_connections =[]
    loc_devs = local_devices
    if local_lldp_ports:
        #mapping should be done based on interface names
        #loc_devs = {}
        for dev in local_devices.values():
            lldp_local = local_lldp_ports.get(dev.port)
            if not lldp_local and dev.port and dev.port.startswith('ethernet'):
                lldp_local = local_lldp_ports.get('gigabit%s' % dev.port) or local_lldp_ports.get('10gigabit%s' % dev.port)
            #logger.debug('lldp_local %s with port %s ' % (lldp_local, dev.port))
            if lldp_local:
                loc_devs[lldp_local.index] = dev
    for interface_index, remote_device in remote_devices.iteritems():
        local_device = loc_devs.get(interface_index)
        if local_device:
...
class LldpDiscoverer:
    ...
    LLDP_LOCAL_PORTS_DATA = '1.0.8802.1.1.2.1.3.7.1'
    
    def __init__(self, client):
        self.client = client

    def get_lldp_local_port_info(self):
        '''
        Select data related to the local interfaces
        @return: list
        '''
        result = {}
        snmp_agent = snmputils.SnmpAgent(None, self.client)
        query_builder = SnmpQueryBuilderWithRowIndex(self.LLDP_LOCAL_PORTS_DATA)
        query_builder.addQueryElement(2, 'lldpLocPortIdSubtype')
        query_builder.addQueryElement(3, 'lldpLocPortId')
        query_builder.addQueryElement(4, 'lldpLocPortDesc')
        for elem in snmp_agent.getSnmpData(query_builder):
            name = elem.lldpLocPortDesc.lower()
            result[name] = elem
        return result
    ...
    def get_remote_devices(self):
        ...
        for connection in connections:
            ...
            if device.name and not (device.mac_address or device.ip_address):
                ip_addr = netutils.getHostAddress(device.name)
                if ip_addr:
            ...
            if interface_index:
                devices[interface_index] = device
            logger.debug("Interface index %s" % interface_index)
            logger.debug('Remote device detected %s' % device)
        return devices
...
def DiscoveryMain(Framework):
    ...
    try:
        try:
            ...
            map_lldp_ports_by_names = Boolean.parseBoolean(Framework.getParameter('mapLLDPPortsByNames'))
            ...
            if discover_lldp_mib:
                ...
                if remote_devices_lldp:
                    local_lldp_ports = {}
                    if map_lldp_ports_by_names:
                        local_lldp_ports = lldp_discoverer.get_lldp_local_port_info()
                    layer2_connections_lldp = get_layer2_connections(local_devices, remote_devices_lldp, local_lldp_ports)

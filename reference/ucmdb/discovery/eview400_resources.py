...
def ev1_getCpulistOutput(ls):
    ...
    if output.isSuccess() and len(output.cmdResponseList) > 0:
        cpuTable = output.getRegexedValuesFromList(output.cmdResponseList, 'MP(\d+)\s+(\w+-\w+)\s+(\w+-\w+)') or output.getRegexedValuesFromList(output.cmdResponseList,'MP(\d+)\s+(\w+)\s+(\w+)')
        cecTable = output.getRegexedValuesFromList(output.cmdResponseList, '(CEC\d+)\s+(\w+-\w+)\s+(\w+-\w+)')
        memTable = output.getRegexedValuesFromList(output.cmdResponseList, 'MS\d+.*\s(\d+)MB M[Aa][Ii][Nn] S[Tt][Oo][Rr][Aa][Gg][Ee] C[Aa][Rr][Dd]')
...
def ev5_getInstalledSoftware(ls):
    ...
    if output.isSuccess() and len(output.cmdResponseList) > 0:
        ...
        for i in range(1, len(prods)):
            if (prods[i][0]  in ['RESOURCE', 'Resource', 'ID','5722SS1 V']):
                continue 

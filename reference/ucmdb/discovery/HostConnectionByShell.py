...
def getDnsHostName(Framework,ip,shell,hostOSH,OSHVResult):
    discoveryDns = Framework.getParameter('resolveDNS')
    discoveryDnsPara = Boolean.parseBoolean(discoveryDns)
    if discoveryDnsPara == True:
        ...
        if dnsServers:
            dnsName = DNS_Discovery.resolveHostNameByDnsList(ip, shell, dnsServers)
        else:
        ...
        if dnsName == None:
            ...
            # Set ip DNS by dnsName, report host and ip, and these attribute have reported,
            # here just add some attribute, do not report link and host
        ...
        if smallIpDns and smallIpDns in primary_dns_name_candidates:
        ...
        logger.info('The parameter of resolveDNS is not enable, do not need to resolve DNS!')
...
def doDiscovery(Framework, shell, client, ip, codepage, connectedShellCredId = None):
    ...
    #NAT
        vector.addAll(NTCMD_Connection_Utils.doHPCmd(shell, shellObj, ip, langBund, Framework, host_cmdbid, host_key, None, uduid, natIPs, doDnsHostNameMethod=getDnsHostName))
    else:
        vector.addAll(TTY_Connection_Utils.getOSandStuff(shell, shellObj, Framework, langBund, uduid, natIPs, ip, doDnsHostNameMethod=getDnsHostName))

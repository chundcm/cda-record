...
def checkAgentInstalled(Framework, ignoreError=False):
    ...
    agent = AgentUtils.agentConnect(Framework, None, warningsList, errorsList)
    if not agent:
        if not ignoreError:
        ...
        try:
            ...
            # Agent is installed, sync the Smart agent options for agent installation (no matter for update or install job)
            AgentUtils.updateAgentDrivenFlowOptions(Framework, agent)

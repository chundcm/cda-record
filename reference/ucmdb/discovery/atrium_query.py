...
#    credentialsId = credentialIds[0]
    ...
    if credentialsId:
        ...
        logger.debug( "Remedy Protocol pwd is empty: ", ( not password) )

        if not username:
            username = Framework.getProtocolProperty( credentialsId, 'protocol_username', '' )
            password = Framework.getProtocolProperty( credentialsId, 'protocol_password', '' )
            logger.debug( 'Protocol_username: ', username )
            logger.debug( "Protocol pwd is empty: ", ( not password) )

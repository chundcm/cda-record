...
class VirtualCenterDiscoverer(BaseDiscoverer):
    ...
    def discoverLicenses(self):
        self.licensesDiscoverer = VirtualCenterLicensesDiscoverer(self.client, self.framework, self.discoveryConfig,
                                                                  None, self.osh)
            ...
            for objectContent in contents:
                ...
                if ref.getType() == 'ClusterComputeResource':
                    ...
                    if self.client.getVersionString() == VMWARE_PROTOCOL_VERSION_25:
                        computeResourceDiscoverer = ClusterComputeResourceDiscoverer25(self.client, self.framework,
                                                                                       self.discoveryConfig, self.osh)
                    elif self.client.getVersionString() == VMWARE_PROTOCOL_VERSION_20:
                        computeResourceDiscoverer = ClusterComputeResourceDiscoverer20(self.client, self.framework,
                                                                                       self.discoveryConfig, self.osh)
                    else:
                        raise ValueError, "Unknown protocol version"
                else:
                    # non-clustered managed ESX
                    computeResourceDiscoverer = NonclusteredEsxComputeResourceDiscoverer(self.client, self.framework,
                                                                                         self.discoveryConfig, self.osh)
...
class EsxDiscoverer(ManagedEntityDiscoverer):
    ...
    PROP_BIOSINFO = 'hardware.biosInfo'
    supportedProperties = ManagedEntityDiscoverer.supportedProperties + [PROP_SUMMARY, PROP_PRODUCT, PROP_DNS_CONFIG,
                                                                         PROP_VMS, PROP_CONNECTION_STATE, PROP_BIOSINFO]
    ...
    def __init__(self, client, framework, discoveryConfig):
        ...
        self.handlers[EsxDiscoverer.PROP_BIOSINFO] = self.handleBiosInfo
        self.createLayerOsh()
        self.createEsxOsh()
        self.ip = None
        self.licensesDiscoverer = None
    ...
    def handleBiosInfo(self, biosInfo):
        biosVersion = biosInfo.getBiosVersion()
        self.esxOsh.setStringAttribute('bios_version', biosVersion)
        biosReleaseDate = biosInfo.getReleaseDate()
        if biosReleaseDate:
            try:
                biosReleaseDate = biosReleaseDate.getTime()
            except:
                biosReleaseDate = biosReleaseDate.toGregorianCalendar().getTime()
            self.esxOsh.setDateAttribute('bios_date', biosReleaseDate)
    ...
    def discoverLicenses(self):
        self.licensesDiscoverer = LicensesDiscoverer(self.client, self.framework, self.discoveryConfig, self.selfRef,
                                                     self.osh)
        self.licensesDiscoverer.discover()
...
class StandaloneEsxDiscoverer(BaseDiscoverer):
    ...
    def discoverStandaloneEsxComputeResource(self):
        ...
        if contents is not None:
            ...
            if computeResourcesCount == 1:
                ...
                ref = objectContent.getObj()
                props = objectContent.getPropSet()
                esxComputeResourceHandler = StandaloneEsxComputeResourceDiscoverer(self.client, self.framework,
    ...
    def discoverLicenses(self):
        ...
        if esxOsh is not None:
            ...
            self.licensesDiscoverer = LicensesDiscoverer(self.client, self.framework, self.discoveryConfig, None,
                                                         esxOsh)
            self.licensesDiscoverer.discover()
...
class VmwareServerConnectionDiscoverer(BaseDiscoverer):
    ...
    def discover(self):
        ...
        if self.apiType == VmwareServerConnectionDiscoverer.VIM_API_VC:
            ...
            self.osh = modeling.createApplicationOSH('virtualization_layer', 'Virtualization Layer Software',
                                                     self.hostOsh)
        else:
    ...
    def retrieveEsxUuid(self):
        ...
        folderTraversalSpec.setSelectSet(
            [datacenterToHostFolderSelectionSpec, recurseFoldersSelectionSpec, computeResourceSelectionSpec])
...
class UrlFromProtocolGenerator(UrlGenerator):
    ...
    def getUrl(self, credentialsId, errorList, warningsList):
        ...
        if port and isinstance(port, basestring):
            logger.debug('Port value is not a valid integer, will coerce')
            port = int(port.strip())
        if useSsl:
            prefix = 'https'
        else:
        ...
        for credentialsId in credentialsIdList:
            if not isConnected:
                try:
                    ...
                    warningsList = []
                    try:
                        serverDiscoverer = VmwareServerConnectionDiscoverer(client.getAgent(), Framework, urlString,
                                                                            credentialsId, ipAddress)
                    ...
                    if faultType == 'InvalidLogin':
                        msg = errormessages.makeErrorMessage(VMWARE_PROTOCOL_NAME, None,
                        ...
                        logger.debug(msg)
                        shouldStop = errormessages.resolveAndAddToCollections(msg, VMWARE_PROTOCOL_NAME, warningsList,
                                                                              errorsList)
                        if shouldStop:
                            break
                    else:
                        ...
                        logger.debug(dump)
                        shouldStop = errormessages.resolveAndAddToCollections(faultString, VMWARE_PROTOCOL_NAME,

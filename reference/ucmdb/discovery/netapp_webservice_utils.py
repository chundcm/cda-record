...
def connectVserver(wsConnection, vserver):
        try:
            debugPrint(2, '[' + SCRIPT_NAME + ':connectVserver] Connecting to Vserver <%s>' % (vserver))
            if vserver:
                wsConnection.setVserver(vserver)
                return wsConnection
        except:
            excInfo = logger.prepareJythonStackTrace('')
            logger.warn('[' + SCRIPT_NAME + ':connectVserver] Exception: <%s>' % excInfo)
            pass

...
def __isInsideGuest(output):
    ''' str -> bool
    '''
    keyword = "running inside an hpvm guest."
    lowerOutput = output.lower()
    if lowerOutput.find(keyword) >= 0:
        return True
    return False

def __isInsideVpar(output):
    keyword = "running inside an hpvm vpar."
    lowerOutput = output.lower()
    if lowerOutput.find(keyword) >= 0:
        return True
    return False
...
class IvmHypervisorDiscoverer(IvmBaseDiscoverer):
    ...
    def isIvmGuest(self):
        logger.debug("Checking if running inside a HPVM guest")
        output = self._shell.execCmd(IvmHypervisorDiscoverer.HPVMINFO_PATH)
        if output:
            if __isInsideGuest(output):
                return True

    def isIvmVpar(self):
        logger.debug("Checking if running inside an HPVM vPar")
        output = self._shell.execCmd(IvmHypervisorDiscoverer.HPVMINFO_PATH)
        if output:
            if __isInsideVpar(output):
                return True
...
def isIvmGuest(shell):
    return IvmHypervisorDiscoverer(shell).isIvmGuest()

def isIvmVpar(shell):
    return IvmHypervisorDiscoverer(shell).isIvmVpar()
...
def discoverAixLparEthernetAdapters(shell):
    ...
    for ethAdapterName in ethAdaptersDict.keys():
        ...
        if output and shell.getLastCmdReturnCode() == 0:
            ...
            output = shell.execAlternateCmds('lscfg -vpl %s' % ethAdapterName, '/usr/sbin/lscfg -vpl %s' % ethAdapterName)
            if output and shell.getLastCmdReturnCode() == 0:
                parseEthAdapterVpdInformation(output, ethAdaptersDict)
    validateEthAdapters(ethAdaptersDict)
...
def parseFiberChannedAdditionalAttributes(output, fcHba):
    if output and fcHba:
        for line in re.split('[\r\n]+', output):
            wwnBuf = re.search('\s*Device\s+Specific\.\(Z8\)\.+(\w+)', line)
            if wwnBuf:
    ...
    for hdisk in physicalVolumesDict.keys():
        ...
        for i in range(len(devices)):
            ...
            if physicalVolume:
                ...
                full_path_command = "/usr/sbin/%s" % command
                try:
                    output = shell.execAlternateCmds(command, "ioscli " + command, "/usr/ios/cli/ioscli " + command, full_path_command, "ioscli " + full_path_command, "/usr/ios/cli/ioscli " + full_path_command) 
                    if output:
    ...
    for i in range(len(devices)):
        ...
        if physicalVolume:
            ...
            try:
                ...
                if output:
                    local_slot_number = re.match(".*-[Cc](\d+)", output.strip())
                    if local_slot_number:
                        scsi = storage_topology.ScsiAdapter()
                        scsi.physicalPath = output.strip()
                        scsi.slotNumber = local_slot_number.group(1).strip()
                        physicalVolume.scsiAdapterSlotNumber = scsi.slotNumber
                        physVolumes.append(physicalVolume)
                        scsiList.append(scsi)
            except:
...
def createFiberChannelOsh(fiberChannelAdapter, parentOsh):
    if fiberChannelAdapter and parentOsh:
        ...
        if fiberChannelAdapter.wwn is not None:
            formated_wwn = wwn.parse_from_str(fiberChannelAdapter.wwn)
            fiberChannelOsh.setStringAttribute('fchba_wwn', str(formated_wwn).upper())
...
def doDiscovery(shell, hostOsh, managedSystemOsh, framework, osType):
    ...
    try:
        ...
        for cpu in cpus:
            cpuOsh = modeling.createCpuOsh(cpu.idStr, managedSystemOsh, cpu.speed, cpu.coresCount, 'ibm_corp',
                                           data_name = cpu.model, logicalProcessorCount=cpu.logicalCpuCount)
            vector.add(cpuOsh)
    except:
...
def DiscoveryMain(Framework):
    ...
    client = None
    shell = None
    hostOsh = modeling.createOshByCmdbId('unix', hostCmdbId)

...
def reportTopology(switchOsh, interfaces_dict, vlans, remote_peers_map, ports_map):
    ...
    if ports_map:
        for (port_name, port) in ports_map.items():
            ...
            vector.add(linkOsh)
            portNameToPortOshMap[port_name] = portOsh# portNameToPortOshMap['Ethernet*']=portOsh
            portNameToPortOshMap[port_name.replace('ernet', '')] = portOsh# portNameToPortOshMap['Eth*']=portOsh

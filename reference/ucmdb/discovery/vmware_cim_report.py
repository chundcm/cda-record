    ...
    def build(self, unitaryComputerSystem):
        ...
        if unitaryComputerSystem._hypervisorSoftwareIdentity:
            if unitaryComputerSystem._hypervisorSoftwareIdentity.lastStartTime is not None:
                hostOsh.setDateAttribute('host_last_boot_time',
                                         unitaryComputerSystem._hypervisorSoftwareIdentity.lastStartTime)
...
class HypervisorBuilder:
    ...
    def _getVersionTokens(self, software):
        versionValues = [software.majorVersion, software.minorVersion, software.revisionNumber,
                         software.largeBuildNumber]
...
class CpuBuilder:
    ...
    def build(self, processor, esxOsh):
        ...
        return modeling.createCpuOsh(cpuId, esxOsh, processor.currentClockSpeed, processor.numberOfEnabledCores, None,
                                     processor.modelName, processor.modelName)
...
class VirtualMachineBuilder:
    ...
    def build(self, vm):
        ...
        if vm.primaryIpAddress:
            vmOsh = modeling.HostBuilder.incompleteByIp(vm.primaryIpAddress, "host_node")
        else:
            vmOsh = modeling.HostBuilder.fromClassName('host_node')

        vmOsh.setAsVirtual(True)

        vmOsh.setStringAttribute('platform_vendor', PlatformVendors.VMware)

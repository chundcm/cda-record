...
class TopologyBuilder:
    ...
    def setHostAttributes(self):
        if self.hostOsh:
            ...
            if self.hostDo.biosVersion:
                self.hostOsh.setStringAttribute('bios_version', self.hostDo.biosVersion)
            if self.hostDo.biosDate:
                self.hostOsh.setDateAttribute('bios_date', self.hostDo.biosDate)
            if self.hostDo.osinstalledDate:
                self.hostOsh.setDateAttribute('os_installed_date',self.hostDo.osinstalledDate)

            if self.hostDo.osDomain:
                if self.hostDo.partOfDomain or self.hostDo.partOfDomain is None:
                    self.hostOsh.setStringAttribute('host_osdomain', self.hostDo.osDomain)
                else:
                    self.hostOsh.setStringAttribute('nt_workgroup', self.hostDo.osDomain)
            ...
            if self.hostDo.cloudInstanceId:
                self.hostOsh.setStringAttribute('cloud_instance_id', self.hostDo.cloudInstanceId)
            ...
            # fill in list of WINS servers
            if self.winsServerIpList and len(self.winsServerIpList) > 0:
                list_ = StringVector(map(str, self.winsServerIpList))
                list_.sort()
                attr = AttributeStateHolder('wins_servers', list_)
                self.hostOsh.setListAttribute(attr)

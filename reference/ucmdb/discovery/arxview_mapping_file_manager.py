...
from arxview_mapping_implementation import *

from arxview_decorators import abstract_method
...
class ArxviewMappingFileManager(FolderBasedMappingFileManager):
    def getMapping(self, fileName):
        ...
        for mappingDef in root.findall("./targetcis/source_ci_type"):
            ...
            parentType = mappingDef.get('parentType')
            parentQuery = mappingDef.get('parentQuery')
            needContainer = mappingDef.get('needContainer') == 'true'

            ciMapping = SimpleCiMapping(sourceCiName, targetCiName, query, ref, base, idKey, parentType, parentQuery, needContainer=needContainer)
        ...
        for linkMappingDef in root.findall("./targetrelations/link"):
            ...
            sourceMappingType = linkMappingDef.get('source_mapping_type')
            mappingAttribute = linkMappingDef.get('reference_attribute')
            linkMapping = ReferenceLinkMapping(sourceLinkName, targetLinkName, sourceCi1Name, sourceCi2Name,
                    ...
                    failurePolicy, isContainer, sourceMappingType, mappingAttribute)            

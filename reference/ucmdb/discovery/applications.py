...
def createApplicationSignature(framework, client, shell=None, connectionIp=None, hostClass=None, hostOsh=None):
    '''
    ...
    if hostClass:
        applicationSignature.setHostClass(hostClass)
    if hostOsh:
        applicationSignature.setHostOsh(hostOsh)
...
class ApplicationSignature:
    ...
    def setHostClass(self, hostClass):
        if hostClass:
            self.hostClass = hostClass

    def setHostOsh(self, setHostOsh):
        if setHostOsh:
            self.setHostOsh = setHostOsh

    def getHostOsh(self):
        return self.hostOsh

    def getHostClass(self):
        return self.hostClass
...
class Application:
    ...
    def process(self):
        ...
        self._processValidateTypeRules()

    def _processValidateTypeRules(self):
        '''
            This method evaluates only those parse rules which do not affect the attribute values
            Meaning only parse rules which existence is defined due to validation/identification of the Application
            e.g. Application should be created only in case the parse rule has matched.
        '''
        ...
        if parseRuleContexts and not attributeDescriptors:
            #we need to evaluate only those parse rules
            #which have do not take part in attribute updates
            #Which mean that they are used only for process level restrictions
            rule_ids =  parseRuleContexts.keys()
            for rule_id in rule_ids:
                expression = Expression('${%s}' % rule_id)
    ...
    def getHostClass(self):
        return self.getApplicationComponent().getApplicationSignature().getHostClass() or "host"
    ...
    def _createHostOsh(self):
        ...
        if appComponent.isClustered():
            ...
            if isinstance(hostId, ObjectStateHolder):
                ...
                hostClass = self.getHostClass()
                self.hostOsh = modeling.createOshByCmdbIdString(hostClass, hostId)

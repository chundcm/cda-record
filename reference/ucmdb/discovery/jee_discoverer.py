    ...
    def getJVMInfo(self, javaCommand):
        ...
        for line in output.strip().split('\n'):
            ...
            # Java(TM) SE Runtime Environment..
                ...
                if not vendor:
                    vendor = 'Oracle'
                if line.lower().find('ibm') == 0:
                    vendor = 'IBM Corporation'
                elif line.lower().find('openjdk') == 0:
                    vendor = 'OpenJDK'
...
def findTnsnamesOra(shell, isWindows, installLocs):
    try:
        ## Locals
        searchLocations = []
        resultPath = []
        ...
        useCustomizedLocation = False
        globalSettings = GeneralSettingsConfigFile.getInstance()
        locationsString = globalSettings.getPropertyStringValue('tnsnamesFilePaths', '')
        ...
        if locations:
            ...
            for location in locations:
                if location.find('/') != -1:
                    unixLocations.append(location.strip())
                else:
                    windowsLocations.append(location.strip())
            if isWindows == 'true':
                if windowsLocations:
                    searchLocations = windowsLocations
                    useCustomizedLocation = True
            else:
                if unixLocations:
                    searchLocations = unixLocations
                    useCustomizedLocation = True

        fileCount = 0
        for location in searchLocations:
            filesFound = findFile(shell, 'tnsnames.ora', location, isWindows)
            if filesFound:

# coding=utf-8
from remedyforce_mapping_interfaces import CiMapping, AttributeMapping, Filter
from remedyforce_mapping_interfaces import Mapping, LinkMapping, Link, Validator, InvalidValueException
...
class SimpleCiMapping(CiMapping):
    def __init__(self, sourceType, targetType, query, ref, base, idKey, **kwargs):
        ...
        self.__children = kwargs['children']
    ...
    def getRequiredFields(self):
        fields = [mapping.getRequiredField() for mapping in self.getAttributeMappings() if mapping.getRequiredField()]
        return set(fields)
...
class DirectAttributeMapping(AttributeMapping):
    def __init__(self, sourceName, targetName, owner=None):
        AttributeMapping.__init__(self)
    ...
    def getValue(self, ci):
        ...
        if self.__owner:
            obj = ci[self.__owner]
        return obj.getValue(self.__sourceName)

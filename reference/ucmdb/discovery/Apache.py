...
class VirtualHostProcessor:
    ...
    def __init__(self, vhostContent, allIps, allPorts, allListenAddresses, mostRecentListenPort, isWin, serverNameList):
        ...
        self.defaultServerNameList = serverNameList
    ...
    def processServerName(self):
        logger.debug('Trying to get ServerName')
        serverName = getAttribute(self.vhostContent, 'ServerName')
        logger.debug('Cannot get ServerName, try default')
        if not serverName and self.defaultServerNameList and self.defaultServerNameList[0]:
            serverName = self.defaultServerNameList[0]
        if serverName:
...
class ApacheConfigDiscoverer:
    ...
    def __init__(self, mainConfigPath, shellUtils, hostOsh, framework, processDiscoverer, resourceFactory):
        ...
        if self.isWinOs:
            self.fsSeparator = ApacheConfigDiscoverer.SEPARATOR_WIN
        else:
            self.fsSeparator = ApacheConfigDiscoverer.SEPARATOR_UNIX
    ...
    def parseConfigFile(self, configFileObject):
        ...
        self.parseDefaultServerName(configFileObject)
    ...
    def discoverIncludes(self, configFileObject):
        ...
        includeDirs.extend(getAttributeList(configFileObject.contentNoComments, 'includeOptional'))
    ...
    def discoverJkWorkersFile(self, configFileObject):
        ...
        for includeDir in includeDirs:
            ...
            try:
                ...
                try:
                    filePath2 = filePath
                    if self.serverRootWrapper.fixedPath[-1] != self.fsSeparator:
                        filePath2 = self.serverRootWrapper.fixedPath + self.fsSeparator + filePath2
                    else:
                        filePath2 = self.serverRootWrapper.fixedPath + filePath2
                    logger.debug("File path: ", filePath2)
                    fileContent = self.shellUtils.safecat(filePath2)
                except:
            ...
            if matches:
                for match in matches:
                    ...
                    url = self.proxyList[i] + ':' + port + '/'
                    logger.debug('create url:',url)
                    self.proxyList[i] = url
                    i = i + 1
    ...
    def parseVirtualHosts(self, configFileObject):
        ...
        getvhostProcessors = False
        while match.find():
            virtualHostInfo = match.group(1)
            logger.debug('---VirtualHost:', virtualHostInfo)
            vhostProcessor = VirtualHostProcessor(virtualHostInfo, self.allIps, self.listenPorts, self.listenAddresses.keys(), self.mostRecentListenPort, self.isWinOs, self.serverNameList)
            self.vhostProcessors.append(vhostProcessor)
            getvhostProcessors = True

        if not getvhostProcessors:
            pattern = Pattern(r"(ServerName\s+[\.\w]+):?(\d+)?")
            match = pattern.matcher(configFileObject.contentNoComments)
            while match.find():
                virtualHostInfo = '<VirtualHost %s>' %VirtualHostProcessor.VHOST_DEFAULT_TAG
                vhostProcessor = VirtualHostProcessor(virtualHostInfo, self.allIps, self.listenPorts,
                                                      self.listenAddresses.keys(), self.mostRecentListenPort,
                                                      self.isWinOs, self.serverNameList)
                self.vhostProcessors.append(vhostProcessor)
    ...
    def parseDefaultServerName(self, configFileObject):
        ServerNameList = getAttributeList(configFileObject.contentNoComments, 'ServerName')
        if ServerNameList:
            for serverName in ServerNameList:
                if serverName:
                    self.serverNameList.append(serverName)
    ...
    def parseOAMWebgateModule(self, configFileObject):
        ...
        oracle_newpath = oracleInstancePath.getPathWrapperOneFolderUp()
        Paths = [oamWrapper, oracle_newpath.appendPath('webgate/access/oblix/lib/ObAccessClient.xml'),oracle_newpath.appendPath('webgate/config/ObAccessClient.xml')]
        content =None
        for newpath in Paths:
            try:
                filePath = newpath.fixedPath
                fileName = newpath.getFileName()
                content = self.shellUtils.safecat(filePath)
                break
            except:
                logger.debugException('')
                logger.warnException("Error reading file by path '%s'" % newpath)
        if not content:
            raise GeneralDiscoveryException, "Error reading file by path"
        code = self.shellUtils.getLastCmdReturnCode()

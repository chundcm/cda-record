    ...
    def discover(self):
        ...
        try:
            for genericParameters in genericParametersList:
                ...
                if not server:
                    logger.warn('Failed to relate a server with its parameters. Server name %s' % genericParameters.name)
                    continue
...
class HostBasedResolver:
    def __init__(self, shell):
        self.shell = shell
        
    def _parseNameResolution(self, output):
        if not (output and output.strip()):
            return
        m = re.search('.*has\s+address\s+([\d\.]+)', output)
        return m and m.group(1)
    
    def resolveName(self, dnsName):
        output = self.shell.execCmd('host %s' % dnsName)
        if output and self.shell.getLastCmdReturnCode() == 0:
            return self._parseNameResolution(output)
    
    def _parseIpResolution(self, output):
        if not (output and output.strip()):
            return
        m = re.search('.*domain\s+name\s+(?:pointer\s+)*([\w\.\-]+)', output)
        return m and m.group(1)

    def resolveIP(self, ipStr):
        output = self.shell.execCmd('host %s' % ipStr)
        if output and self.shell.getLastCmdReturnCode() == 0:
            return self._parseIpResolution(output)
...
class LParDiscoverer(ibm_hmc_lib.GenericHmc):
    ...
    def discover(self, managedSystem):
        ...
        if managedSystem:
            ...
            for lparKey in lpars.keys():
                self._checkLparNameReverseResolvable(lpars[lparKey])
    ...
    def _checkLparNameReverseResolvable(self, lpar):
        lparName = lpar.lparName
        fqdn_name = self._getIpDnsName(lpar.rmcIp)
        if fqdn_name and fqdn_name.lower().find(lparName.lower()) != -1:
            #Lpar IP resolves into a name which is a superstring of LPar name. 
            #It's ok to use this lparName as host name in the future
            lpar.isLparNameResolvable = True
    
    def _getIpDnsName(self, ip_str):
        dns_name = None
        if ip_str:
            resolver = HostBasedResolver(self._shell)
            dns_name = resolver.resolveIP(ip_str)
            if not dns_name :
                dns_name = netutils.getHostName(ip_str)
        return dns_name
...
class ChassisDiscoverer:
    ...
    def _parseCmm(self, elems):
        if not elems:
            return
        cmm = ibm_fsm.CMM(elems.get('MM-name') or elems.get('Name'), elems.get('Name'))
        return cmm
    ...
    def _parseChassisData(self, output):
        ...
        for chunk in re.split('[\r\n]{4}', output):
            ...
            host_name = elems.get('Name', '') or elems.get('Hostname', '')
...
class SwitchDiscoverer:
    ...
    def _parseSwitchData(self, output):
        ...
        for chunk in re.split('[\r\n]{4}', output):
            ...
            raw_name = elems.get('DisplayName', '').strip()
            if raw_name and not netutils.isValidIp(raw_name) and re.match("^[\w\-]+\.[\w\-\.]+$", raw_name):
                st_node.switch_fqdn = raw_name
            if not st_node.hostname and st_node.switch_fqdn:
                m = re.match('([\w\-]+)', st_node.switch_fqdn)
                st_node.hostname = m and m.group(1)
            ...
            st_node.switch_type = elems.get('SwitchType')
...
class StorageDiscoverer:
    ...
    def _parseSubSystemData(self, output):
        ...
        for chunk in re.split('[\r\n]{4}', output):
            ...
            #st_node.description = '%s %s' % (elems.get('DisplayName'), elems.get('Description')) 
            st_node.description = elems.get('DisplayName')
            st_node.discovered_model = elems.get('Description')
            st_node.node_name = elems.get('NodeName')
            st_node.type = elems.get('MachineType')
            st_node.serial = elems.get('SerialNumber')
            st_node.vendor = elems.get('Manufacturer')
...
class ServerDiscoverer:
    ...
    def _parseCmm(self, output_dict):
        if output_dict.get('MachineType') and str(output_dict.get('MachineType')) == '7917':
            logger.debug('Found IBM CMM module with name: %s' % output_dict.get('DisplayName'))
            if not (output_dict.get('DisplayName') and output_dict.get('ChassisName')):
                logger.warn('Detected CMM appeared to be invalid. No name or chassis present.')
                return
            return ibm_fsm.CMM(output_dict.get('DisplayName'), output_dict.get('ChassisName'), output_dict.get('OccupiedSlots'))
    
    def __parseFsm(self, output_dict):
        if output_dict.get('MachineType') and str(output_dict.get('MachineType')) == '7955':
            logger.debug('Found IBM FSM location')
            hostDo = ibm_fsm.Host()
            ips_raw = output_dict.get('IPv4Address')
            hostDo.referenced_chassis = output_dict.get('ChassisName')
            ...
            if not hostDo.ipList:
                logger.debug('No IP information for FSM node. Scipping.')
                return
            fsm = ibm_fsm.IbmFsm(ibm_fsm.FsmTypeInformation(None, None), ibm_fsm.VersionInformation(None, None))
            fsm.slot_number = output_dict.get('OccupiedSlots')
            hostDo.fsmSoftware = fsm
            return hostDo
    ...
    def _parseServerData(self, output):
        ...
        for chunk in re.split('[\r\n]{4}', output):
            ...
            hostDo.machineType =  elems.get('MachineType')
    ...
    def _splitVms(self, hosts, cmms, fsms):
        return [x for x in hosts if not x.is_virtual], [x for x in hosts if x.is_virtual], cmms, fsms
         
    def discover(self):
        hosts, cmms, fsms = self.getServer()
        return self._splitVms(hosts, cmms, fsms)

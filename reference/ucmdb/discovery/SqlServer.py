...
class SqlServer:
    def __init__(self,connection, discoveryOptions):
        ...
        self.alwaysOnConfig = AlwaysOnConfiguration.AlwaysOnConfiguration(connection, discoveryOptions)
    ...
    def getDbSize(self, dbMap):
        rs = self.connection.getTable(Queries.DATABASE_SIZES)
        while rs.next():
            db_name = rs.getString(1)
            db_size = rs.getDouble(2)
            db_osh = dbMap.get(db_name, None)
            if not db_osh:
                db_osh = ObjectStateHolder('sqldatabase')
                db_osh.setAttribute('name', db_name)
            db_osh.setAttribute('db_size', db_size)
        rs.close()

    def getDbMirror(self, dbMap):
        rs = self.connection.getTable(Queries.DATABASE_MIRRORING)
        while rs.next():
            db_name = rs.getString(1)
            principal_server_name = rs.getString(2)
            mirror_server_name = rs.getString(3)
            db_osh = dbMap.get(db_name, None)
            if not db_osh:
                db_osh = ObjectStateHolder('sqldatabase')
                db_osh.setAttribute('name', db_name)
            db_osh.setAttribute('principal_server_name', principal_server_name)
            db_osh.setAttribute('mirror_server_name', mirror_server_name)
        rs.close()
        ...
        try:
            ...
            if self.discoveryOptions and self.discoveryOptions.discoverDbUser:
                try:
                    ...
                    logger.debugException("Failed to get users for database")

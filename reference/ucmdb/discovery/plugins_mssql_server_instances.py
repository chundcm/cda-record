...
class MssqlMsClusterHandler:
    ...
    def __parseMsSqlRelatedClusterGroup(self, output, instanceName = None):
        if instanceName and output:
            for line in output.split('\n'):
                #match = re.match('.*\)\s+([\w\.\- ]+)\s+([\w\.\-]+)\s+([\w\.\-]+)\s*', line)
                match = re.match('SQL Server \(%s\)\s+(\S+)\s+' % instanceName, line, re.I)
                if match:
        ...
        if output:
            ...
            for line in output.strip().splitlines():
                ...
                if count_flag:
                    ...
                    name_len = len(result[0])
                    state_len = len(result[1])
                    group_len = len(result[2])
                    count_flag = False
                if endOfHeader == 1:
                    logger.debug(line)
                    resourceName = line[0:name_len - 1]
                    resourceName = resourceName.strip()
                    groupName = line[name_len + state_len - 1:name_len + state_len + group_len - 1]
    ...
    def getMsSqlRelatedClusterGroup(self, instanceName):
        if instanceName:
            ...
            #in case the instance is present as named one
            output = self.__cluster_client.execClusterCmd('CLUSTER RESOURCE "SQL Server (%s)" | find "Online"' % instanceName)
            if output and output.strip() and self.__cluster_client.shell.getLastCmdReturnCode() == 0:
                return self.__parseMsSqlRelatedClusterGroup(output, instanceName)
    ...
    def getClusterIpResources(self, groupName, instanceName):
        result = []
        if groupName:
            if self.__isUsingPowerShellCmdlets:
                return self.getClusterIpResourcesByPowerShell(groupName)
            output = self.__cluster_client.execClusterCmd('CLUSTER RESOURCE | find "SQL IP Address" | find "%s" ' % instanceName)
...
class MSSQLServerInstancesPlugin(Plugin):
    ...
    def handleMsClusteredInstance(self, context, applicationOsh):
        ...
        if msClusterHandler.isClustered():
            ...
            clusterResources = resourceGroup and msClusterHandler.getClusterIpResources(resourceGroup, instanceName)
...
class RegistryShellUtil:
    ...
    def doQuery(self, queryStr):
        ...
        try:
            ...
            remoteFile = self.shell.copyFileIfNeeded(localFile, True)
            if not remoteFile:

...
## identify oracle cdb and pdb
dbVerifyCDBQuery = (
    "SELECT NAME, CDB, CON_ID FROM V$DATABASE"
)

dbOraclePDBQuery = (
    "SELECT PDB_ID, PDB_NAME FROM DBA_PDBS ORDER BY PDB_ID"
)

## get oracle service
dbOracleServiceWithPDBQuery = (
    "SELECT NAME, PDB from V$SERVICES WHERE NETWORK_NAME IS NOT NULL"
)

dbOracleServiceQuery = (
    "SELECT NAME from V$SERVICES WHERE NETWORK_NAME IS NOT NULL"
)
...
dbSchemaQuery = ("SELECT OWNER FROM DBA_OBJECTS GROUP BY OWNER")
...
SYS_USER_LIST = (
    'ADAMS', 'ANONYMOUS', 'APEX_030200', 'APEX_040200', 'APEX_PUBLIC_USER', 'APPQOSSYS', 'AUDSYS',
    'AURORA$JIS$UTILITY$',
    'AURORA$ORB$UNAUTHENTICATED', 'AWR_STAGE', 'BI', 'BLAKE', 'C##ORACLE', 'CLARK', 'CLOTH', 'CSMIG', 'CTXSYS',
    'DBSNMP', 'DEMO', 'DIP',
    'DMSYS', 'DSSYS', 'DVF', 'DVSYS', 'EXFSYS', 'FLOWS_040100', 'FLOWS_FILES', 'GSMADMIN_INTERNAL', 'GSMCATUSER',
    'GSMUSER', 'HR', 'IX',
    'JONES', 'LBACSYS', 'MDDATA', 'MDSYS', 'MGMT_VIEW', 'ODM', 'ODM_MTR', 'OE', 'OJVMSYS', 'OLAPSYS', 'ORACLE_OCM',
    'ORDDATA',
    'ORDPLUGINS', 'ORDSYS', 'OSE$HTTP$ADMIN', 'OUTLN', 'OWBSYS', 'OWBSYS_AUDIT', 'PAPER', 'PERFSTAT', 'PM', 'REPADMIN',
    'SCOTT', 'SH',
...
def parseDbUserTableQueryRes(dbUserTableQueryRes, executor, OSHVResult,isreportEmptySchemas):
...
def discoverSchema(dbUserTableQueryRes, executor, OSHVResult,):
    if dbUserTableQueryRes is not None:
        while dbUserTableQueryRes.next():
            dataName = dbUserTableQueryRes.getString(1)
            dbschemaOSH = ObjectStateHolder('oracle_schema')
...
def discoverUserAndSchema(isreportEmptySchemas):
    def wrapper(dbUserTableQueryRes, executor, OSHVResult):
        parseDbUserTableQueryRes(dbUserTableQueryRes, executor, OSHVResult,isreportEmptySchemas)
    return wrapper
...
def parseAllRacTableQueryRes(dbRacInfoTableQueryTable, dbRacNodeInfoTableQueryTable, executor, OSHVResult):
    if dbRacInfoTableQueryTable and dbRacNodeInfoTableQueryTable:
        ...
        for racNodeInfoTableRow in dbRacNodeInfoTableQueryTable:
            getAllOracleNodeObjects(dbRacInfoTableQueryTable, racNodeInfoTableRow, sid, port, oracleOSH, racOSH,
                                    OSHVResult)
...
def parseAllDataGuardTableQueryRes(dbRoleTableQueryRes, log_archive_config, executor, OSHVResult):
    ...
    data_guard_osh.setAttribute('name', dbName)
        ...
        while dbProcessDbClientTableQueryRes.next():
            ...
            try:
                ...
                if remote_machine_name.find('/') > -1:  # if the output is like dummyserver/10.10.10.10
                    ...
                    remote_machine_ip = fptools.safeFunc(
                        lambda: str(SocketDnsResolver().resolve_ips(remote_machine_name)[0]))()
                if remote_machine_ip is None:
                    continue
            except:
                logger.warn('Cannot resolve IP address by name ', remote_machine_name, ' Ignoring client program: ',
                            program_dataName)
                continue
            ...
            if (netutils.isValidIp(remote_machine_ip)):
                if not netutils.isLocalIp(remote_machine_ip):
                    ...
                    if executor.getDatabaseSid():
                        nodeDependencyOSH = modeling.createLinkOSH('node_dependency', remote_machineOSH,
                                                                   executor.getHostOsh())
                    ...
                    logger.debug('Resolve of machine name ', remote_machine_name, ' was local Ignoring client ',
                                 program_dataName)
    ...
    if dbSnapshotOwnerDbUserTableQueryRes is not None:
        ...
        while dbSnapshotOwnerDbUserTableQueryRes.next():
            ...
            if dbsnapshotOSH:
                ...
                logger.warn(
                    "Snapshot name %s, owner %s was not modeled. Link from user to snapshot will not be created." % (
                    dbsnapshotName, dbsnapshotOwner))
...
def parseDbVerifyCDBQueryRes(dbVerifyCDBQueryRes, oracleOSH, listenerOsh, serviceNames, OSHVResult):
    oracleCDBOSH = None
    while dbVerifyCDBQueryRes.next():
        if dbVerifyCDBQueryRes.getString(2).strip() == "YES" and dbVerifyCDBQueryRes.getString(3).strip() == "0":
            cdbName = dbVerifyCDBQueryRes.getString(1)
            oracleCDBOSH = ObjectStateHolder('oracle_cdb')
            oracleCDBOSH.setAttribute('name', cdbName)
            oracleCDBOSH.setContainer(oracleOSH)
            OSHVResult.add(oracleCDBOSH)
            for serviceName in serviceNames:
                if cdbName.lower() == serviceName.getName().lower():
                    serviceNameOSH = ObjectStateHolder("oracle_servicename")
                    serviceNameOSH.setAttribute('name', serviceName.getName())
                    serviceNameOSH.setContainer(listenerOsh)
                    OSHVResult.add(serviceNameOSH)
                    OSHVResult.add(modeling.createLinkOSH('realization', serviceNameOSH, oracleCDBOSH))
                    break

    return oracleCDBOSH


def parseDbOraclePDBQueryRes(dbOraclePDBQueryRes, oracleOSH, oracleCDBOSH, listenerOsh, serviceNames, OSHVResult):
    while dbOraclePDBQueryRes.next():
        if int(dbOraclePDBQueryRes.getString(1).strip()) > 2:
            pdbName = dbOraclePDBQueryRes.getString(2)
            oraclePDBOSH = ObjectStateHolder('oracle_pdb')
            oraclePDBOSH.setAttribute('name', pdbName)
            oraclePDBOSH.setContainer(oracleOSH)
            OSHVResult.add(oraclePDBOSH)
            OSHVResult.add(modeling.createLinkOSH('containment', oracleCDBOSH, oraclePDBOSH))
            for serviceName in serviceNames:
                if pdbName.lower() == serviceName.getName().lower() and serviceName.isPdb():
                    serviceNameOSH = ObjectStateHolder("oracle_servicename")
                    serviceNameOSH.setAttribute('name', serviceName.getName())
                    serviceNameOSH.setContainer(listenerOsh)
                    OSHVResult.add(serviceNameOSH)
                    OSHVResult.add(modeling.createLinkOSH('realization', serviceNameOSH, oraclePDBOSH))
                    break


def parseDbOracleServiceWithPDBQueryRes(dbOracleServiceWithPDBQueryRes):
    services = []
    while dbOracleServiceWithPDBQueryRes.next():
        name = dbOracleServiceWithPDBQueryRes.getString(1)
        pdb = dbOracleServiceWithPDBQueryRes.getString(2).strip() != '' and dbOracleServiceWithPDBQueryRes.getString(
            2) != 'CDB$ROOT'
...
def parseDbOracleServiceQueryRes(dbOracleServiceQueryRes):
    services = []
    while dbOracleServiceQueryRes.next():
        name = dbOracleServiceQueryRes.getString(1)
        service = db.OracleServiceName(name)
        services.append(service)
    return services


def getOracleServices(oracleClient):
    services = []
    try:
        res = oracleClient.executeQuery(dbOracleServiceWithPDBQuery)
        services = parseDbOracleServiceWithPDBQueryRes(res)
    except:
        logger.debugException('')
        try:
            res = oracleClient.executeQuery(dbOracleServiceQuery)
            services = parseDbOracleServiceQueryRes(res)
        except:
            logger.debugException('')
    return services


def identifyOracleCDB(OSHVResult, oracleClient, oracleOSH, listenerOsh, serviceNames):
    # verify whether it is a CDB
    try:
        res = oracleClient.executeQuery(dbVerifyCDBQuery)
        return parseDbVerifyCDBQueryRes(res, oracleOSH, listenerOsh, serviceNames, OSHVResult)
    except:
        logger.debug('Failed to get CDB info in table V$DATABASE')
    return None


def getPDBsConnectedToCDB(OSHVResult, oracleClient, oracleOSH, oracleCDBOSH, listenerOsh, serviceNames):
    # get all pdbs connected to current cdb
    res = oracleClient.executeQuery(dbOraclePDBQuery)
    parseDbOraclePDBQueryRes(res, oracleOSH, oracleCDBOSH, listenerOsh, serviceNames, OSHVResult)


def discoverDBTopology(OSHVResult, oracleClient, oracleOSH,isOracle=True,discoveredHostOSH = None):
    hostOSH = modeling.createHostOSH(oracleClient.getIpAddress(), 'node')
    if not isOracle:
        hostOSH = discoveredHostOSH
    ...
    if isOracle:
        ipCommunicationEndpoint = modeling.createServiceAddressOsh(hostOSH,
                                                               oracleClient.getIpAddress(),
                                                               oracleClient.getPort(),
                                                               modeling.SERVICEADDRESS_TYPE_TCP)
        usageLink = modeling.createLinkOSH('usage', oracleOSH, ipCommunicationEndpoint)
        OSHVResult.add(ipCommunicationEndpoint)
        OSHVResult.add(usageLink)
    OSHVResult.add(oracleOSH)

    listener = db.OracleListener(oracleClient.getIpAddress(), oracleClient.getPort())
    services = getOracleServices(oracleClient)
    if services:
        if isOracle:
            OSHVResult.addAll(reporter.reportTnsListener(listener, hostOSH))
        else:
            vector = ObjectStateHolderVector()
            listenerOsh = listener.build(reporter._builder)
            listenerOsh.setContainer(hostOSH)
            vector.add(listenerOsh)
            vector.add(hostOSH)
            OSHVResult.addAll(vector)

        OSHVResult.addAll(reporter.reportServiceNameTopology(services, listener.getOsh(), oracleOSH))
    # identify oracle cdb and pdb
    oracleCDBOSH = identifyOracleCDB(OSHVResult, oracleClient, oracleOSH, listener.getOsh(), services)
    if oracleCDBOSH:
        getPDBsConnectedToCDB(OSHVResult, oracleClient, oracleOSH, oracleCDBOSH, listener.getOsh(), services)
    else:
        logger.debug("It's not an Oracle CDB!")
...
def discoverOracle(oracleClient, oracleOSH, discoveredHostOSH, Framework):
    ...
    discoverDbLinks = Boolean.parseBoolean(Framework.getParameter("discoverDbLinks"))
    if discoverDbLinks:
        queries += \
            [
                Query(dbLinkobjTableQuery, parseDbLinkobjTableQueryRes)  # db link
            ]
    ...
    reportEmptySchemas = Boolean.parseBoolean(Framework.getParameter("reportEmptySchemas"))
    if discoveryUsers or comprehensiveDiscovery:
        if reportEmptySchemas:
            queries += \
                [
                    Query(dbUserTableQuery, discoverUserAndSchema(reportEmptySchemas))  # dbuser
                ]
        else:
            queries += \
                [
                    Query(dbSchemaQuery, discoverSchema),   #dbschema
                    Query(dbUserTableQuery, discoverUserAndSchema(reportEmptySchemas))  # dbuser
                ]
        ...
        if discoverDbLinks:
            queries += \
                [
                    ...
                    Query(dbLinkobjOwnerDbUserPublicTableQuery, parseDbLinkobjOwnerDbUserPublicTableQueryRes)
                    # dblinkobj, dbuser
                ]

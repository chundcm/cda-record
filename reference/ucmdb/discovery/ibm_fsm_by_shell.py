...
def DiscoveryMain(Framework):
    ...
    try:
        ...
        chassis, cmms = chassis_discoverer.discover()
        ...
        servers, vms, cmms_, fsms = server_discoverer.discover()
        cmms.extend(cmms_)
        ...
        logger.debug('Switches discovered %s' % switches)
        ...
        #remove invalid servers from the list
        servers = [x for x in servers if x.managedSystem]
        ...
        OSHVResult = ibm_fsm.ReportTopology(chassis, servers, system_pools, vms, switches, storages, fsm_osh, cmms, fsms, reportHostName)
    except:

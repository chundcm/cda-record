...
class ServiceGuardClusterDiscoverer:
    ...
    def _parseVersion(self, versionBuffer):
        ...
        if versionBuffer:
            ...
            for packageInfo in packageInfos:
                clusterVersion = re.search('\s+([\w\.\-]+)\s+Serviceguard\s*$', packageInfo) or re.search('\s*Service[Gg]uard\s+([\w\.\-]+)\s*', packageInfo)
    ...
    def getVersion(self):
        ...
        if self._version is None:
            try:
                resultBuffer = self._shell.execAlternateCmds("swlist | grep Serviceguard", "/usr/sbin/swlist | grep Serviceguard", "/usr/sbin/swlist -a revision -l product | grep -i serviceguard")

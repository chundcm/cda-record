...
class RestClient(object):
    ...
    def request(cls, method, url, params=None, headers=None):
        ...
        if body:
            body = body.strip()
            json_body = cls.toJson(body)
            nextPageUri = json_body.get("nextPageUri")
            while body and nextPageUri and not (nextPageUri in ('None', 'null')):
                next_url = url + '?' + urlparse.urlparse(nextPageUri).query
                logger.debug('Try to get next page result with URL: %s' % next_url)
                body = cls.__request(method, next_url, params, headers)
                if body:
                    body = body.strip()
                    json_body_page = cls.toJson(body)
                    nextPageUri = json_body_page.get("nextPageUri")
                    body_list.append(json_body_page)
            if body_list:
                body_list.append(json_body)
                return body_list
            return json_body
        return None

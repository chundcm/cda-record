...
#          IsReadOnly
"DATABASEPROPERTYEX(name, 'IsReadOnly') as _IsReadOnly from master..sysdatabases where name in (??)")
...
MASTER_FILES = String("select name, physical_name, size, max_size, growth, dbname, file_id, volume_mount_point" +
                      " from (select sysfiles.name, sysfiles.physical_name, sysfiles.[size] as size, sysfiles.max_size, sysfiles.growth, sysdatabases.[name] as dbname, sysfiles.file_id as fileid, sysfiles.database_id as database_id" +
                      " from sys.master_files as sysfiles" +
                      " INNER JOIN master..sysdatabases as sysdatabases ON sysfiles.database_id=sysdatabases.dbid" +
                      "	) as P " +
                      " CROSS APPLY sys.dm_os_volume_stats(P.database_id, P.fileid)")
...
GET_DATABASE_OF_AVAILABILITYGROUP=String("SELECT database_name"+
" FROM sys.availability_databases_cluster"+
" WHERE group_id = '??' ")

GET_LISTENER_IP_OF_AVAILABILITYGROUP=String("SELECT b.port,c.ip_address"+
" FROM sys.availability_groups_cluster a, sys.availability_group_listeners b, sys.availability_group_listener_ip_addresses c"+
" WHERE a.group_id = b.group_id and b.listener_id = c.listener_id and a.group_id = '??' ")

########################## ATTRS ###########
DATA_NAME = "data_name"

DATABASE_SIZES = "SELECT d.name, ROUND(SUM(mf.size) * 8 / 1024, 0) SizeInMB FROM sys.master_files mf INNER JOIN sys.databases d ON d.database_id = mf.database_id WHERE d.database_id > 4 GROUP BY d.name"

DATABASE_MIRRORING = "select database_name, principal_server_name, mirror_server_name from sys.database_mirroring_witnesses"
